...
#find matchString in the files from a specified filePath. optionally, if it exists, replace it with the newString value.
  ...
  $filePath = "\\MSP-DFS-01\InformationTechnology\UniTrac_Master_Configs\Test\QA2"
  ...
  $matchString = '<add key = "RPAService.WriteUrl" value = "http://rpaservices-test.alliedsolutions.net/RPAInbound/RPAInbound.svc/Write" /'


   # only used if doReplace is set to yes
  $newString =   '      <add key = "RPAService.Vehicle" value = "http://rpaservices-test.alliedsolutions.net/RPAInbound/RPAInbound.svc/VehicleVOW" />
