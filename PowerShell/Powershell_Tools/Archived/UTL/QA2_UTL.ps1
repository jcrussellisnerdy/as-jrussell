...
Get-ChildItem -Path "\\utqa2-app1\c$\inetpub\UTL API\UTLSupportService" -Recurse -ErrorAction SilentlyContinue | 
Remove-Item -Force -Recurse -ErrorAction SilentlyContinue
...
#BEFORE STARTING ENSURE THE SOURCEPATH ($SOURCEPATH) HAS BEEN UPDATED TO MOST CURRENT PATH BEFORE DEPLOYING
$sourcePath = "\\tfs-build-06\E$\UniTrac\Setups\Version 8.3\LDH\API\UTLSupportService"
$destPath =  "\\utqa2-app1\c$\inetpub\UTL API\UTLSupportService"
...
Get-ChildItem   "\\as.local\shared\InfoTech\UniTrac_Master_Configs\Test\QA2\UTLSupportService\Web.config" |Copy-Item -Destination  "\\utqa2-app1\c$\inetpub\UTL API\UTLSupportService" -Force
