...
$configRepository = "\\as.local\shared\InfoTech\UniTrac_Master_Configs\Test\STAGE"
Start-Sleep -s 3
...
New-Service -Name "$Service" -BinaryPathName """E:\Services\$Service\UnitracBusinessService.exe"" -SVCNAME:$Service -SVCTYPE:QUEUEPROCESSOR -QUEUESVC://UniTrac/UBSReadyToExecuteService" -DisplayName "$Service" -StartupType Automatic -Description "$Service"}
