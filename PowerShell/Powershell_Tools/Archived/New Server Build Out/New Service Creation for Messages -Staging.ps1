...
$configRepository = "\\MSP-DFS-01\InformationTechnology\UniTrac_Master_Configs\Test\STAGE"
Start-Sleep -s 3
...
Foreach ($service in $service_names){
   ...
   Copy-Item -Path "$configRepository\$service\*.exe.config" -Destination $destPath -Force
New-Service -Name "$Service" -BinaryPathName """E:\Services\$Service\Osprey.MessageServer.exe"" -SVCNAME:$Service" -DisplayName "$Service" -StartupType Automatic -Description "$Service"}

$UserName = "SQL2000@as.local" 
$Password = "sqlpower"
