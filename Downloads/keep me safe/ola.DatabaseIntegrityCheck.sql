USE [DBA]
GO
/****** Object:  StoredProcedure [ola].[DatabaseIntegrityCheck]    Script Date: 9/13/2019 12:23:28 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[ola].[DatabaseIntegrityCheck]') AND type in (N'P', N'PC'))
BEGIN
	EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [ola].[DatabaseIntegrityCheck] AS RETURN 0;' 
END
GO

ALTER PROCEDURE [ola].[DatabaseIntegrityCheck]
...
BEGIN
  ...
  IF NOT EXISTS (SELECT * FROM sys.objects objects INNER JOIN sys.schemas schemas ON objects.[schema_id] = schemas.[schema_id] WHERE objects.[type] = 'P' AND schemas.[name] = 'ola' AND objects.[name] = 'CommandExecute')
  BEGIN
  ...
  IF EXISTS (SELECT * FROM sys.objects objects INNER JOIN sys.schemas schemas ON objects.[schema_id] = schemas.[schema_id] WHERE objects.[type] = 'P' AND schemas.[name] = 'ola' AND objects.[name] = 'CommandExecute' AND OBJECT_DEFINITION(objects.[object_id]) NOT LIKE '%@LockMessageSeverity%')
  BEGIN
  ...
  IF @LogToTable = 'Y' AND NOT EXISTS (SELECT * FROM sys.objects objects INNER JOIN sys.schemas schemas ON objects.[schema_id] = schemas.[schema_id] WHERE objects.[type] = 'U' AND schemas.[name] = 'ola' AND objects.[name] = 'CommandLog')
  BEGIN
  ...
  IF @DatabasesInParallel = 'Y' AND NOT EXISTS (SELECT * FROM sys.objects objects INNER JOIN sys.schemas schemas ON objects.[schema_id] = schemas.[schema_id] WHERE objects.[type] = 'U' AND schemas.[name] = 'ola' AND objects.[name] = 'Queue')
  BEGIN
  ...
  IF @DatabasesInParallel = 'Y' AND NOT EXISTS (SELECT * FROM sys.objects objects INNER JOIN sys.schemas schemas ON objects.[schema_id] = schemas.[schema_id] WHERE objects.[type] = 'U' AND schemas.[name] = 'ola' AND objects.[name] = 'QueueDatabase')
  BEGIN
  ...
  BEGIN
    ...
    BEGIN TRY
      ...
      BEGIN
        ...
        SELECT @QueueID = QueueID
        FROM ola.[Queue] WITH (UPDLOCK, TABLOCK)
        WHERE SchemaName = @SchemaName
        AND ObjectName = @ObjectName
        AND [Parameters] = @Parameters
        ...
        IF @QueueID IS NULL
        BEGIN
          INSERT INTO ola.[Queue] (SchemaName, ObjectName, [Parameters])
      ...
      AND NOT EXISTS (SELECT *
                      FROM ola.QueueDatabase QueueDatabase
      ...
      IF @@ROWCOUNT = 1
      BEGIN
        INSERT INTO ola.QueueDatabase (QueueID, DatabaseName)
        SELECT @QueueID AS QueueID,
               DatabaseName
        FROM @tmpDatabases tmpDatabases
        WHERE Selected = 1
        AND NOT EXISTS (SELECT * FROM ola.QueueDatabase WHERE DatabaseName = tmpDatabases.DatabaseName AND QueueID = @QueueID)
  ...
  BEGIN
    ...
    BEGIN
      ...
      BEGIN
        ...
        EXECUTE @CurrentCommandOutput01 = [ola].[CommandExecute] @Command = @CurrentCommand01, @CommandType = @CurrentCommandType01, @Mode = 1, @DatabaseName = @CurrentDatabaseName, @LogToTable = @LogToTable, @Execute = @Execute
        SET @Error = @@ERROR
      ...
      BEGIN
        ...
        BEGIN
          ...
          BEGIN
            ...
            EXECUTE @CurrentCommandOutput04 = [ola].[CommandExecute] @Command = @CurrentCommand04, @CommandType = @CurrentCommandType04, @Mode = 1, @DatabaseName = @CurrentDatabaseName, @LogToTable = @LogToTable, @Execute = @Execute
            SET @Error = @@ERROR
      ...
      BEGIN
        ...
        EXECUTE @CurrentCommandOutput05 = [ola].[CommandExecute] @Command = @CurrentCommand05, @CommandType = @CurrentCommandType05, @Mode = 1, @DatabaseName = @CurrentDatabaseName, @LogToTable = @LogToTable, @Execute = @Execute
        SET @Error = @@ERROR
      ...
      BEGIN
        ...
        BEGIN
          ...
          BEGIN
            ...
            EXECUTE @CurrentCommandOutput08 = [ola].[CommandExecute] @Command = @CurrentCommand08, @CommandType = @CurrentCommandType08, @Mode = 1, @DatabaseName = @CurrentDatabaseName, @SchemaName = @CurrentSchemaName, @ObjectName = @CurrentObjectName, @ObjectType = @CurrentObjectType, @LogToTable = @LogToTable, @Execute = @Execute
            SET @Error = @@ERROR
      ...
      BEGIN
        ...
        EXECUTE @CurrentCommandOutput09 = [ola].[CommandExecute] @Command = @CurrentCommand09, @CommandType = @CurrentCommandType09, @Mode = 1, @DatabaseName = @CurrentDatabaseName, @LogToTable = @LogToTable, @Execute = @Execute
        SET @Error = @@ERROR
