USE [DBA]
GO
/****** Object:  StoredProcedure [ddbma].[RestoreDatabase]    Script Date: 9/13/2019 12:23:28 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[ddbma].[RestoreImage]') AND type in (N'P', N'PC'))
BEGIN
	EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [ddbma].[RestoreImage] AS RETURN 0;' 
END
GO

ALTER PROCEDURE [ddbma].[RestoreImage]
...
BEGIN
	...
	' -a "DDBOOST_COMPRESSED_RESTORE=TRUE" ' +
	...
	IF( @DryRun = 1 )
		BEGIN
			...
			DECLARE  @osCMD nvarchar(4000), @TargetInstanceName varchar (50),TargetDomain varchar(50);
			SET @TargetInstanceName = (select isnull(''MSSQL$''+ convert(varchar(max),serverproperty(''instancename'')),''MSSQL'') );
			SET @osCMD = '''+ @ddbmacmd +' "''+ @TargetInstanceName +'':'+ @SQLDatabaseName +'"'+ @SQLInstanceName + ':' + @SQLDatabaseName + '"'';
			
			PRINT @osCMD -- provided to check quotes
	...
	ELSE
		BEGIN
			...
			INSERT INTO @t
			exec xp_cmdshell @ddbmacmd --DBA DDBoost restore backup
			SET @cur = Cursor FOR
			SELECT LINE FROM @t
			open @cur
