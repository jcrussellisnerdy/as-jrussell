USE [DBA]
GO
/****** Object:  StoredProcedure [ola].[CommandExecute]    Script Date: 9/13/2019 12:23:28 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[ola].[CommandExecute]') AND type in (N'P', N'PC'))
BEGIN
	EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [ola].[CommandExecute] AS RETURN 0;' 
END
GO

ALTER PROCEDURE [ola].[CommandExecute]
...
BEGIN
  ...
  IF @LogToTable = 'Y'
  BEGIN
    INSERT INTO ola.CommandLog (DatabaseName, SchemaName, ObjectName, ObjectType, IndexName, IndexType, StatisticsName, PartitionNumber, ExtendedInfo, CommandType, Command, StartTime)
