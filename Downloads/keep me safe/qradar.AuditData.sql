...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[qradar].[AuditData]') AND type in (N'U'))
BEGIN
	CREATE TABLE [qradar].[AuditData](
...
/* Does it exist */
IF EXISTS (SELECT * FROM sys.indexes AS I WHERE I.object_id = OBJECT_ID('[qradar].[AuditData]') AND I.name = 'IX_AuditData')
BEGIN	
	...
	IF NOT EXISTS (
			...
			WHERE i.object_id = OBJECT_ID('[qradar].[AuditData]') 
...
IF NOT EXISTS (SELECT * FROM sys.indexes AS I WHERE I.object_id = OBJECT_ID('[qradar].[AuditData]') AND I.name = 'IX_AuditData')
BEGIN	
	CREATE INDEX IX_AuditData ON [qradar].[AuditData] ([event_time]);
END;

/* Drop misplaced table */
USE MASTER;
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[AuditData]') AND type in (N'U'))
BEGIN
	DROP TABLE [dbo].[AuditData]
END;