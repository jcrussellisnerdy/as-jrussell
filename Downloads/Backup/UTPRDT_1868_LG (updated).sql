...
BEGIN TRAN;
/* Declare variables */
DECLARE @Ticket NVARCHAR(15) =N'UTPRDT_1868';
...
IF NOT EXISTS (SELECT *
               ...
               WHERE name like  @Ticket+'%LG%' AND type IN (N'U') )
	BEGIN
		...
		SELECT  AGENCY_ID,	LENDER_ID,	LOGO_ID,	TYPE_CD,	DESCRPTION_TX,	XSLT_XML,	EXAMPLE_XML,	CREATE_DT,	UPDATE_DT,	UPDATE_USER_TX,	PURGE_DT,	LOCK_ID,	SUB_TYPE_CD
          		INTO UnitracHDStorage..UTPRDT_1868_temp_LG
          		FROM TEMPLATE
		...
		SELECT TEMPLATE_ID,	CONTENT_BLOCK_NAME_TX,	USE_LIBRARY_IN,	CREATE_DT,	UPDATE_DT,	UPDATE_USER_TX,	PURGE_DT,	LOCK_ID
				INTO UnitracHDStorage..UTPRDT_1868_CBA_LG
		...
		INSERT INTO UnitracHDStorage..UTPRDT_1868_temp_LG
		...
		INSERT INTO UnitracHDStorage..UTPRDT_1868_CBD_LG
		...
		INSERT INTO UnitracHDStorage..UTPRDT_1868_CBA_LG
		Select 1,	NAME_TX,	'Y',	GETDATE(),	GETDATE(),	@Ticket,	PURGE_DT,	LOCK_ID
		from #tmpCBDs
		Set @RowsInsertedCBA = @@ROWCOUNT

		INSERT INTO UnitracHDStorage..UTPRDT_1868_CBF_LG
		Select NAME_TX,	GETDATE(),	GETDATE(),	'UTPRDT_1868',	NULL,	1,	1,	GUID_TX
		from #tmpCBDs
		...
		IF( @RowsToInsertCBD = @RowsInsertedCBD and @RowsToInsertCBA = @RowsInsertedCBA and @RowsToInsertCBF = @RowsInsertedCBF and @RowsToInsertTemp = @RowsInsertedTemp )
			BEGIN
				...
				Select AGENCY_ID,	LENDER_ID,	LOGO_ID,	TYPE_CD,	DESCRPTION_TX,	XSLT_XML,	EXAMPLE_XML,	CREATE_DT,	UPDATE_DT,	UPDATE_USER_TX,	PURGE_DT,	LOCK_ID,	SUB_TYPE_CD
				FROM UnitracHDStorage..UTPRDT_1868_temp_LG
				Set @RowsInsertedTemp = @@ROWCOUNT
				...
				from UnitracHDStorage..UTPRDT_1868_CBD_LG
				Set @RowsInsertedCBD = @@ROWCOUNT
