...
/* Set variables */
DECLARE @JobName VARCHAR(200) =  'SHAVLIK data refresh',
	...
	@JobCategory VARCHAR(100) = 'SHAVLIK',
	@description VARCHAR(256),
	@notifyOperator varchar(100) = 'NOCAlert';

SELECT @description = 'Refreshes the CurrentPatchStatus and CurrentPatchCOUNT tables with the latest data' 
...
EXEC msdb.dbo.sp_update_jobstep 
	...
	@os_run_priority=0, 
	@subsystem=N'TSQL',
	@command=N'DECLARE @RETRYCOUNT INT = 1
			DECLARE @RETRYMAX INT = 3
			DECLARE @LASTBATCH INT
			DECLARE @LASTERROR NVARCHAR(4000)
			...
			EXEC dbo.xtr_UpdateData

			ALTER INDEX ALL ON dbo.xtrCurrentPatchStatus
			REORGANIZE ;
			ALTER INDEX ALL ON dbo.xtrLinuxCurrentPatchStatus
			REORGANIZE ;

			SELECT @LASTBATCH = MAX(BatchId) FROM dbo.xtrEntityProcessLog

			SELECT @LASTERROR = ErrorMessage FROM dbo.xtrEntityProcessErrorLog WHERE BatchId = @LASTBATCH

			--SELECT @LASTERROR

			WHILE (@RETRYCOUNT <= @RETRYMAX AND @LASTERROR LIKE ''7303:%'')
			BEGIN
				SET @LASTERROR = NULL
			...
			if @RETRYCOUNT >= @RETRYMAX AND @LASTERROR LIKE ''7303:%''
				raiserror(''The stored procedure xtr_UpdateData was not able to connect to the Security Controls database after multiple retries'',16,1)', 
		@database_name='SHAVLIK', 
		@flags=0
