...
use [IVOS]
IF NOT EXISTS (SELECT OBJECT_NAME(major_id),  USER_NAME(grantee_principal_id),* FROM sys.database_permissions where OBJECT_NAME(major_id) = 'pl_usr' and USER_NAME(grantee_principal_id) = 'IdentityNow_APP_ACCESS' AND permission_name ='UPDATE')
BEGIN   
		GRANT UPDATE ON [IVOS].[dbo].[pl_usr] TO [IdentityNow_APP_ACCESS] 
