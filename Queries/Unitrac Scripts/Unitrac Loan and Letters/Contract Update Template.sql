...
SELECT  * --INTO UniTracHDStorage.dbo.LOAN_####_1
FROM    dbo.LOAN
WHERE   LENDER_ID IN (  )
        AND NUMBER_TX IN (  )

SELECT * FROM UniTracHDStorage.dbo.LOAN_####_1
...
SELECT 'Allied.UniTrac.Loan',LOAN.ID,'INC0xxxxxx','N','2016-04-01 00:00:00.000',1,'Y','N',1,'Allied.UniTrac.Loan',LOAN.ID,'PEND','N'
FROM LOAN
WHERE ID IN (SELECT ID FROM UnitracHDStorage.dbo.LOAN_####_1)
--42
...
select PROPERTY_CHANGE.ID,'LOAN',ENTITY_ID,'NUMBER_TX',UnitracHDStorage.dbo.LOAN_####_1.NUMBER_TX,LOAN.NUMBER_TX,1,'2016-04-01 00:00:00.000','Y','U'
from PROPERTY_CHANGE
INNER JOIN LOAN ON PROPERTY_CHANGE.ENTITY_ID = LOAN.ID AND ENTITY_NAME_TX = 'Allied.UniTrac.Loan' 
INNER JOIN UnitracHDStorage.dbo.LOAN_####_1 ON LOAN.ID = UnitracHDStorage.dbo.LOAN_####_1.ID --AND ENTITY_NAME_TX = 'Allied.UniTrac.Loan' 
WHERE PROPERTY_CHANGE.CREATE_DT = '2016-04-01 00:00:00.000'
AND ENTITY_ID IN (SELECT ID FROM UnitracHDStorage.dbo.LOAN_####_1)
--0
...
SELECT dbo.LOAN.ID, dbo.LOAN.NUMBER_TX,dbo.LOAN.EFFECTIVE_DT,GETDATE(),GETDATE(),'INC0xxxxxx',1
FROM UnitracHDStorage.dbo.LOAN_####_1 HD INNER JOIN dbo.LOAN ON HD.ID = LOAN.ID
--42
...
AND LOAN_ID in (select ID FROM UnitracHDStorage.dbo.LOAN_####_1)
--42
