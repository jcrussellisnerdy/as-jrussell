...
SET ntc.TEMPLATE_ID = 4

FROM NOTICE ntc

WHERE ntc.TEMPLATE_ID = 10

AND ntc.ID IN (SELECT ID FROM NOTICE WHERE ID IN (SELECT RELATE_ID FROM PROCESS_LOG_ITEM WHERE PROCESS_LOG_ID = 75775077 AND RELATE_TYPE_CD = 'Allied.UniTrac.Notice'))
