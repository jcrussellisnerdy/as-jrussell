...
---- CASE I
---- OVER LAP WITH DIVISION & COVERAGE TYPE
---- DROP TABLE #TMPCR_01
...
SELECT CODE_TX , NAME_TX ,  EFFECTIVE_DT , RPARAMVALUE , RPARAMVALUE_01
FROM #TMPCR_02
GROUP BY CODE_TX , NAME_TX , EFFECTIVE_DT , RPARAMVALUE , RPARAMVALUE_01
HAVING COUNT(*) > 1
...
WHERE  (
           DATEDIFF( DAY , T1.EFFECTIVE_DT , T2.EXPIRATION_DT) > 0 AND
           DATEDIFF( DAY , T2.EFFECTIVE_DT , T1.EXPIRATION_DT ) > 0 
       ) 
...
----- WITH ONLY DIVISION SET
SELECT CODE_TX , NAME_TX ,  EFFECTIVE_DT , RPARAMVALUE 
FROM #TMPCR_03
GROUP BY CODE_TX , NAME_TX , EFFECTIVE_DT , RPARAMVALUE 
HAVING COUNT(*) > 1
---- 0
