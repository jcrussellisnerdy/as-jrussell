...
INNER JOIN DELIVERY_INFO_GROUP DIG ON DI.ID = DIG.DELIVERY_INFO_ID
INNER JOIN DELIVERY_DETAIL DD ON DIG.ID = DD.DELIVERY_INFO_GROUP_ID
WHERE DD.VALUE_TX LIKE '%GreatPlains%' and external_id_tx = '7545'
--16
...
INNER JOIN DELIVERY_INFO_GROUP DIG ON DI.ID = DIG.DELIVERY_INFO_ID
INNER JOIN DELIVERY_DETAIL DD ON DIG.ID = DD.DELIVERY_INFO_GROUP_ID
WHERE TP.ID IN (2524) and dig.name_tx = '.*.xlsx'
...
--7) Carrier TP is not setup (no 820 process will be created)
