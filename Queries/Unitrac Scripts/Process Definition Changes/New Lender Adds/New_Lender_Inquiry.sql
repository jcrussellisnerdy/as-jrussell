...
SELECT
DISTINCT  RC.MEANING_TX,
TNL.Agency,
	 TNL.[Lender Code] ,
	TNL.[Lender Name] ,
	LDR.CODE_TX,
	LDR.NAME_TX,
	tpc.ProcId [ProcessId 39], tpd.ProcId [Outbound LDH], tpe.ProcId[MessageServer],PD.STATUS_CD [Staus]
...
LEFT JOIN REF_CODE RC on RC.CODE_CD = TNL. [Product Tracking Type]
WHERE [Product Tracking Type] <> 'ORDERUP' 
