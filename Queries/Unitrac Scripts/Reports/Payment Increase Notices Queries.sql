...
WHERE L.CODE_TX = '3900' AND DESCRIPTION_TX like 'Payment Increase %'
...
WHERE NAME_TX LIKE '%3900%' AND ACTIVE_IN = 'Y' AND ONHOLD_IN = 'N' AND PROCESS_TYPE_CD = 'CYCLEPRC'


--date preference would start at the 1st of the previous month 
SELECT ID INTO #tmpPL FROM dbo.PROCESS_LOG
WHERE PROCESS_DEFINITION_ID IN (SELECT * FROM #tmpPD) AND UPDATE_DT >= '2019-07-01' 
AND STATUS_CD = 'Complete' AND END_DT IS NOT NULL
...
WHERE REPORT_ID IN (58) AND RH.ID IN (SELECT * FROM #tmpRH) and 
rh.REPORT_DATA_XML.value( '(/ReportData/Report/Title/@value)[1]', 'varchar(500)' ) = 'Payment Increase Notices - Default PI Notice'
...
SELECT * into
UnitracHDStorage..INC0437806
FROM dbo.REPORT_HISTORY
WHERE   ID IN (19660054) AND REPORT_ID = '58'
...
REPORT_DATA_XML.modify('replace value of (/ReportData/Report/StartDate/@value)[1] with "2019-07-02T04:17:27"') 
--	, REPORT_DATA_XML = ''
--		,REPORT_ID = '27'
...
UPDATE  RH
SET     STATUS_CD = ic.status_cd ,
        RETRY_COUNT_NO = ic.RETRY_COUNT_NO ,
        MSG_LOG_TX = ic.MSG_LOG_TX ,
        RECORD_COUNT_NO = ic.RECORD_COUNT_NO ,
        ELAPSED_RUNTIME_NO = ic.ELAPSED_RUNTIME_NO, 
		UPDATE_DT = GETDATE(), DOCUMENT_CONTAINER_ID = ic.DOCUMENT_CONTAINER_ID	
--SELECT * 
FROM dbo.REPORT_HISTORY rh
join UnitracHDStorage..INC0437806 ic on ic.id = rh.id 
...
SELECT RELATE_ID INTO #tmpFPC 
FROM dbo.PROCESS_LOG_ITEM
WHERE PROCESS_LOG_ID = '68416582' AND RELATE_TYPE_CD = 'Allied.UniTrac.ForcePlacedCertificate'
-----------------------------------
...
select * INTO UniTracHDStorage..INC0390067_FPC
from FORCE_PLACED_CERTIFICATE
WHERE ID IN (SELECT * FROM #tmpFPC)
ORDER BY FORCE_PLACED_CERTIFICATE.ID
...
JOIN  UniTracHDStorage..INC0390067_FPC fpc2 ON fpc.id=fpc2.id
...
WHERE PROCESS_TYPE_CD LIKE '%good%'
