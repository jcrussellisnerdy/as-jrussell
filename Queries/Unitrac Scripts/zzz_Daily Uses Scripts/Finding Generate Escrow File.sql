...
Generate Escrow File - Process Definition (Id=962508) was created
Generate Escrow File - Process Definition (Id=963837) was created
...
where id in (962508,963837) 

select * from process_definition p
JOIN dbo.LENDER L ON L.CODE_TX = p.SETTINGS_XML_IM.value('(/ProcessDefinitionSettings/LenderCode)[1]','INT')
where process_type_cd  ='ESCBILFIL'
and l.code_tx in ('2258','1065','2468','3525') and p.create_dt >= '2019-06-06'
...
join #tmpPD pd2  on pd2.id = pl.process_definition_id
...
where log_message like '%54414764%'
order by tpl.create_dt asc



select * from message
where id in (19518703,19506746, 19441432) or relate_id_tx in (19518703,19506746,19441432) 
