SELECT * FROM UniTrac..DELIVERY_DETAIL WHERE VALUE_TX LIKE '%\vut-app%' ORDER BY UPDATE_DT
SELECT * FROM UniTrac..PREPROCESSING_DETAIL WHERE VALUE_TX LIKE '\%'

SELECT * FROM UniTrac..DELIVERY_DETAIL
WHERE DELIVERY_INFO_GROUP_ID = '5689'

--DELIVERY DETAILS
BEGIN

update DELIVERY_DETAIL set VALUE_TX = replace(VALUE_TX, '\\vut-app\Eastern\Data\4804 JD Byrider\ArchiveInput', 'C:\LenderFiles') 
...
--added 3/1/17-jcr
update DELIVERY_DETAIL set VALUE_TX = replace(VALUE_TX, '\\vut-app\Escrow\InsFile\3039\Input', 'C:\LenderFiles') 
where DELIVERY_CODE_TX in ('InputFolder', 'ArchInFolder');

update DELIVERY_DETAIL set VALUE_TX = replace(VALUE_TX, '\\vut-app\Escrow\InsFile\3039\Output', 'C:\LenderFiles') 
where DELIVERY_CODE_TX in ('OutputFolder', 'ArchOutFolder');

update DELIVERY_DETAIL set VALUE_TX = replace(VALUE_TX, '\\vut-app\Escrow\InsFile\3039\ArchiveOutput', 'C:\LenderFiles') 
where DELIVERY_CODE_TX in ('OutputFolder', 'ArchOutFolder');
