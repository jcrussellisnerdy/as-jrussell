--IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Support_BackoffNotice]') AND type in (N'P', N'PC'))
--DROP PROCEDURE [dbo].[Support_BackoffNotice]
--GO

-- 4/16/14 RRankins - changed from drop/create to ALTER PROC 
--   because this proc is replicated
-- the dummy CREATE is needed in case the proc does not already exist

IF NOT EXISTS (select * from sys.procedures where name = 'Support_BackoffNotice')
	Exec ('CREATE PROC Support_BackoffNotice as return')
Go

ALTER PROCEDURE [dbo].[Support_BackoffNotice]
(
   @noticeId bigint
)
