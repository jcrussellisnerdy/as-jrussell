...
and LL.CODE_TX IN ('XXXX')  and ih.effective_dt between '' and ''
...
*/
select ID, relate_id, * from unitrachdstorage..INC0XXXXX



---Purging the Interaction History 
update IH
set purge_dt = GETDATE(), UPDATE_dt =GETDATE(), update_user_tx = 'INC0XXXXX',LOCK_ID = CASE WHEN ih.LOCK_ID >= 255 THEN 1 ELSE ih.LOCK_ID + 1 END
--select *
...
--Purging the notices 
update n
set purge_dt = GETDATE(), UPDATE_dt =GETDATE(), update_user_tx = 'INC0XXXXX',LOCK_ID = CASE WHEN n.LOCK_ID >= 255 THEN 1 ELSE n.LOCK_ID + 1 END
--select *
from NOTICE n 
where id in (select RELATE_ID from unitrachdstorage..INC0XXXXX)
