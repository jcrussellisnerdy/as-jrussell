...
----REPLACE 1868 WITH THE THE Lender ID
----REPLACE INC0254957 WITH THE TICKET ID
---Checking UTLs WIs
USE UniTrac
...
--INTO UniTracHDStorage..INC0254957
FROM    WORK_ITEM
WHERE   RELATE_TYPE_CD = 'Allied.UniTrac.UTLMatchResult'
        ...
        WHERE   LENDER.CODE_TX = '1868' AND WORK_ITEM.STATUS_CD NOT IN ('Complete', 'Withdrawn')
			   )
...
WHERE WORK_ITEM.STATUS_CD NOT IN ('Complete', 'Withdrawn')) AND WORK_ITEM.LENDER_ID = '374'
...
SET     STATUS_CD = 'Complete' ,
        UPDATE_USER_TX = 'INC0254957',
		 LOCK_ID = CASE WHEN LOCK_ID >= 255 THEN 1
                  ELSE LOCK_ID + 1 END
WHERE   ID IN  (  SELECT ID FROM UniTracHDStorage..INC0254957)
...
SELECT * INTO UniTracHDStorage..LOAN_INC0254957
