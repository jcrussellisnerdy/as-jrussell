...
SELECT #TMPPROPERTY.LoanNumber, PR.REPLACEMENT_COST_VALUE_NO [Current RCV], PC.Amount_no
INTO UniTracHDStorage..INC0228273_CurrentRCV
FROM PROPERTY PR 
...
----- SELECT PR.REPLACEMENT_COST_VALUE_NO , PC.BASE_AMOUNT_NO
FROM PROPERTY PR 
JOIN #TMPPROPERTY on #TMPPROPERTY.PR_ID = PR.ID
JOIN #TMPPLCY on #TMPPLCY.PROPERTY_ID = PR.ID
JOIN POLICY_COVERAGE PC on PC.ID = #TMPPLCY.PC_ID
 ...
 SELECT DISTINCT  'Allied.UniTrac.Property' , PR.ID  , 'INC0228273' , 'N' , 
 GETDATE() ,  1 , 
 'Changed RCV from ' + convert(varchar(20), ISNULL(REPLACEMENT_COST_VALUE_NO,0))  + ' to ' + 
 ISNULL(convert(varchar(20), CAST(AMOUNT_NO AS DECIMAL(18,2))), 'NULL') , 
 'N' , 'Y' , 1 ,  'Allied.UniTrac.Property' , PR.ID  , 'PEND' , 'N'
FROM PROPERTY PR 
JOIN #TMPPROPERTY on #TMPPROPERTY.PR_ID = PR.ID
JOIN #TMPPLCY on #TMPPLCY.PROPERTY_ID = PR.ID
JOIN POLICY_COVERAGE PC on PC.ID = #TMPPLCY.PC_ID
...
JOIN #TMPPLCY on #TMPPLCY.PROPERTY_ID = PR.ID
JOIN POLICY_COVERAGE PC on PC.ID = #TMPPLCY.PC_ID
JOIN dbo.REQUIRED_COVERAGE RC ON RC.PROPERTY_ID = PR.ID
WHERE RC.TYPE_CD = 'HAZARD'
------ 11262



SELECT L.NUMBER_TX FROM LOAN L
...
SELECT * INTO UniTracHDStorage..INC0228273_PLCY
FROM #TMPPLCY

SELECT * INTO UniTracHDStorage..INC0228273_Property_2
FROM #TMPPROPERTY