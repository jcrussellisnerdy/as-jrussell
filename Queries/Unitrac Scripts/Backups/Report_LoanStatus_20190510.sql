...
/****** Object:  StoredProcedure [dbo].[Report_LoanStatus]    Script Date: 5/10/2019 8:23:55 AM ******/
SET ANSI_NULLS ON
GO
...
/*TFS#48839:
	select
	 @ReportType='FLDZNRPT'
	,@SpecificReport='FLDZNRPT_UCP'
	--,@debug=1

	--,@Report_History_ID='18094173'
	--OR:
	--,@LenderCode='1385'
	--,@LenderCode='4286' --RoyalCU
	--,@LenderCode='2252' --SpaceCoast
	--,@LenderCode='1630'--,@Report_History_ID=16815758 --MID Minnesota FCU
	,@LenderCode='2771'--,@Report_History_ID=16816887 --PenFed, staging
	,@LenderCode='2771'--,@Report_History_ID= --PenFed, qa2
*/
...
DECLARE @ProcessLogID as bigint = NULL
...
--OWNER:
	...
	[OWNER_CREDIT_SCORE_TX] [nvarchar](20) NULL,
	FLOOD_INSURANCE_TYPE_CD nvarchar(10) ,--NOT NULL,
	FLOOD_INSURANCE_TYPE_MEANING nvarchar(1000) NULL,
--PROPERTY:
	[COLLATERAL_YEAR_TX] [nvarchar](4) NULL,
	[COLLATERAL_MAKE_TX] [nvarchar](30) NULL,
	[COLLATERAL_MODEL_TX] [nvarchar](30) NULL,
...
--COVERAGE:
   ...
   [NEXT_NOTICE_DT] [datetime2](7) NULL,
--INSURANCE:
...
-- PARAMETERS:
	...
	[REPORT_FOOTER_TX] [nvarchar](4000) NULL,
...
Declare @FooterTx as varchar(4000)
...
if @debug>0
	select @FilterBySQL as '@FilterBySQL';

DECLARE @IS_CUMULATIVE AS VARCHAR(1) = 'F'
SELECT @IS_CUMULATIVE = ISNULL(VALUE_TX,'F') FROM #tmpfilter WHERE ATTRIBUTE_CD = 'FIL_IsCumulative'

IF @BeginDate IS NULL OR @EndDate IS NULL 
BEGIN

   if @IS_CUMULATIVE = 'T' AND @ProcessLogID is not null
   begin
      
      -- Get information from the proces log and its process defintion
      declare @HasDailyCycles bit
      declare @LastFullCycleRunDate datetime

      SELECT   @HasDailyCycles = CASE WHEN PD.SETTINGS_XML_IM.value('(/ProcessDefinitionSettings/LastCycleMode)[1]', 'varchar(20)') IS NULL THEN 0 ELSE 1 END,
               @LastFullCycleRunDate = ISNULL(PD.SETTINGS_XML_IM.value('(/ProcessDefinitionSettings/LastFullCycleDate)[1]', 'datetime'), PD.LAST_RUN_DT)
      FROM     PROCESS_LOG PL
               INNER JOIN PROCESS_DEFINITION PD ON PL.PROCESS_DEFINITION_ID = Pd.ID
      WHERE    PL.ID = @ProcessLogID

      if @HasDailyCycles = 1
         SET @BeginDate = @LastFullCycleRunDate
...
N'
	   ...
	   SUBSTRING(@FillerZero, 1, 15 - len(C.LOAN_BALANCE_NO)) + CAST(C.LOAN_BALANCE_NO AS nvarchar(22)) AS [LOAN_BALANCESORT_TX],
...
OWNER_ZIP_TX,
OWNER_CREDIT_SCORE_TX,
FLOOD_INSURANCE_TYPE_CD,
FLOOD_INSURANCE_TYPE_MEANING,
--PROPERTY:
COLLATERAL_YEAR_TX,
COLLATERAL_MAKE_TX,
COLLATERAL_MODEL_TX,
...
OP.FLOOD_INSURANCE_TYPE_CD,
FLOOD_INSURANCE_TYPE_MEANING
 = (SELECT TOP 1 RC.MEANING_TX FROM REF_CODE AS RC WITH(NOLOCK)
    WHERE RC.DOMAIN_CD = ''FloodInsuranceType''
	  AND RC.CODE_CD = OP.FLOOD_INSURANCE_TYPE_CD
...
'
   if @IS_CUMULATIVE = 'T'
      select @columns = @columns + '
         NE.NEXT_NOTICE_DT as [NEXT_NOTICE_DT],
'
   else
      select @columns = @columns + '
         NULL as [NEXT_NOTICE_DT],
'
...
(
   SELECT   ISNULL(
      (
      SELECT   TOP 1 EVENT_DT
      FROM     EVALUATION_EVENT EE_ORDERED
      WHERE    GROUP_ID = 
               (
                  SELECT   MAX(GROUP_ID)
                  FROM     EVALUATION_EVENT EE
                  WHERE    REQUIRED_COVERAGE_ID = REQUIREDCOVERAGE_ID
                           AND STATUS_CD = ''PEND''
               ) 
...
if @debug = 1
begin
	select @insert as [@insert:ln_1801]
	select @columns as [@columns:ln_1802]
	select @joins as [@joins:ln_1803]
	select isnull(@conditions,'') as [@conditions:ln_1804]
	select replace(@finalquery, @insert, '') as [@selectquery:ln_1805]
end
...
--select @sqlstring
EXECUTE sp_executesql @sqlstring, N'@FooterTx NVARCHAR(4000)', @FooterTx
