...
----REPLACE 1572 WITH THE THE Lender ID
----REPLACE INC0243524 WITH THE TICKET ID
---Checking UTLs WIs
USE UniTrac

SELECT  * 
--INTO UniTracHDStorage..INC0243524
FROM    WORK_ITEM
...
ORDER BY WORK_ITEM.STATUS_CD ASC 
	...
	SELECT CONTENT_XML.value('(/Content/Collateral/Type)[1]', 'varchar (50)') [Collateral Type], * 
	INTO UniTracHDStorage..INC0243524
	FROM UniTrac..WORK_ITEM
	WHERE CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') = '1572'
	AND WORKFLOW_DEFINITION_ID IN (2,3,6,8) AND CONTENT_XML.value('(/Content/Collateral/Type)[1]', 'varchar (50)') <> 'VEH'
...
SET     STATUS_CD = 'Complete' ,
        UPDATE_USER_TX = 'INC0243524',
		 LOCK_ID = CASE WHEN LOCK_ID >= 255 THEN 1
                  ELSE LOCK_ID + 1 END
WHERE   ID IN  (  SELECT ID FROM UniTracHDStorage..INC0243524)
...
WHERE   LENDER.CODE_TX = '1572'
       ...
       -- AND LOAN.RECORD_TYPE_CD IN ('G','A')
		AND CONTENT_XML.value('(/Content/Collateral/Type)[1]', 'varchar (50)') <> 'VEH'

SELECT * FROM #TMPWI

SELECT * INTO UniTracHDStorage..LOAN_INC0243524
