...
SELECT CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') Lender, * INTO UniTracHDStorage..WI_INC0217736
FROM    UniTrac..WORK_ITEM
WHERE   ID IN ( 26183984   )

SELECT * FROM UniTracHDStorage..WI_INC0217736
...
WHERE   LENDER.CODE_TX = '1220'
		...
		AND WI.ID IN (26183984 )
		
SELECT * FROM #TMPWI

SELECT * INTO UniTracHDStorage..LOAN_INC0217736
FROM dbo.LOAN WHERE ID IN (SELECT LOAN_ID FROM #TMPWI)

SELECT * FROM UniTracHDStorage..LOAN_INC0217736
