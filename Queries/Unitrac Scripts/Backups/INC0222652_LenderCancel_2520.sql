...
SELECT * 
--INTO UniTracHDStorage..INC0222652
FROM UniTrac..WORK_ITEM
WHERE CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') = '2520'
...
SELECT * FROM UniTracHDStorage..INC0222652

SELECT ID FROM UniTracHDStorage..INC0222652

-------- Clear UTLs (- rows)
UPDATE  UniTrac..WORK_ITEM
SET     STATUS_CD = 'Complete' ,
        UPDATE_USER_TX = 'INC0222652',
		 LOCK_ID = CASE WHEN LOCK_ID >= 255 THEN 1
                  ELSE LOCK_ID + 1 END
WHERE   ID IN  ( SELECT ID FROM UniTracHDStorage..INC0222652)
...
SELECT * INTO UniTracHDStorage..LOAN_INC0222652
...
ORDER BY [ServiceName]
	...
	FROM #process P
	left JOIN #reprocess R oN R.CODE_TX = P.CODE_TX
