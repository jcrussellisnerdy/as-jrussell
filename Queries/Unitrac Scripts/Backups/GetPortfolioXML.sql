/****** Object:  StoredProcedure [dbo].[GetPortfolioXML]    Script Date: 05/08/2013 09:56:49 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[GetPortfolioXML]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[GetPortfolioXML]
GO
...
CREATE PROCEDURE [dbo].[GetPortfolioXML] (@lenderId bigint,
@branchId bigint = NULL,
@divisionId bigint = NULL,
@coverageTypeCode nvarchar(1000) = NULL,
@lccgId bigint = NULL,
@collCodeId bigint = NULL,
@activeLoansIn char(1) = 'N', -- => activeLoansOnlyIn
...
declare @escrowInMatch char(1) = @escrowIn
	...
	DECLARE @cvgTypeCodeTable TABLE (
		cvgTypecode NVARCHAR(30)
	)
	...
	BEGIN
		...
		INSERT INTO @tmpCC (CC_ID)
			...
			INNER JOIN LCCG_COLLATERAL_CODE_RELATE r
				ON r.LCCG_ID = lccg.id
			INNER JOIN COLLATERAL_CODE cc
				ON cc.id = r.COLLATERAL_CODE_ID
			WHERE r.PURGE_DT IS NULL
				AND cc.PURGE_DT IS NULL
				AND lccg.id = @lccgId
	END

	IF (ISNULL(@collCodeId, 0) > 0
	...
	DECLARE @tmpLoans TABLE (
		LOAN_ID BIGINT,
		PROPERTY_ID BIGINT,
		REQ_ESCROW_IN VARCHAR(1)
	)
	INSERT INTO @tmpLoans (LOAN_ID,
	PROPERTY_ID, 
	REQ_ESCROW_IN)
		SELECT DISTINCT	ln.id,
						p.id,
						CASE 
							WHEN re.ID IS NULL THEN 'N'
							ELSE 'Y'
						END
		FROM LOAN ln
		...
		LEFT JOIN LENDER_ORGANIZATION LO_BRANCH
			ON (LO_BRANCH.LENDER_ID = LN.LENDER_ID
		...
		WHERE ln.LENDER_ID = @lenderId
			...
			AND (@filterByCollCode = 'N'
			OR c.COLLATERAL_CODE_ID IN (SELECT
				cc_id
			FROM @tmpCC)
			)
			...
			AND ((@activeLoansIn = 'N'
			AND LN.STATUS_CD IN ('A', 'B', 'F', 'L', 'N'))
			OR (@activeLoansIn = 'Y'
			AND LN.STATUS_CD = 'A'))
			AND (rc.STATUS_CD IN ('A', 'M', 'D', 'T'))
			AND c.Status_Cd = 'A'
			AND ((@includeUTLs = 'N'
			AND LN.RECORD_TYPE_CD IN ('G'))
			OR (@includeUTLs = 'Y'
			AND LN.RECORD_TYPE_CD IN ('G', 'E', 'I', 'U')))
			AND (@coverageTypeCode IS NULL
			OR rc.TYPE_CD IN (SELECT
				cvgTypeCode
			FROM @cvgTypeCodeTable)
			)
			AND ((@escrowInMatch = 'Y' and NOT re.ID IS NULL)
			OR (@escrowInMatch = 'N' and re.ID IS NULL)
			...
			AND (@uninsuredLoansOnlyIn = 'N'
			OR rc.INSURANCE_STATUS_CD IN ('A', 'C', 'CH', 'E', 'EH', 'N')
			OR rc.INSURANCE_SUB_STATUS_CD IN ('I'))
			AND (@excludeLoansWithCPIIn = 'N'
			OR rc.SUMMARY_SUB_STATUS_CD NOT IN ('P', 'C'))
			AND LN.EXTRACT_UNMATCH_COUNT_NO = 0

	--Remove inactive RE when coll/cov has active RE and inactive RE, or we get duplicate loan/prop entries.
	DELETE from @tmpLoans
	where PROPERTY_ID IN
		(
		SELECT PROPERTY_ID from @tmpLoans
		group by LOAN_ID, PROPERTY_ID
		having COUNT(PROPERTY_ID) > 1 and COUNT(LOAN_ID) > 1 
		)
	AND REQ_ESCROW_IN = 'N'
	...
	SELECT	ISNULL(rd.VALUE_TX, 'N') AS [@directPayIn],
			...
			(SELECT
				...
				NULLIF(rd.VALUE_TX, '') AS [@misc1], -- In VUT it is used for Old Policy number,
				[@dlnqDt] =
							CASE
								WHEN GETDATE() >= DATEADD(DAY, 120, LN.NEXT_SCHEDULED_PAYMENT_DT) THEN CONVERT(CHAR(8), DATEADD(DAY, 120, LN.NEXT_SCHEDULED_PAYMENT_DT), 112)
								ELSE NULL
							END,
				...
				(SELECT
					...
					NULLIF(P.ACV_NO, 0) AS [@acv], --COVERAGES, POLICIES, NOTICE, CERT
					(SELECT
						RC.id AS [@id],
						RC.TYPE_CD AS [@typCd],
						...
						[@fpcInd] =
									CASE
										WHEN FPC_INFO.NUMBER_TX IS NULL THEN 'N'
										ELSE 'Y'
									END,
						...
						(SELECT
							...
							NULLIF(OP.BASE_PROPERTY_TYPE_CD, '') AS [@basePropType],
							(SELECT
								NULLIF(PC2.SUB_TYPE_CD, '') AS [@subTypCd],
							...
							FOR XML PATH ('PlcyCvg'), TYPE)
							AS 'PlcyCvgs'
						FROM OWNER_POLICY OP
						LEFT JOIN ESCROW e
							ON (e.PROPERTY_ID = RC.PROPERTY_ID
							AND LN.ID = e.LOAN_ID
							AND e.END_DT > GETDATE())
						LEFT JOIN BORROWER_INSURANCE_AGENCY BIA
							ON BIA.id = OP.BIA_ID
						LEFT JOIN BORROWER_INSURANCE_COMPANY_OFFERING bico
						    ON (bico.BORROWER_INSURANCE_COMPANY_ID = OP.BIC_ID
							AND bico.COVERAGE_TYPE_CD = @coverageTypeCode)
						WHERE BIA.PURGE_DT IS NULL
							AND OP.id IN (SELECT ID FROM dbo.GetCurrentCoverage(RC.PROPERTY_ID, RC.ID, RC.TYPE_CD))
							AND (@escrowInMatch = REQ_ESCROW_IN
								OR @escrowInMatch IS NULL)
						FOR XML PATH ('Plcy'), TYPE)
						AS 'Plcys'

					FROM REQUIRED_COVERAGE RC
					LEFT JOIN REF_CODE stat
						ON stat.CODE_CD = rc.INSURANCE_STATUS_CD
						AND stat.DOMAIN_CD = 'RequiredCoverageInsStatus'
					LEFT JOIN REF_CODE substat
						ON substat.CODE_CD = rc.INSURANCE_SUB_STATUS_CD
						AND substat.DOMAIN_CD = 'RequiredCoverageInsSubStatus'
					LEFT JOIN REF_CODE typ
						ON typ.CODE_CD = rc.TYPE_CD
						AND typ.DOMAIN_CD = 'Coverage'
					--NOTICE          
					LEFT JOIN REF_CODE ntcType
						ON ntcType.CODE_CD = RC.NOTICE_TYPE_CD
						AND ntcType.DOMAIN_CD = 'NoticeType'
					--FPC
					LEFT JOIN (SELECT
						FPCRCR.REQUIRED_COVERAGE_ID,
						FPC.id,
						FPC.NUMBER_TX,
						FPC.EFFECTIVE_DT,
						FPC.CANCELLATION_DT,
					...
					INNER JOIN FORCE_PLACED_CERTIFICATE FPC
						ON FPC.id = FPCRCR.FPC_ID
						AND FPC.LOAN_ID = LN.id
						AND FPC.ACTIVE_IN = 'Y'
					INNER JOIN 
						(SELECT CPI_QUOTE_ID AS CPI_QUOTE_ID, 
						SUM(TOTAL_PREMIUM_NO) AS TOTAL_PREMIUM_NO
						FROM CPI_ACTIVITY WHERE	TYPE_CD IN ('I', 'C', 'MT') AND PURGE_DT IS NULL
						GROUP BY CPI_QUOTE_ID
 						) AS ca
						ON ca.CPI_QUOTE_ID = FPC.CPI_QUOTE_ID
					WHERE FPCRCR.PURGE_DT IS NULL
					AND FPC.PURGE_DT IS NULL) FPC_INFO
						ON FPC_INFO.REQUIRED_COVERAGE_ID = RC.id
					--waive
					...
					AND purge_dt IS NULL) WT
						ON WT.REQUIRED_COVERAGE_ID = rc.id
					LEFT JOIN IMPAIRMENT i
						ON i.REQUIRED_COVERAGE_ID = rc.id
					...
					OR rc.SUMMARY_SUB_STATUS_CD NOT IN ('P', 'C'))
					AND rc.STATUS_CD IN ('A', 'M', 'D', 'T')
					AND (@escrowInMatch IS NULL
						 OR REQ_ESCROW_IN = @escrowInMatch) 
					FOR XML PATH ('Cvg'), TYPE)
					AS [Cvgs]
				FROM Collateral COLL
				JOIN property P
					ON COLL.PROPERTY_ID = P.id
				...
				JOIN OWNER_ADDRESS OA
					ON OA.id = O.ADDRESS_ID
				WHERE OLR.PURGE_DT IS NULL
				AND o.PURGE_DT IS NULL
				AND OA.PURGE_DT IS NULL
				AND LN.id = OLR.LOAN_ID
				AND OLR.OWNER_TYPE_CD IN ('B', 'CS', 'DBA')
				FOR XML PATH ('Owner'), TYPE)
				AS [Owners]

			FROM Loan LN
			INNER JOIN @tmpLoans tmp
				ON tmp.loan_id = ln.id
			LEFT JOIN REF_CODE rc
				ON rc.CODE_CD = LN.STATUS_CD
				AND rc.DOMAIN_CD = 'LoanStatus'
			LEFT JOIN RELATED_DATA rd
				ON rd.RELATE_ID = LN.id
				AND rd.DEF_ID = (SELECT
					id
				FROM RELATED_DATA_DEF
				WHERE RELATE_CLASS_NM = 'Loan'
				AND NAME_TX = 'Misc1'
				AND ACTIVE_IN = 'Y')
			LEFT JOIN RELATED_DATA rd2
				ON rd2.RELATE_ID = LN.id
				AND rd2.DEF_ID = (SELECT
					id
				FROM RELATED_DATA_DEF
				WHERE RELATE_CLASS_NM = 'Loan'
				AND NAME_TX = 'Misc2'
	...
	FROM LENDER l
	LEFT OUTER JOIN RELATED_DATA rd
		ON rd.DEF_ID = @isDirectPayDefId
