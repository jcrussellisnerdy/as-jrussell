...
/****** Object:  StoredProcedure [dbo].[RetrieveMonthlyBillingPeriods]    Script Date: 8/25/2017 1:21:00 PM ******/
SET ANSI_NULLS ON
GO
   ...
   BEGIN
      ...
      (
         ...
         SELECT   
            ...
            START_DTF,
            CASE 
               WHEN CPICANCELS.CPI_CANCEL_DT >= START_DTF AND CPICANCELS.CPI_CANCEL_DT < END_DTF THEN CPICANCELS.CPI_CANCEL_DT
               ELSE END_DTF
            END AS END_DTF,
            START_DTM,
            CASE 
               WHEN CPICANCELS.CPI_CANCEL_DT >= START_DTM AND CPICANCELS.CPI_CANCEL_DT < END_DTM THEN CPICANCELS.CPI_CANCEL_DT
               ELSE END_DTM
            END AS END_DTM
         FROM (
            SELECT   
            ...
            WHERE    
               FTX.TXN_TYPE_CD = 'R'
            GROUP BY 
               FTX.FPC_ID, FTX.TERM_NO, FPC.CPI_QUOTE_ID
         ) DATE_TEMP
         OUTER APPLY (
            SELECT   MAX(DATEADD(DAY,DATEDIFF(DAY,0,START_DT), 0)) AS CPI_CANCEL_DT
            FROM     CPI_ACTIVITY
            WHERE    CPI_ACTIVITY.TYPE_CD = 'C'
                     AND CPI_ACTIVITY.CPI_QUOTE_ID = DATE_TEMP.CPI_QUOTE_ID
                     AND CPI_ACTIVITY.PURGE_DT IS NULL
         ) AS CPICANCELS
      ) DATE_TERM_DATA
