...
----REPLACE 2301 WITH THE THE Lender ID
----REPLACE INC0254991 WITH THE TICKET ID
---Checking UTLs WIs
USE UniTrac
...
--INTO UniTracHDStorage..INC0254991
FROM    WORK_ITEM
WHERE   RELATE_TYPE_CD = 'Allied.UniTrac.UTLMatchResult'
        ...
        WHERE   LENDER.CODE_TX = '2301' AND WORK_ITEM.STATUS_CD NOT IN ('Complete', 'Withdrawn')
			   )
...
WHERE WORK_ITEM.STATUS_CD NOT IN ('Complete', 'Withdrawn')) AND WORK_ITEM.LENDER_ID = '833'
...
UPDATE WI
SET WI.STATUS_CD = W.STATUS_CD
--SELECT *
FROM dbo.WORK_ITEM WI
JOIN UniTracHDStorage..INC0254991 W ON Wi.ID = W.ID
WHERE [Collateral] <> 'RE'
...
--SELECT *
FROM dbo.LOAN L
JOIN UniTracHDStorage..LOAN_INC0254991 LL ON L.ID = LL.ID
JOIN UniTracHDStorage..INC0254991 W ON L.ID = W.RELATE_ID --AND W.WORKFLOW_DEFINITION = '8'
WHERE W.[Collateral] <> 'RE'


SELECT * FROM UniTracHDStorage..LOAN_INC0254991

-------- Clear UTLs (- rows)
UPDATE  UniTrac..WORK_ITEM
SET     STATUS_CD = 'Complete' ,
        UPDATE_USER_TX = 'INC0254991',
		 LOCK_ID = CASE WHEN LOCK_ID >= 255 THEN 1
                  ELSE LOCK_ID + 1 END
WHERE   ID IN  (  SELECT ID FROM UniTracHDStorage..INC0254991)
...
SELECT * INTO UniTracHDStorage..LOAN_INC0254991
