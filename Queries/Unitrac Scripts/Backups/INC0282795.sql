...
SELECT  *
FROM    UniTrac..WORK_ITEM
WHERE   ID IN (SELECT ID FROM UniTracHDStorage..INC0282795 )


--Create a temp table for notices and certs 
select RELATE_ID INTO #tmpN from process_log_item PLI
where PLI.process_log_ID iN (41673919) and relate_type_cd IN ('Allied.UniTrac.Notice')

select RELATE_ID INTO #tmpFPC from process_log_item PLI
where PLI.process_log_ID iN (41673919) and relate_type_cd IN ('Allied.UniTrac.ForcePlacedCertificate')
