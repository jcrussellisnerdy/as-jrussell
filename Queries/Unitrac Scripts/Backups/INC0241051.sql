...
WHERE   WORKFLOW_DEFINITION_ID = 11
        ...
        AND ID IN ( 32199427, 32447157, 32465396, 32470941, 32474691, 32491461, 32502488, 32506353, 32513999, 32556751, 32570141, 32570442, 32576596, 32586611,  32606137  )
...
UPDATE PLI
SET PLI.PURGE_DT = T.[PLI.PURGE_DT]
--SELECT T.*
FROM dbo.PROCESS_LOG_ITEM PLI
JOIN #tmp T ON PLI.ID = T.PLI_ID
...
SELECT  WI_ID ,
        ...
        [PLI.PURGE_DT] ,
        [WIA ID] ,
        [WIA.PURGE_DT] FROM #TMP

UPDATE WIP
SET WIP.PURGE_DT = T.[WIPLI.PURGE_DT]
--SELECT T.*
FROM dbo.WORK_ITEM_PROCESS_LOG_ITEM_RELATE WIP
JOIN #tmp T ON WIP.ID = T.WIPLI_ID
--SELECT * FROM dbo.WORK_ITEM_PROCESS_LOG_ITEM_RELATE
WHERE ID IN (SELECT WIPLI_ID FROM #TMP)
...
SELECT esc.id, ESC.TYPE_CD, ESC.SUB_TYPE_CD, ESC.EXCESS_IN, *
FROM WORK_ITEM_PROCESS_LOG_ITEM_RELATE WIPLIR 			 
  JOIN #TMPWI ON #TMPWI.id = WIPLIR.WORK_ITEM_ID
 JOIN PROCESS_LOG_ITEM PLI ON PLI.ID = WIPLIR.PROCESS_LOG_ITEM_ID	
  AND PLI.PURGE_DT IS NULL AND WIPLIR.PURGE_DT IS NULL
 JOIN ESCROW ESC  ON ESC.ID = PLI.RELATE_ID AND PLI.RELATE_TYPE_CD = 'Allied.UniTrac.Escrow'	
 CROSS APPLY (
     SELECT TOP 1  RC1.TYPE_CD , RC1.ID AS RC_ID ,RC1.PROPERTY_ID ,RE1.STATUS_CD , RE1.SUB_STATUS_CD 
...
--758752
  ...
  SELECT   RC1.TYPE_CD [RC_TYPE_CD] , RC1.ID AS RC_ID ,RC1.PROPERTY_ID ,RE1.STATUS_CD , RE1.SUB_STATUS_CD ,
  RE1.TYPE_CD,RE1.SUB_TYPE_CD,ER1.ESCROW_ID, RE1.ACTIVE_IN, RE1.EXCESS_IN
 -- INTO #tmp
	 FROM REQUIRED_COVERAGE RC1 
     JOIN ESCROW_REQUIRED_COVERAGE_RELATE ER1 ON ER1.REQUIRED_COVERAGE_ID = RC1.ID
     JOIN REQUIRED_ESCROW RE1 ON RE1.REQUIRED_COVERAGE_ID = RC1.ID
    -- AND RE1.ACTIVE_IN = 'Y' 
     WHERE --RE1.TYPE_CD = 'INS' AND RE1.SUB_TYPE_CD = 'PB' AND 
      -- RE1.EXCESS_IN = 'N' AND
       ER1.ESCROW_ID = '758752'
	   --ESC.ID AND (RC1.TYPE_CD = 'HAZARD' OR RC1.TYPE_CD = RC1.TYPE_CD)
     ORDER BY 
     Case when RC1.TYPE_CD = 'HAZARD' Then 0 else 1 end 
     asc
	 ...
	 SELECT *
	 --into #tmp2
	  FROM REQUIRED_ESCROW RE1
	 where REQUIRED_COVERAGE_ID = '107970949'
