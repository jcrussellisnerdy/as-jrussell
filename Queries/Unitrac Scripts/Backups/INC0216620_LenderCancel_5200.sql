...
----REPLACE INC0216620 WITH THE TICKET ID
---Checking UTLs WIs
...
SELECT * --INTO UniTracHDStorage..INC0216620
FROM UniTrac..WORK_ITEM
WHERE CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') = '5200'
AND WORKFLOW_DEFINITION_ID IN (2,3,6,8)
--AND STATUS_CD = 'Initial'
AND WORK_ITEM.STATUS_CD NOT LIKE 'Complete' AND WORK_ITEM.STATUS_CD NOT LIKE 'Withdrawn'
ORDER BY WORKFLOW_DEFINITION_ID ASC

SELECT ID FROM UniTracHDStorage..INC0216620

-------- Clear UTLs (- rows)
UPDATE  UniTrac..WORK_ITEM
SET     STATUS_CD = 'Complete' ,
        UPDATE_USER_TX = 'INC0216620',
		 LOCK_ID = CASE WHEN LOCK_ID >= 255 THEN 1
                  ELSE LOCK_ID + 1 END
WHERE   ID IN  ( SELECT ID FROM UniTracHDStorage..INC0216620)
...
SELECT * INTO UniTracHDStorage..LOAN_INC0216620
...
--                WHERE   UPDATE_USER_TX = 'INC0216620' AND CODE_TX = '####')



--SELECT CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)'),  * FROM dbo.WORK_ITEM
--		WHERE WORKFLOW_DEFINITION_ID = '8' AND STATUS_CD = 'WITHDRAWN' AND  UPDATE_USER_TX = 'INC0216620'
--		ORDER BY RELATE_ID ASC

--		SELECT * FROM dbo.LOAN
--		WHERE UPDATE_USER_TX = 'INC0216620' 
--		ORDER BY ID ASC
