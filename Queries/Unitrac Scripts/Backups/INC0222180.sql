...
WHERE   ID IN ( SELECT ID FROM UniTracHDStorage..INC0222180   ) AND WORK_ITEM.STATUS_CD <> 'Withdrawn'
...
WHERE   LENDER.CODE_TX = '1854'
		...
		AND WI.ID IN (SELECT ID FROM UniTracHDStorage..INC0222180 )
		
SELECT * FROM #TMPWI

SELECT * INTO UniTracHDStorage..LOAN_INC0222180
