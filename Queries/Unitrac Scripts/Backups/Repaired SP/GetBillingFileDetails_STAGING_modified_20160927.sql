USE [UniTrac]
GO
/****** Object:  StoredProcedure [dbo].[GetBillingFileDetails]    Script Date: 9/26/2016 9:03:19 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [dbo].[GetBillingFileDetails]
(
...
BEGIN
   ...
   BEGIN
      ...
      -- Elimiminate those rows who are not owed a charge or refund
	  SELECT  t1.* 
      INTO #t6
	  from #t1 t1
	  inner join @monthlyPeriods mp on mp.FPC_ID = t1.FPC_ID
		  and ((mp.MTHN_R_TOT_PRM - mp.MTHN_C_TOT_PRM) - MTHN_P_TOT_PRM <> 0)
      ...
      FROM     #t6 t6
               inner join FINANCIAL_TXN FTX ON t6.FT_ID = FTX.ID AND FTX.PURGE_DT IS NULL
               inner join @monthlyPeriods mp ON t6.FPC_ID = mp.FPC_ID AND ftx.TERM_NO = mp.TERM_NO			
      WHERE    t6.TXN_TYPE_CD in ('R','C')
