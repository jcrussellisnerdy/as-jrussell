...
----REPLACE 0264 WITH THE THE Lender ID
----REPLACE INC0245109 WITH THE TICKET ID
---Checking UTLs WIs
USE UniTrac
...
--INTO UniTracHDStorage..INC0245109
FROM    WORK_ITEM
WHERE   RELATE_TYPE_CD = 'Allied.UniTrac.UTLMatchResult'
        ...
        WHERE   LENDER.CODE_TX = '0264' AND WORK_ITEM.STATUS_CD NOT IN ('Complete', 'Withdrawn')
			   )
...
WHERE CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') = '0264'
...
SET     STATUS_CD = 'Complete' ,
        UPDATE_USER_TX = 'INC0245109',
		 LOCK_ID = CASE WHEN LOCK_ID >= 255 THEN 1
                  ELSE LOCK_ID + 1 END
WHERE   ID IN  (  SELECT ID FROM UniTracHDStorage..INC0245109)
...
SELECT * INTO UniTracHDStorage..LOAN_INC0245109
