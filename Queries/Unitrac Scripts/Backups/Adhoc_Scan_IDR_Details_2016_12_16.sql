...
CREATE    procedure [dbo].[Adhoc_Scan_IDR_Details] 
(
...
from ScanBatch
left Join lkuScanQueueTypes on lkuScanQueueTypes.QueueType = ScanBatch.QueueType
Left Join tblScanOptions on intkey = intftpkey
...
Select LenderKey,sum(ScannedDocumentCount) as SubTotal into #tmpScan from ScanBatch
where ScanBatch.BatchType = 3 and 
(@md = 'Day' and datediff(d,batchdate,getdate()) = @Increment)			--0
or
(@md = 'Week' and datediff(wk,batchdate,getdate()) = @Increment)			--1
...
FAST_FORWARD 
FOR
Select distinct REPLACE(REPLACE(isnull(FTPDescription,isnull(Description,'FaxQueue')),' ','_'),'-','_') as FTPDescription from ScanBatch 
left Join tblScanOptions on intftpkey=intkey
left Join lkuScanQueueTypes on lkuScanQueueTypes.QueueType = ScanBatch.QueueType
where Scanbatch.BatchType = 3 and
...
from #tmpScan
Join tblLender on tblLender.LenderKey = #tmpScan.LenderKey
