SELECT  L.NUMBER_TX, RC.* --INTO UniTracHDStorage..INC0275546_RC
FROM    dbo.LOAN L
...
WHERE RC.ID IN(975516) AND 

 L.ID IN (SELECT ID FROM UniTracHDStorage..INC0275546)
...
WHERE
	...
	AND rc.STATUS_CD='A' 
	AND RC.ID IN (SELECT ID FROM UniTracHDStorage..INC0275546_RC)
	AND rc.TYPE_CD = 'PHYS-DAMAGE'
	AND l.LENDER_ID = (SELECT TOP 1 ID FROM LENDER WHERE CODE_TX = '2221')
...
IF EXISTS (SELECT * 
                 ...
                 AND  TABLE_NAME = 'INC0275546_RC_UPDATE_DETAILS')
BEGIN
	DROP TABLE UniTracHDStorage.dbo.INC0275546_RC_UPDATE_DETAILS
END

SELECT
	*
INTO UniTracHDStorage.dbo.INC0275546_RC_UPDATE_DETAILS
FROM
	#TMP_NEW tn
...
SELECT GOOD_THRU_DT, RC.UPDATE_USER_TX, RC.* 
--SELECT rc.ID INTO #tmp
FROM    dbo.LOAN L
        JOIN dbo.COLLATERAL C ON C.LOAN_ID = L.ID
        JOIN dbo.PROPERTY P ON P.ID = C.PROPERTY_ID
        JOIN dbo.REQUIRED_COVERAGE RC ON RC.PROPERTY_ID = P.ID
WHERE  RC.ID IN (SELECT ID FROM UniTracHDStorage..INC0275546_RC)
 AND INSURANCE_STATUS_CD ='A'

--294'


SELECT * FROM dbo.PROCESS_DEFINITION
WHERE UPDATE_USER_TX IN ('UBSGoodThruWINT', 'UBSGoodThru3')



SELECT * FROM dbo.PROPERTY_CHANGE PC
LEFT JOIN dbo.PROPERTY_CHANGE_UPDATE PCU ON PC.ID = PCU.CHANGE_ID 
WHERE PC.ENTITY_ID IN (SELECT * FROM #tmp) AND PC.ENTITY_NAME_TX = 'Allied.UniTrac.RequiredCoverage'
AND PCU.COLUMN_NM = 'INSURANCE_STATUS_CD' AND PCU.FROM_VALUE_TX = 'A' 