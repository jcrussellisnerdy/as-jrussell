...
----Backup WI
SELECT  *
INTO UniTracHDStorage..INC0282150
FROM    UniTrac..WORK_ITEM
WHERE   ID IN (37069231  )
...
where PLI.process_log_ID iN (41588137) and relate_type_cd IN ('Allied.UniTrac.Notice')

select RELATE_ID INTO #tmpFPC from process_log_item PLI
where PLI.process_log_ID iN (41588137) and relate_type_cd IN ('Allied.UniTrac.ForcePlacedCertificate')
...
/*
select *
INTO UniTracHDStorage..INC0282150_Notice
 from notice
WHERE ID IN (SELECT * FROM #tmpN) AND PDF_GENERATE_CD = 'ERR'
...
/*
update N
set purge_dt= getdate(), lock_id = lock_id+1, update_dt = getdate(), update_user_tx = 'INC0282150'
--select *
 from notice N
WHERE ID IN (SELECT * FROM #tmpN) AND PDF_GENERATE_CD = 'ERR'
...
SET     STATUS_CD = 'Withdrawn' ,
        PURGE_DT = GETDATE() ,
        UPDATE_DT = GETDATE() ,
        UPDATE_USER_TX = 'INC0282150'
--SELECT *
FROM Work_item WI
WHERE   ID IN ( SELECT ID FROM UniTracHDStorage..INC0282150 )
