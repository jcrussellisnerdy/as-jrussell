...
/****** Object:  StoredProcedure [dbo].[Report_LenderCarrier]    Script Date: 7/20/2016 11:05:09 AM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
 /*
 --debug:
 declare @debug bigint=6681
 select
  @LenderCode=cast(@debug as nvarchar(10))
...
[Master_Assignment_ID] bigint, PE_IDs nvarchar(max), CC_CODE_TX nvarchar(30), 
...
,START_DT DateTime2
,END_DT DateTime2
,PRIMARY_CLASS_MEAN NVarChar(1000)
,PRIMARY_CLASS_DESC NVarChar(1000)
)
...
[MASTER_POLICY_NO],
[MASTER_POLICY_ID],
[Master_Assignment_ID], PE_IDs, CC_CODE_TX, 
[COVERAGE_TYPE_CD],
[ISSUE_PROC_CD],
[ISSUE_PROC_TX],
[CANCEL_PROC_CD],
...
Select distinct  
 ...
 Master_Assignment_ID = MPA.ID, PE_IDs, CC.CODE_TX, 
...
,START_DT = MPA.START_DT
,END_DT = Case When MPA.END_DT = '12/31/9999' Then Null Else MPA.END_DT End
,PRIMARY_CLASS_MEAN = RC_PrimClass.MEANING_TX
,PRIMARY_CLASS_DESC = RC_PrimClass.DESCRIPTION_TX
...
LEFT    Join LENDER_ORGANIZATION LO on LO.ID = LCGCT.DIVISION_LENDER_ORG_ID AND LO.PURGE_DT IS NULL
 ...
 --Join MASTER_POLICY_ASSIGNMENT MPA0 on 
 -- MPA0.MASTER_POLICY_ID = MP.ID and 
 -- ((MPA0.BRCH_LENDER_ORG_ID is null) or (MPA0.BRCH_LENDER_ORG_ID = LCGCT.BRANCH_LENDER_ORG_ID)) and 
 -- (LCGCT.DIVISION_LENDER_ORG_ID is null or MPA0.DIV_LENDER_ORG_ID = LCGCT.DIVISION_LENDER_ORG_ID) and 
 -- MPA0.PURGE_DT IS NULL
 --CROSS Apply(Select PE_IDs = Null) As PE
CROSS Apply(
 Select ID=Max(MPA0.ID), PE_IDs=PE.IDs, CC_CODE=CC.CODE_TX From MASTER_POLICY_ASSIGNMENT MPA0
 CROSS Apply(Select IDs = (Select ';' + Cast(PE.ID As VarChar(10))
             From (
     Select Distinct PE.ID
     From MASTER_POLICY_ASSIGNMENT MPA
     Join MASTER_POLICY_ENDORSEMENT MPE On MPA.ID = MPE.MASTER_POLICY_ASSIGNMENT_ID AND MPA.PURGE_DT Is Null
     Join POLICY_ENDORSEMENT PE On MPE.POLICY_ENDORSEMENT_ID = PE.ID AND MPE.PURGE_DT Is Null
     Where PE.PURGE_DT Is Null
       AND MPA.MASTER_POLICY_ID = MP.ID) As PE
    Order By PE.ID
    For XML Path('')) ) As PE
 LEFT Join COLLATERAL_CODE CC On CC.ID = MPA0.COLLATERAL_CODE_ID AND CC.PURGE_DT Is Null
 Where MPA0.MASTER_POLICY_ID = MP.ID and 
  ((MPA0.BRCH_LENDER_ORG_ID is null) or (MPA0.BRCH_LENDER_ORG_ID = LCGCT.BRANCH_LENDER_ORG_ID)) and 
  (LCGCT.DIVISION_LENDER_ORG_ID is null or MPA0.DIV_LENDER_ORG_ID = LCGCT.DIVISION_LENDER_ORG_ID) and 
  MPA0.PURGE_DT IS NULL
 Group By PE.IDs, CC.CODE_TX
     ) As MPA0
Join MASTER_POLICY_ASSIGNMENT MPA On MPA.ID = MPA0.ID
LEFT Join REF_CODE RC_PrimClass On RC_PrimClass.CODE_CD = MPA.PRIMARY_CLASS_CD AND RC_PrimClass.DOMAIN_CD = 'PrimaryClassification'
 left Join LENDER_ORGANIZATION ROB on ROB.ID = LCGCT.BRANCH_LENDER_ORG_ID AND ROB.PURGE_DT IS NULL 
LEFT Join LENDER_ORGANIZATION ROD on ROD.ID = LCGCT.DIVISION_LENDER_ORG_ID AND ROD.PURGE_DT IS NULL  
...
Order By Convert(NVarChar(10), CARRIER_EFFECTIVE_DT, 111) Desc, Convert(NVarChar(10), CARRIER_EXPIRATION_DT, 111) Desc, ReportGroupName
END
