...
SELECT  *
--INTO UniTracHDStorage..INC0321419
FROM    UniTrac..WORK_ITEM
WHERE   ID IN (SELECT ID FROM dbo.WORK_ITEM
WHERE WORKFLOW_DEFINITION_ID = 10
AND LENDER_ID = 2267 AND CURRENT_QUEUE_ID = 156
AND STATUS_CD = 'Initial' AND PURGE_DT IS NULL 
AND CREATE_DT <= '2017-10-01' )
...
SET     STATUS_CD = 'Complete' ,
        LOCK_ID = LOCK_ID + 1 ,
        UPDATE_DT = GETDATE() ,
        UPDATE_USER_TX = 'INC0321419'
WHERE   ID IN ( SELECT ID FROM UniTracHDStorage..INC0321419 )
        AND WORKFLOW_DEFINITION_ID = 10
        AND ACTIVE_IN = 'Y'
