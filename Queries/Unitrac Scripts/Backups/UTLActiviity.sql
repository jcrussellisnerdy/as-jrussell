USE [UniTrac]
GO
/****** Object:  StoredProcedure [dbo].[Report_UTLActivity]    Script Date: 10/2/2015 1:25:01 PM ******/
SET ANSI_NULLS ON
GO
...
[MortgageAddressLine1],[MortgageAddressLine2],[MortgageAddressCity], [MortgageAddressStateProvince],[MortgageAddressPostalCode],[PROPERTY_TYPE_CD],[CALL_ATTEMPTS] )
--UTL Match
select 
...
WIA.UPDATE_USER_TX, U.FAMILY_NAME_TX, U.GIVEN_NAME_TX, WIA.UPDATE_DT, WIA.ACTION_CD, 
IH.NOTE_TX + 
CASE WHEN ISNULL(IH.SPECIAL_HANDLING_XML.value(''(/SH/Resolution/text())[1]'', ''NVARCHAR(50)''), '''') = '''' OR
          ISNULL(IH.NOTE_TX, '''') = ''''
	THEN ''''
	ELSE '' / '' 
END +
ISNULL(IH.SPECIAL_HANDLING_XML.value(''(/SH/Resolution/text())[1]'', ''NVARCHAR(50)''), '''') AS ACTION_NOTE_TX, 
WI.ID as WorkItemID, 
CASE 
...
+ case when @Division <> '1' and @Division <> '' and @Division is NOT NULL then ' AND
(L.DIVISION_CODE_TX = @Division OR (@Division in (''3'',''8'') and L.DIVISION_CODE_TX not in (''3'',''8'') and RCA_PROP.VALUE_TX in (''VEH'',''BOAT''))
OR (@Division in (''7'',''9'') and L.DIVISION_CODE_TX not in (''7'',''9'') and RCA_PROP.VALUE_TX in (''EQ''))
OR (@Division in (''4'',''10'') and L.DIVISION_CODE_TX not in (''4'',''10'') and RCA_PROP.VALUE_TX not in (''VEH'',''BOAT'',''EQ'')))'
else '' end
...
+ case when @Branch <> '1' and @Branch <> '' and @Branch is NOT NULL then char(10) + ' AND (L.BRANCH_CODE_TX IN (SELECT STRVALUE FROM #BranchTable)) ' else '' end
+ case when @Division <> '1' and @Division <> '' and @Division is NOT NULL then ' AND
((@Division in (''3'',''8'') and L.DIVISION_CODE_TX not in (''3'',''8'') and RCA_PROP.VALUE_TX in (''VEH'',''BOAT''))
OR (@Division in (''7'',''9'') and L.DIVISION_CODE_TX not in (''7'',''9'') and RCA_PROP.VALUE_TX in (''EQ''))
OR (@Division in (''4'',''10'') and L.DIVISION_CODE_TX not in (''4'',''10'') and RCA_PROP.VALUE_TX not in (''VEH'',''BOAT'',''EQ'')))'
else '' end
...
BEGIN
	...
	*/
	...
	,ACTION_NOTE_TX = SortGroup.StatusDate + ' ' + IH_SH.AgentName + ' (' + CASE WHEN IH_SH.AgentLetterOptionValue = 'Fax' THEN SortGroup.AgentFax WHEN IH_SH.AgentLetterOptionValue = 'Email' THEN SortGroup.AgentEmail ELSE IH_SH.AgentEmail END + ')' + ' ' + IsNull(IH.NOTE_TX, '')
	...
	AND
	(L.DIVISION_CODE_TX = @Division OR IsFiltered.Division = 0 -- @Division = '1' OR @Division  = '' OR @Division IS NULL 
	OR ((@Division IN ('3','8') AND L.DIVISION_CODE_TX NOT IN ('3','8') AND RCA_PROP.VALUE_TX IN ('VEH','BOAT'))
	OR (@Division IN ('7','9') AND L.DIVISION_CODE_TX NOT IN ('7','9') AND RCA_PROP.VALUE_TX IN ('EQ'))
	OR (@Division IN ('4','10') AND L.DIVISION_CODE_TX NOT IN ('4','10') AND RCA_PROP.VALUE_TX NOT IN ('VEH','BOAT','EQ'))))
