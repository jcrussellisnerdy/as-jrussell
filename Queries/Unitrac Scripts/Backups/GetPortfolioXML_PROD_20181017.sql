...
ALTER PROCEDURE [dbo].[GetPortfolioXML] (
	@lenderId BIGINT
...
BEGIN
	...
	SELECT DISTINCT ln.id
		...
		,rc.id
		,CASE 
			WHEN re.ID IS NULL or (c.PRIMARY_LOAN_IN = 'N' and ln.ESCROW_IN = 'N')
				THEN 'N'
			ELSE 'Y'
			END
	FROM LOAN ln
				...
				(
					SELECT
						...
						(
										...
										,(
											...
											WHERE 
												...
												AND CASE WHEN
													...
													ELSE 'PB'
													END = EP.SUB_TYPE_CD
											ORDER BY EP.END_DT DESC, EP.STATUS_CD DESC, EP.ID DESC
							...
							INNER JOIN (SELECT DISTINCT REQUIRED_COVERAGE_ID, COLLATERAL_ID,REQ_ESCROW_IN FROM #tmpLoans) tmpRC ON tmpRC.REQUIRED_COVERAGE_ID = RC.ID and tmpRC.COLLATERAL_ID = COLL.ID						
