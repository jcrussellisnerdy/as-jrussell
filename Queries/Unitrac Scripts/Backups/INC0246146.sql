...
SELECT DISTINCT 
		...
		pcu.COLUMN_NM [Reason]
FROM    WORK_ITEM AS wi
		LEFT OUTER JOIN dbo.LOAN L ON WI.RELATE_ID = L.ID 
		...
		LEFT OUTER JOIN dbo.PROPERTY_CHANGE_UPDATE PCU ON PCU.TABLE_ID = CONTENT_XML.value('(/Content/VerifyData/Properties/Property/@Id)[1]', 'varchar (50)') AND TABLE_NAME_TX = 'PROPERTY'
