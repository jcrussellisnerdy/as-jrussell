...
/****** Object:  StoredProcedure [dbo].[SearchFullTextProperty]    Script Date: 9/25/2017 11:54:38 AM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
   ...
   IF @userDealer = ''			
	...
	DECLARE @LenderIdCount BIGINT = 0
	IF (@useLenderGrouping = 'Y' and isnull(@lenderid, 0) <> 0)
	BEGIN
     CREATE TABLE #tmpLenderId(LENDER_ID BIGINT NOT NULL)
	  INSERT INTO #tmpLenderId
	  SELECT * 
	  FROM fnGetLenderGroupLenderIdsForLender( @lenderId, @useLenderGrouping )
	  SET	@LenderIdCount = @@rowcount
	END
   ...
   select @where = '
			...
			' + case when @useLenderGrouping = 'Y' and isnull(@lenderid, 0) <> 0 AND @LenderIdCount <> 1 then '' 
   ...
   BEGIN
      ...
      begin
        select @joins = @joins + case WHEN @useLenderGrouping = 'Y' AND @LenderIdCount > 1 then '
			join #tmpLenderId tli on tli.LENDER_ID = ft.LENDER_ID
			'
			else '' end
      end
      ...
      BEGIN
         ...
         select @joins2 = '
            ...
            INNER JOIN dbo.LOAN L1  ON C.LOAN_ID = L1.ID AND L1.PURGE_DT IS NULL AND L1.RECORD_TYPE_CD <> ''D''
				join SEARCH_FULLTEXT ft on ft.PROPERTY_ID = p.id 
		    ' + case WHEN @useLenderGrouping = 'Y' AND @LenderIdCount > 1 then '
				join #tmpLenderId tli on tli.LENDER_ID = ft.LENDER_ID
			'
			else '' end + '
            where CONTAINS ( ft.KEY_WORD, @searchText )
