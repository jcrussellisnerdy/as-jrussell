IF EXISTS (select * from sys.objects where object_id = OBJECT_ID(N'[dbo].[GetForcePlacedCertificates]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[GetForcePlacedCertificates]
GO
     ...
     IF (@billingGroupId is not null) BEGIN
       ...
       where F.PURGE_DT is null
                ...
                (
                    (FT.FT_AMOUNT_NO <> 0)
		            OR 
		            ((dbo.GetFPCOutstandingAmount(ofu.FPC_ID) = 0) AND
		            (isnull(dbo.GetFPCTotalFundAmount(ofu.FPC_ID),0) = isnull(dbo.GetFPCTotalRefundAmount(ofu.FPC_ID),0)))
                )
