...
/****** Object:  UserDefinedFunction [dbo].[fn_GetOutstandingFinancialTxns]    Script Date: 11/6/2017 3:37:45 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN
		...
		Where fpc.ID = @FPC_ID and FTA.BILLING_GROUP_ID = @BILLING_GROUP_ID 
         AND FTA.HOLD_IN = 
            CASE WHEN @IncludeHolds = 'Y' 
               THEN FTA.HOLD_IN 
            ELSE 
               CASE WHEN FPC.MONTHLY_BILLING_IN = 'Y' 
                  THEN 'N' 
                  ELSE FTA.HOLD_IN 
               END 
            END
      ...
      -- Create a Transaction Temp Table to hold splits when necessary
      DECLARE @TransactionsTemp TABLE(ftx_id bigint, ftpt_id bigint, fpc_id bigint, ftx_type_cd nvarchar(10), new_txn_in char(1), 
         amount_no decimal(18,2), running_total decimal(18,2), term_no int, transaction_dt datetime2(7), create_dt datetime2(7), 
         rownumber int, trans_term_no int, term_amount_no decimal(18,2), term_hold_in varchar(1)) 

      -- Build a Transaction Table with Split Payments If there are Any
      declare @hasSplitPayments int = 0
      insert into @TransactionsTemp
      select   T.ftx_id, T.ftpt_id, T.fpc_id, T.ftx_type_cd, T.new_txn_in, T.amount_no, T.running_total, 
               T.term_no, T.transaction_dt, T.create_dt, T.rownumber, D.TERM_NO, D.AMOUNT_NO, D.HOLD_IN
      from     @Transactions T
               INNER JOIN dbo.[fn_GetDistributedOldMonthlyPayments]
                  (@FPC_ID, @BILLING_GROUP_ID, @IncludeHolds) D ON T.ftx_id = D.FTX_ID

      -- Get a Split Payment Count
      set @hasSplitPayments = @@ROWCOUNT
      
      -- If there are split payments, do the splits and readd them (re-number the row number too)
      -- NOTE: Honor holds when doing this
      if @hasSplitPayments > 0
      BEGIN

         -- Remove Split Payments from the Main Transaction Table                        
         DELETE   t
         FROM     @Transactions t
                  inner join @TransactionsTemp t2 on t.ftx_id = t2.ftx_id    
         
         -- Readd Split Payments from the Main Transaction Table
         INSERT INTO @Transactions 
         SELECT   T.ftx_id, T.ftpt_id, T.fpc_id, T.ftx_type_cd, T.new_txn_in, T.term_amount_no, 
                  T.running_total, T.trans_term_no, T.transaction_dt, T.create_dt, (ROW_NUMBER () Over(order by transaction_dt asc, term_no asc))
         FROM     @TransactionsTemp T
         WHERE    T.term_hold_in =  
                     CASE WHEN @IncludeHolds = 'N' THEN 'N' 
                     ELSE T.term_hold_in END
      END

      -- If payments are not splitting out, these it is possible that months were split out in the 
      -- payment split system, but one or month of the months were put on hold, be sure that if any
      -- months that were put on hold that their payments are also removed
      ELSE
      BEGIN
         IF @IncludeHolds = 'N'

            -- Insert into the @Transactions table payments that are already split or didn't need to be split
            insert into @Transactions (ftx_id, ftpt_id,fpc_id, ftx_type_cd, new_txn_in, amount_no, term_no, transaction_dt, create_dt, rowNumber)
            SELECT   ftx.ID, ftptd.ID, fpc.ID, ftx.TXN_TYPE_CD, fta.NEW_TXN_IN, 
			            ISNULL(ftptd.AMOUNT_NO, ftx.AMOUNT_NO), 
			            ISNULL(ftptd.term_no, ftx.term_no), 
			            ftx.TXN_DT, ftx.CREATE_DT, 1
		      From     FORCE_PLACED_CERTIFICATE FPC 
					      LEFT JOIN FINANCIAL_TXN ftx on FPC.ID = ftx.FPC_ID and ftx.PURGE_DT is null
					      LEFT JOIN FINANCIAL_TXN_PAYMENT_TERM_DISTRIBUTION ftptd on ftx.ID = ftptd.FTX_ID and ftptd.PURGE_DT is null
					      LEFT JOIN FINANCIAL_TXN_APPLY FTA ON FTX.ID = FTA.FINANCIAL_TXN_ID AND FTA.PURGE_DT IS NULL
            ...
            DELETE FROM @Transactions WHERE term_no IN
            (
               SELECT   FTX.TERM_NO
               FROM     FINANCIAL_TXN FTX
                        INNER JOIN FINANCIAL_TXN_APPLY FTA ON FTA.FINANCIAL_TXN_ID = FTX.ID AND FTX.PURGE_DT IS NULL
               WHERE    FTX.FPC_ID = @FPC_ID
            ...
            -- Renumber the Row Number for the Transaction Table
            DECLARE @Renumber TABLE(ftx_id bigint, ftpt_id bigint, fpc_id bigint, ftx_type_cd nvarchar(10), new_txn_in char(1), amount_no decimal(18,2), running_total decimal(18,2), term_no int, transaction_dt datetime2(7), create_dt datetime2(7), rownumber int) 
            insert into @Renumber
            select ftx_id, ftpt_id, fpc_id, ftx_type_cd, new_txn_in, amount_no, running_total, term_no, transaction_dt, create_dt, (ROW_NUMBER () Over(order by fpc_id, transaction_dt, term_no desc)) 
            from @Transactions
            delete from @Transactions
            insert into @Transactions select * from @Renumber
      END
   ...
   END
	...
	Set @rownumber = (Select Top 1 Case When rownumber <> 0 Then rownumber Else null End From @Output order by rownumber desc)
