...
----REPLACE 7506 WITH THE THE Lender ID
----REPLACE INC0251009 WITH THE TICKET ID
---Checking UTLs WIs
USE UniTrac
...
--INTO UniTracHDStorage..INC0251009
FROM    WORK_ITEM
WHERE   RELATE_TYPE_CD = 'Allied.UniTrac.UTLMatchResult'
        ...
        WHERE   LENDER.CODE_TX = '7506' AND WORK_ITEM.STATUS_CD NOT IN ('Complete', 'Withdrawn')
			   )
...
WHERE CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') IN ('7506', '2904', '1868', '2760', '1405')
AND WORKFLOW_DEFINITION_ID IN (2,3,6,8)
AND WORK_ITEM.STATUS_CD NOT IN ('Complete', 'Withdrawn')
ORDER BY CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)'), WORKFLOW_DEFINITION_ID ASC


-------- Clear UTLs (- rows)
UPDATE  UniTrac..WORK_ITEM
SET     STATUS_CD = 'Complete' ,
        UPDATE_USER_TX = 'INC0251009',
		 LOCK_ID = CASE WHEN LOCK_ID >= 255 THEN 1
                  ELSE LOCK_ID + 1 END
WHERE   ID IN  (  SELECT ID FROM UniTracHDStorage..INC0251009)
...
WHERE   --LENDER.CODE_TX = '7506' AND 
         WI.WORKFLOW_DEFINITION_ID = 8
        AND WI.STATUS_CD NOT IN ( 'COMPLETE', 'Withdrawn' )
        AND WI.PURGE_DT IS NULL AND WI.ID IN ( SELECT ID FROM UniTracHDStorage..INC0251009)
        AND LOAN.PURGE_DT IS NULL
        AND LOAN.RECORD_TYPE_CD IN ('G','A')

SELECT * FROM #TMPWI

SELECT * INTO UniTracHDStorage..LOAN_INC0251009
...
ORDER BY [ServiceName]
	...
	WHERE r.CODE_TX IN ('7506', '2904', '1868', '2760', '1405')
	...
	FROM #process P
		LEFT JOIN dbo.PROCESS_DEFINITION PD ON P.ProcessDefID = PD.ID
			WHERE P.CODE_TX IN  ('7506', '2904', '1868', '2760', '1405')
