...
SELECT * 
--INTO #TMPTASK36923
FROM UTSTAGE01.UNITRAC.DBO.TMPTASK36923
---- 193
...
SELECT * 
INTO UnitracHDStorage.dbo.INC0236195_ORIG
from #TMPTASK36923
---- 193

SELECT * 
INTO UnitracHDStorage.dbo.INC0236195_LN
from #TMPLOAN
---- 43123


SELECT * 
INTO UnitracHDStorage.dbo.INC0236195_LN_01
from #TMPLOAN_01
---- 230

SELECT * 
INTO UnitracHDStorage.dbo.INC0236195_RC
FROM #TMPRC
---- 459


SELECT * 
INTO UnitracHDStorage.dbo.INC0236195_RC_01
FROM #TMPRC_01
---- 435


SELECT * 
INTO UnitracHDStorage.dbo.INC0236195_RC_02
FROM #TMPRC_02
---- 24

SELECT * 
INTO UnitracHDStorage.dbo.INC0236195_WI
FROM #TMPWI
---- 2


UPDATE LN SET BRANCH_CODE_TX = '5230' , 
UPDATE_DT = GETDATE() , UPDATE_USER_TX = 'INC0236195', 
...
UPDATE LN SET DIVISION_CODE_TX = '99' , 
UPDATE_DT = GETDATE() , UPDATE_USER_TX = 'INC0236195', 
LOCK_ID = LN.LOCK_ID % 255 + 1
---- SELECT *
FROM LOAN LN JOIN #TMPRC_01 T1 ON T1.LOAN_ID = LN.ID
...
---- SELECT DISTINCT COLL.ID , COLL.COLLATERAL_CODE_ID
FROM COLLATERAL COLL JOIN #TMPRC_01 T1 ON T1.COLL_ID = COLL.ID
--AND COLL.PURGE_DT IS NULL

 AND coll.COLLATERAL_CODE_ID = '2'
...
SELECT DISTINCT
	...
	GETDATE(),
	1,
	'Moved Loan from Lender 1941 to 1945',
	'N',
	'Y',
	1,
	'Allied.UniTrac.Loan',
...
update WI
set CONTENT_XML.modify('replace value of (/Content/Lender/Code/text())[1] with ("7581")'),
	UPDATE_DT = GETDATE(),
	UPDATE_USER_TX = 'INC0236195',
...
update WI
set CONTENT_XML.modify('replace value of (/Content/Lender/Id/text())[1] with ("1945")'),
	UPDATE_DT = GETDATE(),
	UPDATE_USER_TX = 'INC0236195',
	LOCK_ID = (WI.LOCK_ID + 1) % 256
	----- SELECT *
...
update WI
set CONTENT_XML.modify('replace value of (/Content/Lender/Name/text())[1] with ("Oak Valley Community Bank")'),
	UPDATE_DT = GETDATE(),
	UPDATE_USER_TX = 'INC0236195',
	LOCK_ID = (WI.LOCK_ID + 1) % 256
	----- SELECT *
