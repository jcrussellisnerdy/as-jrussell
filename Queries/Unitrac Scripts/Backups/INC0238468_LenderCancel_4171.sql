...
----REPLACE INC0238468 WITH THE TICKET ID
---Checking UTLs WIs
USE UniTrac

SELECT  * 
--INTO UniTracHDStorage..INC0238468
FROM    WORK_ITEM
...
WHERE CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') = '4171'
...
SET     STATUS_CD = 'Complete' ,
        UPDATE_USER_TX = 'INC0238468',
		 LOCK_ID = CASE WHEN LOCK_ID >= 255 THEN 1
                  ELSE LOCK_ID + 1 END
WHERE   ID IN  ( SELECT ID FROM UniTracHDStorage..INC0238468)
...
SELECT * INTO UniTracHDStorage..LOAN_INC0238468
...
INTO    #TMPWII
FROM    UniTrac..WORK_ITEM
WHERE   ID IN ( 31133860  )



UPDATE WI SET STATUS_CD = 'Withdrawn' , 
UPDATE_DT = GETDATE() , UPDATE_USER_TX = 'INC0238468' , 
...
CURRENT_OWNER_ID , 'INC0238468' , 'Y' , GETDATE() , GETDATE() , 'INC0238468' , 1 , 1
FROM #TMPWII
ORDER BY STATUS_CD
---- 
