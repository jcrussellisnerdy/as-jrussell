...
SELECT CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') Lender, * INTO UniTracHDStorage..INC0207227
FROM    UniTrac..WORK_ITEM
WHERE   ID IN ( 26743184,
26775547,
26775782,
26882101,
26911201,
...
JOIN ESCROW ESC  ON ESC.ID = PLI.RELATE_ID
 ...
 CROSS APPLY (
     ...
     ORDER BY 
     CASE when RC1.TYPE_CD = 'HAZARD' Then 0 else 1 end      asc    ) 
	 RQCOV  
  ...
  WHERE WIPLIR.WORK_ITEM_ID ='27006413'
...
---- SELECT COUNT(*) 
--SELECT *
FROM dbo.WORK_ITEM WI JOIN #TMP_ESCROW tmp2 ON tmp2.WORK_ITEM_ID = WI.ID WHERE USER_ACTION = ''

SELECT *
FROM WORK_ITEM WI JOIN #TMP_ESCROW_04 TMP ON TMP.WORK_ITEM_ID = WI.ID WHERE USER_ACTION = ''
---- 

-------- If there are User Actions Remaining, can find Loan Number using below
SELECT * FROM dbo.WORK_ITEM WI JOIN #TMP_ESCROW tmp2 ON tmp2.WORK_ITEM_ID = WI.ID WHERE USER_ACTION = ''


SELECT PLI.RELATE_TYPE_CD,  * FROM WORK_ITEM_PROCESS_LOG_ITEM_RELATE WIPLIR 			 
 JOIN PROCESS_LOG_ITEM PLI ON PLI.ID = WIPLIR.PROCESS_LOG_ITEM_ID	
 AND PLI.PURGE_DT IS NULL AND WIPLIR.PURGE_DT IS NULL
 ...
 CROSS APPLY (
  ...
  JOIN LOAN ON LOAN.ID = COLL.LOAN_ID 	
  JOIN #TMPWI ON #TMPWI.id = WIPLIR.WORK_ITEM_ID 
  WHERE WORK_ITEM_ID IN (SELECT * FROM #TMP_ESCROW WHERE USER_ACTION = '')


  SELECT * FROM dbo.REQUIRED_COVERAGE
  WHERE PROPERTY_ID IN (SELECT PROPERTY_ID FROM #TMP_ESCROW WHERE USER_ACTION = '')