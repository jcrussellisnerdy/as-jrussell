...
WHERE   WORKFLOW_DEFINITION_ID = 11
        ...
        AND ID IN ( 32701798, 32995453, 33026870, 33069359, 33092989, 33093036, 33093207, 33094645, 33100569, 33126812, 33130488, 33133075, 33133106, 33137980, 33165115, 33165324, 33167953, 33196997, 33198819, 33224495, 33246255, 33246330  )
...
SELECT *
FROM  UniTracHDStorage..zzzzzzzzzz 


UPDATE W
SET W.PURGE_DT = Z.[WIPLI.PURGE_DT]
--SELECT *
FROM dbo.WORK_ITEM_PROCESS_LOG_ITEM_RELATE W
JOIN UniTracHDStorage..zzzzzzzzzz Z ON W.ID = Z.WIPLI_ID
WHERE Z.[WIPLI.PURGE_DT] IS NOT NULL 
...
UPDATE W
SET W.PURGE_DT = Z.[PLI.PURGE_DT]
--SELECT *
FROM dbo.PROCESS_LOG_ITEM W
JOIN UniTracHDStorage..zzzzzzzzzz Z ON W.ID = Z.PLI_ID
WHERE Z.[PLI.PURGE_DT] IS NOT NULL 
