...
----REPLACE 6157 WITH THE THE Lender ID
----REPLACE INC0283294 WITH THE TICKET ID
---Checking UTLs WIs
USE UniTrac

--SELECT * FROM dbo.LENDER WHERE CODE_TX IN ('6157')
...
INTO UniTracHDStorage..INC0283294
FROM UniTrac..WORK_ITEM WI
JOIN dbo.LENDER L ON L.ID = WI.LENDER_ID
WHERE L.CODE_TX IN ('6157')
AND WI.WORKFLOW_DEFINITION_ID IN (2,3,6,8) AND WI.ID NOT IN (36963056)
...
SET     STATUS_CD = 'Complete' ,
        UPDATE_USER_TX = 'INC0283294',
		 LOCK_ID = CASE WHEN LOCK_ID >= 255 THEN 1
                  ELSE LOCK_ID + 1 END
WHERE   ID IN  (  SELECT ID FROM UniTracHDStorage..INC0283294)
...
SELECT * INTO UniTracHDStorage..LOAN_INC0283294
