...
WHERE   WORKFLOW_DEFINITION_ID = 11
        ...
        AND ID IN  ( 30752618, 31185781, 31185880, 31196807, 31199418, 31199751, 3125941, 31221889, 31227533, 31239965, 31261389, 31253416, 31270579, 31284091, 31319567, 31330067, 31348862, 31365935, 31410835 , 31425599  )
...
WHERE WORK_ITEM_ID IN (SELECT ID FROM #TMPWI)
...
SELECT DISTINCT WI_ID FROM #TMP
WHERE [WIPLI.PURGE_DT] IS NOT NULL

SELECT DISTINCT WI_ID FROM #TMP
WHERE [WIA.PURGE_DT] IS NOT NULL

SELECT DISTINCT WI_ID FROM #TMP
WHERE [PLI.PURGE_DT] IS NOT NULL
...
INNER JOIN #tmp T ON T.[WIPLI_ID] = WIP.ID
...
INNER JOIN #tmp T ON T.[WIPLI_ID] = PLI.ID */