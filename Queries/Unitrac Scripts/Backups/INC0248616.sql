---- DROP TABLE #TMPTASK36923
SELECT * 
INTO UniTracHDStorage..INC0248616_Mortgage
FROM dbo.LOAN
WHERE NUMBER_TX IN (SELECT * FROM UniTracHDStorage..['2241Mortgage_Branch'])
AND lender_id = '901'
---- 193




SELECT *
INTO #TMPTASK36923
 FROM dbo.LOAN
WHERE ID IN (SELECT ID FROM UniTracHDStorage..INC0248616_Vehicle) OR ID IN (SELECT ID FROM UniTracHDStorage..INC0248616_Mortgage)

SELECT DISTINCT DIVISION_CODE_TX, BRANCH_CODE_TX FROM #TMPTASK36923

---- DROP TABLE #TMPLOAN
SELECT * 
INTO #TMPLOAN
FROM LOAN 
WHERE LOAN.LENDER_ID = 901
AND RECORD_TYPE_CD IN ('G' , 'A' , 'D' )
AND ID IN (SELECT ID FROM UniTracHDStorage..INC0248616_Vehicle) OR ID IN (SELECT ID FROM UniTracHDStorage..INC0248616_Mortgage)

----- 43123

---- DROP TABLE #TMPLOAN_01
SELECT * 
INTO #TMPLOAN_01
FROM #TMPLOAN
WHERE NUMBER_TX IN 
(
SELECT NEWNUMBER FROM #TMPTASK36923
)
---- 230


----- DROP TABLE #TMPRC
SELECT T1.ID AS LOAN_ID ,  T1.NUMBER_TX , T1.BRANCH_CODE_TX , T1.DIVISION_CODE_TX , T1.RECORD_TYPE_CD , 
T1.STATUS_CD AS LN_STATUS_CD , T1.EXTRACT_UNMATCH_COUNT_NO AS LN_EXTRACT_UNMATCH_COUNT_NO , 
COLL.ID AS COLL_ID , COLL.COLLATERAL_CODE_ID , COLL.PROPERTY_ID , COLL.PRIMARY_LOAN_IN , 
COLL.STATUS_CD AS COLL_STATUS_CD , COLL.EXTRACT_UNMATCH_COUNT_NO AS COLL_EXTRACT_UNMATCH_COUNT_NO ,
PR.RECORD_TYPE_CD AS PR_RECORD_TYPE_CD , PR.ADDRESS_ID , 
OA.LINE_1_TX , OA.CITY_TX , OA.STATE_PROV_TX , OA.POSTAL_CODE_TX , 
RC.ID AS RC_ID , RC.TYPE_CD AS RC_TYPE_CD , RC.NOTICE_DT , RC.NOTICE_SEQ_NO , 
RC.NOTICE_TYPE_CD , RC.CPI_QUOTE_ID , RC.LAST_EVENT_DT , 
RC.LAST_EVENT_SEQ_ID , RC.LAST_SEQ_CONTAINER_ID , RC.RECORD_TYPE_CD AS RC_RECORD_TYPE_CD , 
CC.CODE_TX AS CC_CODE_TX , CC.DESCRIPTION_TX AS CC_DESCRIPTION_TX ,
0 AS EXCLUDE
INTO #TMPRC
FROM #TMPLOAN_01 t1
JOIN COLLATERAL COLL ON COLL.LOAN_ID = T1.ID
AND COLL.PURGE_DT IS NULL AND T1.PURGE_DT IS NULL
JOIN PROPERTY PR ON PR.ID = COLL.PROPERTY_ID
AND PR.PURGE_DT IS NULL
LEFT JOIN OWNER_ADDRESS OA ON OA.ID = PR.ADDRESS_ID
JOIN REQUIRED_COVERAGE RC ON RC.PROPERTY_ID = PR.ID
AND RC.PURGE_DT IS NULL
JOIN COLLATERAL_CODE CC ON CC.ID = COLL.COLLATERAL_CODE_ID
ORDER BY T1.NUMBER_TX
----459



UPDATE #TMPRC SET EXCLUDE = 1
WHERE LOAN_ID IN 
(
SELECT T1.LOAN_ID
FROM #TMPRC T1
JOIN COLLATERAL COLL ON COLL.PROPERTY_ID = T1.PROPERTY_ID
AND COLL.ID <> T1.COLL_ID
AND COLL.LOAN_ID <> T1.LOAN_ID
AND  COLL.PURGE_DT IS NULL
JOIN LOAN ON LOAN.ID = COLL.LOAN_ID
AND LOAN.PURGE_DT IS NULL
AND LOAN.NUMBER_TX NOT IN 
(
SELECT NEWNUMBER FROM #TMPTASK36923
)
)
----24

---- DROP TABLE #TMPRC_01
SELECT * 
INTO #TMPRC_01
FROM #TMPRC
WHERE EXCLUDE = 0
ORDER BY NUMBER_TX
----- 435

--- DROP TABLE #TMPRC_02
SELECT * 
INTO #TMPRC_02
FROM #TMPRC
WHERE EXCLUDE = 1
ORDER BY NUMBER_TX
----- 24

--SELECT DISTINCT NUMBER_TX , BRANCH_CODE_TX , CC_CODE_TX  , RECORD_TYPE_CD FROM #TMPRC_02
--SELECT * FROM LENDER WHERE CODE_TX = '1729' AND PURGE_DT IS NULL
---- 306

--SELECT * FROM #TMPRC_01

---- 'Type' - for those that say HELOC - map to the HOM EQUITY under 1729, those that say 1st Mortgage - map to Mortgage/<Default>.
/*
SELECT * FROM LENDER_COLLATERAL_CODE_GROUP WHERE LENDER_ID = 306
SELECT CC.ID , CC.CODE_TX FROM LCCG_COLLATERAL_CODE_RELATE REL
JOIN COLLATERAL_CODE CC ON CC.ID = REL.COLLATERAL_CODE_ID
WHERE REL.PURGE_DT IS NULL
AND CC.PURGE_DT IS NULL
AND REL.LCCG_ID = 1462
/*
ID	CODE_TX
2	<DEFAULT>
41	HOM EQUITY
*/
*/


SELECT NUMBER_TX , LOAN_ID , WI.*
INTO #TMPWI
	FROM WORK_ITEM WI JOIN #TMPRC_01 T1 ON T1.LOAN_ID = WI.RELATE_ID
where WI.status_cd not in ('Complete', 'Error', 'Withdrawn')
and WI.purge_dt is null
and WI.relate_type_cd = 'Allied.UniTrac.Loan'
----- 2


SELECT * 
INTO UnitracHDStorage.dbo.tmpTask36923_ORIG
from #TMPTASK36923
---- 193

SELECT * 
INTO UnitracHDStorage.dbo.tmpTask36923_LN
from #TMPLOAN
---- 43123


SELECT * 
INTO UnitracHDStorage.dbo.tmpTask36923_LN_01
from #TMPLOAN_01
---- 230

SELECT * 
INTO UnitracHDStorage.dbo.tmpTask36923_RC
FROM #TMPRC
---- 459


SELECT * 
INTO UnitracHDStorage.dbo.tmpTask36923_RC_01
FROM #TMPRC_01
---- 435


SELECT * 
INTO UnitracHDStorage.dbo.tmpTask36923_RC_02
FROM #TMPRC_02
---- 24

SELECT * 
INTO UnitracHDStorage.dbo.tmpTask36923_WI
FROM #TMPWI
---- 2


UPDATE LN SET BRANCH_CODE_TX = '1732' , 
UPDATE_DT = GETDATE() , UPDATE_USER_TX = 'TASK36923', 
LOCK_ID = LN.LOCK_ID % 255 + 1
---- SELECT *
FROM LOAN LN JOIN #TMPRC_02 T1 ON T1.LOAN_ID = LN.ID
---- 24


UPDATE LN SET BRANCH_CODE_TX = '2241' , 
UPDATE_DT = GETDATE() , UPDATE_USER_TX = 'INC0248616', 
LOCK_ID = LN.LOCK_ID % 255 + 1
---- SELECT *
FROM LOAN LN JOIN #TMPRC_01 T1 ON T1.LOAN_ID = LN.ID
WHERE LN.BRANCH_CODE_TX = '1732'
---- 6


UPDATE LN SET DIVISION_CODE_TX = '4' , 
UPDATE_DT = GETDATE() , UPDATE_USER_TX = 'INC0248616', 
LOCK_ID = LN.LOCK_ID % 255 + 1
---- SELECT *
FROM LOAN LN JOIN #TMPRC_01 T1 ON T1.LOAN_ID = LN.ID
WHERE LN.DIVISION_CODE_TX = '10'
---- 6

SELECT * FROM dbo.LENDER
WHERE CODE_TX = '2241'
UPDATE LN SET LENDER_ID = 660 , 
RECORD_TYPE_CD = CASE WHEN LN.RECORD_TYPE_CD IN ( 'D' , 'A') THEN 'G' ELSE LN.RECORD_TYPE_CD END ,
STATUS_CD = 'A',
EXTRACT_UNMATCH_COUNT_NO = 0  , 
UPDATE_DT = GETDATE() , UPDATE_USER_TX = 'INC0248616', 
LOCK_ID = LN.LOCK_ID % 255 + 1
---- SELECT DISTINCT LN.ID , LN.LENDER_ID , LN.STATUS_CD , LN.EXTRACT_UNMATCH_COUNT_NO , LN.DIVISION_CODE_TX
FROM LOAN LN JOIN #TMPRC_01 T1 ON T1.LOAN_ID = LN.ID
---- 218


UPDATE COLL SET COLLATERAL_CODE_ID = 1 ,
STATUS_CD = 'A',
EXTRACT_UNMATCH_COUNT_NO = 0  , 
UPDATE_DT = GETDATE() , UPDATE_USER_TX = 'INC0248616' , 
LOCK_ID = COLL.LOCK_ID % 255 + 1
---- SELECT DISTINCT COLL.ID , T2.*
FROM COLLATERAL COLL JOIN #TMPRC_01 T1 ON T1.COLL_ID = COLL.ID
AND COLL.PURGE_DT IS NULL
JOIN #TMPTASK36923 T2 ON T2.NEWNUMBER = T1.NUMBER_TX
AND T2.[LOANTYPE] = 'HELOC'
---- 79


UPDATE COLL SET COLLATERAL_CODE_ID = 2 ,
STATUS_CD = 'A',
EXTRACT_UNMATCH_COUNT_NO = 0  , 
UPDATE_DT = GETDATE() , UPDATE_USER_TX = 'TASK36923' , 
LOCK_ID = COLL.LOCK_ID % 255 + 1
---- SELECT DISTINCT COLL.ID , T2.*
FROM COLLATERAL COLL JOIN #TMPRC_01 T1 ON T1.COLL_ID = COLL.ID
AND COLL.PURGE_DT IS NULL
JOIN #TMPTASK36923 T2 ON T2.NEWNUMBER = T1.NUMBER_TX
AND T2.[LOANTYPE] = '1st Mortgage'
---- 139




UPDATE PR SET LENDER_ID = 660 , 
RECORD_TYPE_CD = CASE WHEN PR.RECORD_TYPE_CD IN ( 'D' , 'A') THEN 'G' ELSE PR.RECORD_TYPE_CD END ,
UPDATE_DT = GETDATE() , UPDATE_USER_TX = 'TASK36923', 
LOCK_ID = PR.LOCK_ID % 255 + 1
---- SELECT DISTINCT PR.ID , PR.RECORD_TYPE_CD
FROM PROPERTY PR JOIN #TMPRC_01 T1 ON T1.PROPERTY_ID = PR.ID
WHERE PR.LENDER_ID = 331
----- 182


INSERT INTO
	PROPERTY_CHANGE (
		ENTITY_NAME_TX,
		ENTITY_ID,
		USER_TX,
		ATTACHMENT_IN,
		CREATE_DT,
		AGENCY_ID,
		DESCRIPTION_TX,
		DETAILS_IN,
		FORMATTED_IN,
		LOCK_ID,
		PARENT_NAME_TX,
		PARENT_ID,
		TRANS_STATUS_CD,
		UTL_IN)
SELECT DISTINCT
	'Allied.UniTrac.Loan',
	LOAN_ID,
	'TASK36923',
	'N',
	GETDATE(),
	1,
	'Moved Loan from Lender 4266 to 2241',
	'N',
	'Y',
	1,
	'Allied.UniTrac.Loan',
	LOAN_ID,
	'PEND',
	'N'
FROM #TMPRC_01
---- 218


update SEARCH_FULLTEXT
	set LENDER_ID = 660,
		UPDATE_DT = GETDATE()
		----- SELECT DISTINCT SF.PROPERTY_ID
	FROM #TMPRC_01 t
	JOIN SEARCH_FULLTEXT sf ON sf.PROPERTY_ID = t.PROPERTY_ID
	----- 182
	
update WI
set CONTENT_XML.modify('replace value of (/Content/Lender/Code/text())[1] with ("1729")'),
	UPDATE_DT = GETDATE(),
	UPDATE_USER_TX = 'TASK36923',
	LOCK_ID = (WI.LOCK_ID + 1) % 256
	----- SELECT *
	FROM WORK_ITEM WI JOIN #TMPRC_01 T1 ON T1.LOAN_ID = WI.RELATE_ID
where WI.status_cd not in ('Complete', 'Error', 'Withdrawn')
and WI.purge_dt is null
and WI.relate_type_cd = 'Allied.UniTrac.Loan'
----- 2

update WI
set CONTENT_XML.modify('replace value of (/Content/Lender/Id/text())[1] with ("306")'),
	UPDATE_DT = GETDATE(),
	UPDATE_USER_TX = 'TASK36923',
	LOCK_ID = (WI.LOCK_ID + 1) % 256
	----- SELECT *
	FROM WORK_ITEM WI JOIN #TMPRC_01 T1 ON T1.LOAN_ID = WI.RELATE_ID
where WI.status_cd not in ('Complete', 'Error', 'Withdrawn')
and WI.purge_dt is null
and WI.relate_type_cd = 'Allied.UniTrac.Loan'
----- 2

update WI
set CONTENT_XML.modify('replace value of (/Content/Lender/Name/text())[1] with ("Superior Federal Credit Union")'),
	UPDATE_DT = GETDATE(),
	UPDATE_USER_TX = 'TASK36923',
	LOCK_ID = (WI.LOCK_ID + 1) % 256
	----- SELECT *
	FROM WORK_ITEM WI JOIN #TMPRC_01 T1 ON T1.LOAN_ID = WI.RELATE_ID
where WI.status_cd not in ('Complete', 'Error', 'Withdrawn')
and WI.purge_dt is null
and WI.relate_type_cd = 'Allied.UniTrac.Loan'
----- 2
	
update EQ
	set PURGE_DT = GETDATE(),
		UPDATE_DT = GETDATE(),
		UPDATE_USER_TX = 'TASK36923',
		LOCK_ID = (eq.LOCK_ID + 1) % 256
		---- SELECT EQ.*
	from #TMPRC_01 t    
    inner join EVALUATION_QUEUE eq on T.RC_ID = eq.REQUIRED_COVERAGE_ID and eq.PURGE_DT is null
	----- 0
	
update ee
	set STATUS_CD = 'CLR',
		PURGE_DT = GETDATE() ,
		UPDATE_DT = GETDATE() ,
		UPDATE_USER_TX = 'INC0248616',
		LOCK_ID = (ee.LOCK_ID + 1) % 256
		---- SELECT ee.status_cd, *
	from #TMPRC_01 t   
    inner join EVALUATION_EVENT ee on ee.REQUIRED_COVERAGE_ID = T.RC_ID and ee.PURGE_DT is null
	where ee.STATUS_CD = 'PEND'
	----- 0
	
---- NO CPI QUOTE & NO NOTICE CYCLE
update rc
	set UPDATE_DT = GETDATE() ,
		UPDATE_USER_TX = 'INC0248616',
		LOCK_ID = (rc.LOCK_ID + 1) % 256,
		GOOD_THRU_DT = null,
		LENDER_PRODUCT_ID = null,
		LCGCT_ID = null,
		NOTICE_DT = null,
		NOTICE_TYPE_CD = NULL,
		NOTICE_SEQ_NO = null,
		CPI_QUOTE_ID = null,
		LAST_EVENT_SEQ_ID = null,
		LAST_EVENT_DT = null,
		LAST_SEQ_CONTAINER_ID = null ,
		RECORD_TYPE_CD = CASE WHEN RC.RECORD_TYPE_CD IN ( 'D' , 'A') THEN 'G' ELSE RC.RECORD_TYPE_CD END 
		---- SELECT RC.RECORD_TYPE_CD , RC.CPI_QUOTE_ID , RC.NOTICE_SEQ_NO ,  *
	from #TMPRC_01 t    
    inner join REQUIRED_COVERAGE rc on rc.ID = T.RC_ID 
    and rc.PURGE_DT is null 
    ---- 435

UPDATE C
SET C.COLLATERAL_CODE_ID = '2', C.LOCK_ID = C.LOCK_ID+1,
C.UPDATE_DT = GETDATE(), C.UPDATE_USER_TX = 'INC0248616'
--SELECT COLLATERAL_CODE_ID, C.*
FROM dbo.COLLATERAL C 
JOIN dbo.LOAN L ON L.ID = C.LOAN_ID
WHERE L.ID IN (SELECT ID FROM UniTracHDStorage..INC0248616_Mortgage)


	USE [UniTrac]
GO 

--Loan Screen Information - LOAN/COLLATERAL/PROPERTY/OWNER/REQUIRED COVERAGE
SELECT L.NUMBER_TX, L.DIVISION_CODE_TX, C.COLLATERAL_CODE_ID FROM LOAN L
INNER JOIN COLLATERAL C ON L.ID = C.LOAN_ID
INNER JOIN PROPERTY P ON C.PROPERTY_ID = P.ID
INNER JOIN REQUIRED_COVERAGE RC ON P.ID = RC.PROPERTY_ID
INNER JOIN OWNER_LOAN_RELATE OL ON L.ID = OL.LOAN_ID
INNER JOIN OWNER O ON OL.OWNER_ID = O.ID
INNER JOIN OWNER_ADDRESS OA ON O.ADDRESS_ID = OA.ID
INNER JOIN dbo.LENDER LL ON LL.ID = L.LENDER_ID
WHERE --L.ID IN (SELECT ID FROM UniTracHDStorage..INC0248616_Vehicle) OR 
L.ID IN (SELECT ID FROM UniTracHDStorage..INC0248616_Mortgage)
ORDER BY L.NUMBER_TX DESC



---Finding Collateral Code
SELECT  CC.ID ,
        CC.CODE_TX ,
        CC.DESCRIPTION_TX, CC.*
         FROM dbo.COLLATERAL_CODE CC
INNER JOIN dbo.LCCG_COLLATERAL_CODE_RELATE CCR ON CCR.COLLATERAL_CODE_ID = CC.ID
INNER JOIN dbo.LENDER_COLLATERAL_CODE_GROUP LCCG ON LCCG.ID = CCR.LCCG_ID
INNER JOIN dbo.LENDER L ON L.ID = LCCG.LENDER_ID
WHERE L.CODE_TX = '2241'
ORDER BY CC.ID ASC


SELECT * FROM dbo.COLLATERAL_CODE CC
INNER JOIN dbo.LCCG_COLLATERAL_CODE_RELATE CCR ON CCR.COLLATERAL_CODE_ID = CC.ID
INNER JOIN dbo.LENDER_COLLATERAL_CODE_GROUP LCCG ON LCCG.ID = CCR.LCCG_ID

