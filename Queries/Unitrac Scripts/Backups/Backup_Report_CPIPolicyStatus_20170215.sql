...
/****** Object:  StoredProcedure [dbo].[Report_CPIPolicyStatus]    Script Date: 2/15/2017 1:56:34 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[Report_CPIPolicyStatus] 
--declare
...
IF OBJECT_ID(N'tempdb..#REF_CODE_CTE',N'U') IS NOT NULL
  DROP TABLE #REF_CODE_CTE
...
--IDs, STATUS
	...
	[CXL_REASON_TX] [nvarchar](10) NULL,
...
Declare @FilterBySQL_base as varchar(1000)
...
SELECT @FilterBySQL_base=IsNull(FILTER_TX,'') FROM dbo.REPORT_CONFIG RC WITH(NOLOCK) WHERE CODE_TX = @ReportType
--
if @ReportConfig is NULL or @ReportConfig = '' or @ReportConfig = '0000'
  ...
  End  
--
IF @FilterBySQL_base<>'' AND IsNull(@FilterBySQL,'')<>''
BEGIN
	IF lower(@FilterBySQL)<>lower(@FilterBySQL_base) AND REPLACE(REPLACE(lower(@FilterBySQL),'[',''),']','') NOT LIKE '%' + REPLACE(REPLACE(lower(@FilterBySQL_base),'[',''),']','') + '%'
	BEGIN
		Select @FilterBySQL = @FilterBySQL_base + IsNull(' And ' + NullIf(@FilterBySQL,''),'')
	END
END

/*  use variables for MULTICPI_6426
...
'
+ case when  @RealReportConfig Like 'CPICAN%' THEN '' else ' and FPC.ACTIVE_IN = ''Y'' AND @RealReportConfig <> '''' ' end
+ '
OUTER APPLY(
...
where 1=1 
'
+ case when @ReportConfig = 'CPIINFCALL' then '' else ' AND L.RECORD_TYPE_CD = ''G'' and P.RECORD_TYPE_CD = ''G'' and RC.RECORD_TYPE_CD = ''G'' ' end
+ case when @RealReportConfig = 'CPICAN_VSI' then '' else ' AND L.EXTRACT_UNMATCH_COUNT_NO = 0 and C.EXTRACT_UNMATCH_COUNT_NO = 0 AND L.STATUS_CD != ''U'' and C.STATUS_CD != ''U'' ' end
+ '
...
COVERAGE_STATUS_TX,
--CPI
INSCOMPANY_NAME_TX, INSCOMPANY_POLICY_NO, INSCOMPANY_EFF_DT, INSCOMPANY_EFFDTSORT_TX, INSCOMPANY_EXP_DT, INSCOMPANY_CAN_DT, INSCOMPANY_CAN_TRAN_DT, INSCOMPANY_EXPCXL_DT, 
INSCOMPANY_ISSUE_DT, CPI_QUOTE_TERM_NO, CPI_ISSUEDPREMIUM_AMOUNT_NO, CPI_CANCELLEDPREMIUM_AMOUNT_NO, CPI_CANCEL_TYPE, CPI_HOLD_IN,
--IDs, STATUS
...
[PROPERTY_DESCRIPTION], [ISS_REASON_TX], [INS_CXL_DT], [CXL_REASON_TX], 
[I_TOT_PRM], 
[PI_TOT_PRM], 
[C_TOT_PRM], 
[PC_TOT_PRM], 
...
--IDs, STATUS
	   ...
	   PROPERTY_DESCRIPTION = dbo.fn_GetPropertyDescriptionForReports(TT1.COLLATERAL_ID),
	   ISNULL(CPA_I.REASON_CD,'') AS [ISS_REASON_TX],
	   CPA_C.CREATE_DT AS [INS_CXL_DT],
	   CXL_CREASON_TX = IsNull(CPA_C.REASON_CD,''),
...
OUTER APPLY(select top 1 CPA_C.* from dbo.CPI_ACTIVITY CPA_C with (forceseek)
	WHERE CPA_C.CPI_QUOTE_ID = TT1.FPC_CPI_QUOTE_ID
      AND CPA_C.TYPE_CD In ('C','MT')
      AND CPA_C.PURGE_DT IS NULL order by CPA_C.id desc) AS CPA_C
...
BEGIN
	...
	,tt.INSCOMPANY_POLICY_NO
	,Case
		When @ReportConfig LIKE 'MULTICPI%' AND (LATEST_CPI = 1 OR [PRIOR_CPI_CNT_NO] > 0 )
		Then 1
...
--	exec ('SELECT * FROM #tmptable WHERE LATEST_CPI = 1 OR [PRIOR_CPI_CNT_NO] < 1 order by ' + @Sortbysql)
	SELECT * FROM #tmptable WHERE LATEST_CPI = 1 OR [PRIOR_CPI_CNT_NO] > 0 
END
