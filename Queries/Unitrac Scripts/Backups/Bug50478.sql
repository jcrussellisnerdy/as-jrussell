...
WHERE  WI.ID IN (55699373, 54255813,54255811, 53574523, 52252491, 52817114, 52817118) AND WI.STATUS_CD NOT IN ( 'COMPLETE', 'Withdrawn' )
...
SELECT * INTO UniTracHDStorage..LOAN_Bug50478
FROM dbo.LOAN WHERE ID IN (SELECT LOAN_ID FROM #TMPWI)

SELECT * INTO UniTracHDStorage..WI_Bug50478
