...
SELECT * 
INTO UniTracHDStorage..INC0276938
FROM UniTrac..WORK_ITEM
WHERE ID IN (35875054 )
...
VALUES  ( 35875054 , -- WORK_ITEM_ID - bigint
          ...
          N'INC0276938' , -- UPDATE_USER_TX - nvarchar(15)
          1 , -- LOCK_ID - tinyint
          1  -- ACTION_USER_ID - bigint
        )

SELECT * FROM dbo.WORK_ITEM_ACTION
WHERE WORK_ITEM_ID IN (SELECT ID FROM UniTracHDStorage..INC0276938)


----Use Last WIA ID that was created for LAST_WORK_ITEM_ACTION_ID column 
UPDATE  UniTrac..WORK_ITEM
SET     STATUS_CD = 'Complete', 
LAST_WORK_ITEM_ACTION_ID = (SELECT MAX(ID) FROM dbo.WORK_ITEM_ACTION WHERE   WORK_ITEM_ID IN ( 35875054  )),
UPDATE_DT = GETDATE(),LOCK_ID = LOCK_ID+1, UPDATE_USER_TX = 'INC0276938'
WHERE   ID IN (SELECT ID FROM UniTracHDStorage..INC0276938)
