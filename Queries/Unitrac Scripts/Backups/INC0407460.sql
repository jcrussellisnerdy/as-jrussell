...
GO
	...
	declare @ticketnumber nvarchar (255)


set @ticketnumber ='INC0407460'

SELECT  @processlogId = n.id
FROM LOAN L
INNER JOIN COLLATERAL C ON L.ID = C.LOAN_ID
INNER JOIN PROPERTY P ON C.PROPERTY_ID = P.ID
INNER JOIN REQUIRED_COVERAGE RC ON P.ID = RC.PROPERTY_ID
join notice_required_coverage_relate nrcr on rc.id = nrcr.required_coverage_id 
join notice n on n.id = nrcr.notice_id 
INNER JOIN dbo.LENDER LL ON LL.ID = L.LENDER_ID
WHERE LL.CODE_TX IN ('1190') and l.DIVISION_CODE_TX = 10
and rc.type_cd not in ('Flood','EQUIPMENT','REO-Liability') and
rc.id =''
	...
	where n.ID IN (@processlogid)

	    	
	UPDATE INTERACTION_HISTORY SET PURGE_DT = GETDATE(), UPDATE_USER_TX = @ticketnumber ,
	UPDATE_DT = GETDATE() , LOCK_ID = (LOCK_ID % 255) + 1
     where RELATE_ID IN (@processlogid)
 and RELATE_CLASS_TX = 'Allied.UniTrac.Notice' and TYPE_CD = 'NOTICE'
     
    UPDATE NOTICE_REQUIRED_COVERAGE_RELATE SET PURGE_DT = GETDATE(), UPDATE_USER_TX = @ticketnumber ,
    UPDATE_DT = GETDATE() , LOCK_ID = (LOCK_ID % 255) + 1 where NOTICE_ID IN (@processlogid)

     
	UPDATE NOTICE SET PURGE_DT = GETDATE(), UPDATE_USER_TX = @ticketnumber , 
	UPDATE_DT = GETDATE() , LOCK_ID = (LOCK_ID % 255) + 1 where ID IN (@processlogid)

		
	UPDATE EVALUATION_EVENT set STATUS_CD = 'NOTU', PURGE_DT = GETDATE(), UPDATE_USER_TX = @ticketnumber,
	UPDATE_DT = GETDATE() , LOCK_ID = (LOCK_ID % 255) + 1
	where REQUIRED_COVERAGE_ID = @rcId and STATUS_CD = 'PEND'
   AND EVENT_SEQUENCE_ID IS NOT NULL
	
	UPDATE DOCUMENT_CONTAINER SET PURGE_DT = GETDATE() , UPDATE_USER_TX = @ticketnumber,
	UPDATE_DT = GETDATE() , LOCK_ID = (LOCK_ID % 255) + 1
	WHERE RELATE_ID IN (@processlogid)
 AND RELATE_CLASS_NAME_TX = 'ALLIED.UNITRAC.NOTICE'
	...
	IF @NoticeType IN ('CCU','CCF','BI')
		BEGIN
	      UPDATE EVALUATION_EVENT SET STATUS_CD = 'PEND' , UPDATE_USER_TX = @ticketnumber , 
	      UPDATE_DT = GETDATE() , LOCK_ID = (LOCK_ID % 255) + 1	
	      WHERE ID = @eeId
		END	
    ...
    if (@NoticeSequence = 1)
		BEGIN
		  ...
		  GOOD_THRU_DT = NULL , UPDATE_DT = GETDATE() , LOCK_ID = (LOCK_ID % 255) + 1 , UPDATE_USER_TX = @ticketnumber
		  where ID = @rcId and NOTICE_SEQ_NO = @NoticeSequence and NOTICE_TYPE_CD = @NoticeType       
		   ---and  NOTICE_DT = @NoticeDt   
		   
		  UPDATE IH SET PURGE_DT = GETDATE() , UPDATE_USER_TX = @ticketnumber , 
			     ...
			     BEGIN			  
			      ...
			      Begin			        
			         ...
			         UPDATE REQUIRED_COVERAGE set NOTICE_DT = NULL, NOTICE_SEQ_NO = NULL ,
					   ...
					   UPDATE_DT = GETDATE() , LOCK_ID = (LOCK_ID % 255) + 1 , UPDATE_USER_TX = @ticketnumber
					   where ID = @rcId and NOTICE_SEQ_NO = @NoticeSequence and NOTICE_TYPE_CD = @NoticeType 
			      End
