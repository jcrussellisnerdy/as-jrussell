...
WHERE   WI.LENDER_ID IN (1449,1882,2260)
...
SELECT * INTO UniTracHDStorage..LOAN_INC0340805
FROM dbo.LOAN WHERE ID IN (SELECT LOAN_ID FROM #TMPWI)

SELECT * INTO UniTracHDStorage..WI_INC0340805
...
WHERE L.LENDER_ID IN (1449,1882,2260) AND 
 ...
 UPDATE L SET  L.SPECIAL_HANDLING_XML = NULL
 --SELECT * 
 FROM dbo.LOAN L
 WHERE ID IN (SELECT RELATE_ID FROM dbo.WORK_ITEM
 WHERE ID IN (SELECT * FROM #tmpWII))
