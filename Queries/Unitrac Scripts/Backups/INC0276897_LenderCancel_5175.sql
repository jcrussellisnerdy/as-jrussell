...
----REPLACE 5175 WITH THE THE Lender ID
----REPLACE INC0276897 WITH THE TICKET ID
---Checking UTLs WIs
USE UniTrac

--SELECT * FROM dbo.LENDER WHERE CODE_TX IN ('5175')
...
--INTO UniTracHDStorage..INC0276897
FROM    WORK_ITEM
WHERE   RELATE_TYPE_CD = 'Allied.UniTrac.UTLMatchResult'
        ...
        WHERE   LENDER.CODE_TX = '5175' AND WORK_ITEM.STATUS_CD NOT IN ('Complete', 'Withdrawn')
			   )
...
WHERE WORK_ITEM.STATUS_CD NOT IN ('Complete', 'Withdrawn')) AND WORK_ITEM.LENDER_ID = '1872'
...
SET     STATUS_CD = 'Complete' ,
        UPDATE_USER_TX = 'INC0276897',
		 LOCK_ID = CASE WHEN LOCK_ID >= 255 THEN 1
                  ELSE LOCK_ID + 1 END
WHERE   ID IN  (  SELECT ID FROM UniTracHDStorage..INC0276897)
...
SELECT * INTO UniTracHDStorage..LOAN_INC0276897
