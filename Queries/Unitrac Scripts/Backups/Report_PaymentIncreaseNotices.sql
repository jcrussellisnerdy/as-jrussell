...
/****** Object:  StoredProcedure [dbo].[Report_PaymentIncreaseNotices]    Script Date: 8/24/2015 5:15:43 PM ******/
DROP PROCEDURE [dbo].[Report_PaymentIncreaseNotices]
GO
...
SELECT 
	...
	,CASE 
		  WHEN ISNULL(L.DIVISION_CODE_TX,'0') in ('3','8') OR RCA_PROP.VALUE_TX in ('VEH','BOAT') THEN P.YEAR_TX + ' ' + P.MAKE_TX + '/' + P.MODEL_TX
		  WHEN ISNULL(L.DIVISION_CODE_TX,'0') in ('7','9') OR RCA_PROP.VALUE_TX in ('EQ') THEN P.DESCRIPTION_TX
		  WHEN ISNULL(L.DIVISION_CODE_TX,'0') in ('4','10') OR ISNULL(RCA_PROP.VALUE_TX,'') not in ('','VEH','BOAT','EQ')
			THEN AP.LINE_1_TX + CASE WHEN ISNULL(AP.LINE_2_TX,'') = '' THEN ' ' ELSE ', ' +  AP.LINE_2_TX END + AP.CITY_TX + ', ' + AP.STATE_PROV_TX + ' ' + AP.POSTAL_CODE_TX
		  ELSE ''
