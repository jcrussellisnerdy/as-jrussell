...
SELECT * 
INTO UniTracHDStorage..INC0286311
FROM UniTrac..WORK_ITEM
WHERE ID IN (37787782 )
...
VALUES  ( 37787782  , -- WORK_ITEM_ID - bigint
          ...
          N'INC0286311' , -- UPDATE_USER_TX - nvarchar(15)
          1 , -- LOCK_ID - tinyint
          1  -- ACTION_USER_ID - bigint
        )

SELECT * FROM dbo.WORK_ITEM_ACTION
WHERE WORK_ITEM_ID IN (SELECT ID FROM UniTracHDStorage..INC0286311)


----Use Last WIA ID that was created for LAST_WORK_ITEM_ACTION_ID column 
UPDATE  UniTrac..WORK_ITEM
SET     STATUS_CD = 'Complete', 
LAST_WORK_ITEM_ACTION_ID = (SELECT MAX(ID) FROM dbo.WORK_ITEM_ACTION WHERE   WORK_ITEM_ID IN ( 37787782  )),
UPDATE_DT = GETDATE(),LOCK_ID = LOCK_ID+1, UPDATE_USER_TX = 'INC0286311'
WHERE   ID IN (SELECT ID FROM UniTracHDStorage..INC0286311)
