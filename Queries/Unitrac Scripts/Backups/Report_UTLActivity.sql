if not exists (select 1 from sys.procedures where name = 'Report_UTLActivity')
	exec ('create proc Report_UTLActivity as return')
GO

/****** Object:  StoredProcedure [dbo].[Report_UTLActivity]    Script Date: 05/02/2012 11:10:11 ******/
SET ANSI_NULLS ON
GO
...
--debug/test:
  ...
  select @lendercode='2552' --SpaceCoast
...
'
		...
		OUTER apply(select　
			...
			,IsLapse = IH.SPECIAL_HANDLING_XML.value(''(/SH[1]/IsLapse)[1]'', ''nvarchar(5)'')　
		...
		OUTER Apply(select
			 CpiMemo = 
			''Lapse:                '' + @tab + IsNull(IsLapse,''false'') + @pipe + 
			''Susp of Comp/Clsn:    '' + @tab + IsNull(SuspCompClsn,''false'') + @newline + 
			''Deductible:           '' + @tab + IsNull(Deductible,''???'') + @pipe + 
			''Lienholder:           '' + @tab + case when Lower(IsLienholder)=''yes'' then ''Yes'' else ''No'' end + @newline + 
			...
			,CpiMemo1 = 
			''Lapse:                '' + @tab + case when Lower(IsLapse)=''true'' then ''Yes'' else ''No'' end + @newline + 
...
/*TFS#45633:
exec Report_UTLActivity 'usdtest1',null,null,null,'5/1/2018','5/31/2018',null,default,6,@debug=1
go
*/
