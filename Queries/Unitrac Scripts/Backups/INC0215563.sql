...
SELECT CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') Lender, * --INTO UniTracHDStorage..WI_INC0215563
FROM    UniTrac..WORK_ITEM
WHERE   ID IN (  26847393  )
...
WHERE   LENDER.CODE_TX = '6096'
		...
		AND WI.ID IN (26847393)
		
--SELECT * FROM #TMPWI

SELECT * INTO UniTracHDStorage..LOAN_INC0215563
...
SELECT * FROM UniTracHDStorage..LOAN_INC0215563

SELECT * FROM UniTracHDStorage..WI_INC0215563



SELECT * FROM loan 
WHERE ID IN (SELECT ID FROM UniTracHDStorage..LOAN_INC0215563)

SELECT * FROM dbo.WORK_ITEM
WHERE ID IN (SELECT ID FROM UniTracHDStorage..WI_INC0215563)