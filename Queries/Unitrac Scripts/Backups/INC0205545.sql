...
SELECT CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') Lender,CONTENT_XML.value('(/Content/Loan/Number)[1]', 'varchar (50)') [Loan Number], *
FROM    UniTrac..WORK_ITEM
WHERE   ID IN (  25921248 , 23278695)


SELECT CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') Lender, * INTO UniTracHDStorage..INC0205545
FROM    UniTrac..WORK_ITEM
WHERE   ID IN (  25921248 , 23278695)

SELECT * FROM UniTracHDStorage..INC0205545
...
WHERE   LENDER.CODE_TX = '2992'
        ...
        AND LOAN.RECORD_TYPE_CD IN ('G','A', 'D')
		AND WI.ID IN (  25921248 , 23278695)
...
WHERE   LENDER.CODE_TX = '2992'
		...
		WHERE NUMBER_TX IN ('86470-10', 
'1601653-3') AND LENDER_ID = '1837'
