...
WHERE   WORKFLOW_DEFINITION_ID = 11
       -- AND STATUS_CD = 'INITIAL'
        AND ID IN ( SELECT ID FROM UniTracHDStorage..INC0214500 )
--AND CONTENT_XML.value('(/Content/Lender/Code)[1]' , 'varchar(10)') = '2771'
-----
DROP TABLE #TMPWI

--SELECT DISTINCT ID FROM #TMPWI
---- 
...
WHERE WORK_ITEM_ID = (SELECT ID FROM #TMPWI)
...
'Y' , GETDATE() , GETDATE()  , 'UNITRAC' , 1 , 20
...
--UPDATE PLII SET PURGE_DT = NULL 
----SELECT * 
--FROM dbo.WORK_ITEM_PROCESS_LOG_ITEM_RELATE PLII
--WHERE PLII.WORK_ITEM_ID IN (SELECT ID FROM  UniTracHDStorage..INC0214500)
--AND PLI.RELATE_TYPE_CD = 'Allied.UniTrac.Escrow'
...
SELECT ID, UPDATE_DT  --INTO #tmpE
FROM dbo.PROCESS_LOG_ITEM WHERE PROCESS_LOG_ID IN (SELECT RELATE_ID FROM  UniTracHDStorage..INC0214500)
AND PROCESS_LOG_ITEM.RELATE_TYPE_CD = 'Allied.UniTrac.Escrow'


SELECT ID, UPDATE_DT  INTO #tmpEF
FROM dbo.WORK_ITEM_PROCESS_LOG_ITEM_RELATE WHERE WORK_ITEM_ID IN (SELECT ID FROM  UniTracHDStorage..INC0214500)
...
--UPDATE PLII SET PURGE_DT = EF.UPDATE_DT
--SELECT * 
--FROM dbo.WORK_ITEM_PROCESS_LOG_ITEM_RELATE PLII
--INNER JOIN #tmpEF E ON E.ID = PLII.ID
--WHERE PLII.WORK_ITEM_ID IN (SELECT ID FROM  UniTracHDStorage..INC0214500)
