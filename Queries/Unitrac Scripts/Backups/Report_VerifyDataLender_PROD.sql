...
/****** Object:  StoredProcedure [dbo].[Report_VerifyDataLender]    Script Date: 12/17/2015 4:51:27 PM ******/
...
FROM WORK_ITEM WI 
		...
		LEFT JOIN PROPERTY P ON P.ID = COALESCE(P_DETAIL.ID, C_DETAIL.PROPERTY_ID, RC.PROPERTY_ID) AND P.PURGE_DT IS NULL
		LEFT JOIN COLLATERAL C ON (C.ID = C_DETAIL.ID OR C.PROPERTY_ID = P_DETAIL.ID OR C.PROPERTY_ID = RC.PROPERTY_ID) AND C.PURGE_DT IS NULL
