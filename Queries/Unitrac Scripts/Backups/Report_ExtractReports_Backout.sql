...
/****** Object:  StoredProcedure [dbo].[Report_ExtractReports]    Script Date: 06/16/2015 16:32:37 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Report_ExtractReports]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Report_ExtractReports]
GO
...
FROM [TRANSACTION] WITH(NOLOCK) CROSS APPLY DATA.nodes('/Lender/Lender') As T2(Loc) WHERE DOCUMENT_ID = @DocumentID
...
--CollateralMatchResult
	...
	CASE WHEN ISNULL(C.CM_DescriptionChanged_IN ,'N') = 'Y' THEN 1 ELSE 0 END AS CM_DescriptionChanged,
...
ISNULL(C_MultiCollateral,0) AS [ReportUTLCollateral],		--(Mayank) ReportUTLCollateral is MultiCollateral under Collateral
...
CASE WHEN ISNULL(ISNULL(L.NUMBER_TX,#EX.LoanNumber),'') = '' 
	THEN '' 
	ELSE SUBSTRING(@FillerZero, 1, 18 - len(ISNULL(ISNULL(L.NUMBER_TX,#EX.LoanNumber),''))) 
		+ CAST(ISNULL(ISNULL(L.NUMBER_TX,#EX.LoanNumber),'') AS nvarchar(18)) END AS [LOAN_NUMBERSORT_TX],
...
SUBSTRING(@FillerZero, 1, 15 - len(ISNULL(L.BALANCE_AMOUNT_NO,#EX.LoanBalance))) + CAST(ISNULL(L.BALANCE_AMOUNT_NO,#EX.LoanBalance) AS nvarchar(15)) AS [LOAN_BALANCESORT_TX],
...
CASE
	...
	WHEN ISNULL(RCA_PROP.VALUE_TX,#EX.C_CollateralType) = 'MH'
		THEN CASE WHEN CC.VEHICLE_LOOKUP_IN = 'Y'
					THEN COALESCE(P.YEAR_TX,'') + ' ' + COALESCE(P.MAKE_TX,'') + '/' + COALESCE(P.MODEL_TX,'')
				 ELSE  CASE WHEN AM.LINE_1_TX IS NOT NULL 
						THEN ISNULL(AM.LINE_1_TX,'') + CHAR(13) + CHAR(10) + ISNULL(AM.LINE_2_TX + CHAR(13) + CHAR(10),'') + ISNULL(AM.CITY_TX,'') + ', ' + ISNULL(AM.STATE_PROV_TX, '') + ' ' + ISNULL(AM.POSTAL_CODE_TX,'')
						ELSE ''
						END
				END
...
CASE WHEN RC.SUMMARY_SUB_STATUS_CD = 'C' THEN ISNULL(CDPRM.AMOUNT_NO,0) ELSE NULL END AS [CPI_PREMIUM_AMOUNT_NO],
CASE WHEN RC.SUMMARY_SUB_STATUS_CD = 'C' THEN ISNULL(CDPRM.AMOUNT_NO,0) ELSE NULL END AS [PC_PREMIUM_AMOUNT_NO],

OP.EFFECTIVE_DT AS [BORRINSCOMPANY_EFF_DT],
CASE WHEN YEAR(OP.EXPIRATION_DT) = '9999' THEN NULL ELSE OP.EXPIRATION_DT END AS [BORRINSCOMPANY_EXP_DT],
OP.CANCELLATION_DT AS [BORRINSCOMPANY_CAN_DT],
OP.BIC_NAME_TX AS [INSAGENCY_NAME_TX],
BIA.PHONE_EXT_TX AS [INSAGENCY_PHONE_TX],
BIA.EMAIL_TX AS [INSAGENCY_EMAIL_TX],
BIA.FAX_EXT_TX AS [INSAGENCY_FAX_TX],
L.ID AS [LOAN_ID],
...
--needs to be multiline
	CASE WHEN (CM_CPIInplace = 1 OR LM_CPIInplace = 1) THEN '-CPI InForce' + CHAR(13) + CHAR(10) ELSE '' END +
...
LEFT JOIN OWNER_LOAN_RELATE OL WITH(NOLOCK) ON OL.LOAN_ID = L.ID AND OL.OWNER_TYPE_CD = 'B' AND OL.PRIMARY_IN = 'Y' AND OL.PURGE_DT IS NULL
...
SELECT * INTO #T2 FROM #tmpTable WHERE
([LOAN_BRANCHCODE_TX] IN (SELECT STRVALUE FROM @BranchTable) or @Branch = '1' or @Branch = '' or @Branch is NULL)
AND
([LOAN_DIVISIONCODE_TX] = @Division OR @Division = '1' OR @Division  = '' OR @Division is NULL 
OR ((@Division in ('3','8') and [LOAN_DIVISIONCODE_TX] not in ('3','8') and [PROPERTY_TYPE_CD] in ('VEH','BOAT'))
OR (@Division in ('7','9') and [LOAN_DIVISIONCODE_TX] not in ('7','9') and [PROPERTY_TYPE_CD] in ('EQ'))
OR (@Division in ('4','10') and [LOAN_DIVISIONCODE_TX] not in ('4','10') and [PROPERTY_TYPE_CD] not in ('VEH','BOAT','EQ'))))
AND
...
AND ((@ReportType = 'EXTRXCPT' AND (ReportLoanUnMatch = 0 OR (ReportLoanUnMatch = 1 AND [LOAN_RECORDTYPE_TX] IN ('A','D'))) AND [ReportIsDropZeroBalance] =0)
	OR
	(@ReportType <> 'EXTRXCPT' AND ((([LOAN_RECORDTYPE_TX] IS NOT NULL AND [LOAN_RECORDTYPE_TX] = 'G') OR [LOAN_RECORDTYPE_TX] IS NULL) AND
...
AND (@EXTRNUC = 'N' OR (@EXTRNUC = 'Y' AND [EXTR_COVERAGE_STATUS_TX] <> 'Loan Deleted' AND 
    ((@LoanUpdatesOnlyOnWI = 0 AND [LoanUpdatesOnlyOnLoan] = 0  AND 
		(
		([ReportLoanUnMatch] = 1 OR ([ReportCollateralUnMatch] = 1 AND [ReportCollRetainIndicator] = 0)) OR
		(([ReportNewLoan] = 1 OR [ReportNewCollateral] = 1) AND [ReportIsDropZeroBalance] = 0)
		)
	 )
	OR
...
AND (@EXTRXCPTOTER = 'N' OR (@EXTRXCPTOTER = 'Y' AND (
...
Or (ReportLoanReOccurance = 1 AND (ReportNewCollateral = 1 OR ReportCollateralUnMatch = 0))
	)))
