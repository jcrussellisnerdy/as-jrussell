if not exists (select 1 from sys.procedures where name = 'SearchWorkQueue')
	exec ('create proc SearchWorkQueue as return')

GO

/****** Object:  StoredProcedure [dbo].[SearchWorkQueue]    Script Date: 03/08/2011 10:22:33 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[SearchWorkQueue]
(
...
--	UNION
		...
		FROM 
			...
			left outer JOIN LENDER LDR ON LDR.ID =  T.Lender.value('(./Id)[1]', 'bigint')
		WHERE 
			wi.PURGE_DT is NULL
