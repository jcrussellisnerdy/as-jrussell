...
WHERE NAME_TX LIKE '%7450%' AND ACTIVE_IN = 'Y' AND ONHOLD_IN = 'N'


--date preference would start at the 1st of the previous month 
SELECT ID INTO #tmpPL FROM dbo.PROCESS_LOG
WHERE PROCESS_DEFINITION_ID IN (SELECT * FROM #tmpPD) AND UPDATE_DT >= '2017-08-01' 
...
WHERE  RH.ID IN (SELECT * FROM #tmpRH) AND rh.REPORT_ID = 46
ORDER BY rh.UPDATE_DT ASC 

SELECT * FROM dbo.ACCOUNTING_PERIOD

SELECT L.NUMBER_TX, N.* FROM dbo.PROCESS_LOG_ITEM PLI
JOIN dbo.NOTICE N ON N.ID = PLI.RELATE_ID AND PLI.RELATE_TYPE_CD = 'Allied.UniTrac.Notice'
JOIN dbo.LOAN L ON L.ID = N.LOAN_ID
WHERE PLI.PROCESS_LOG_ID = 50742934 AND L.NUMBER_TX=  '0001466730'


SELECT * FROM dbo.LOAN
WHERE NUMBER_TX = '0001466730'


SELECT * FROM dbo.NOTICE
WHERE LOAN_ID = 179182910
...
WHERE PROCESS_LOG_ID = '' AND RELATE_TYPE_CD = 'Allied.UniTrac.ForcePlacedCertificate'
-----------------------------------
...
select * --INTO UniTracHDStorage..NC0xxxxxx_FPC
from FORCE_PLACED_CERTIFICATE
WHERE ID IN (SELECT * FROM #tmpFPC)
ORDER BY FORCE_PLACED_CERTIFICATE.ID
...
JOIN  UniTracHDStorage..INC0xxxxxx_FPC fpc2 ON fpc.id=fpc2.id
