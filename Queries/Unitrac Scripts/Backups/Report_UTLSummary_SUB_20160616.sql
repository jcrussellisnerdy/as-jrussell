...
/****** Object:  StoredProcedure [dbo].[Report_UTLSummary]    Script Date: 6/16/2016 5:19:58 PM ******/
SET ANSI_NULLS ON
GO
...
,
CASE (SELECT COUNT(*) FROM REF_CODE WHERE DOMAIN_CD = 'InsuranceValidationException' AND MEANING_TX = UPPER(ISNULL(UTL.MSG_LOG_TX,'')))
	WHEN 0 THEN CASE SUBSTRING(UTL.MATCH_RESULT_CD,1,3) WHEN 'EXA' THEN 'EXACT' ELSE 'INEXACT' END
	ELSE 
	CASE WHEN (UTL.APPLY_STATUS_CD = 'APP' AND SUBSTRING(CASE WHEN SUBSTRING(ISNULL(UTL.USER_MATCH_RESULT_CD,'   '),1,3) = 'EXA' THEN 'EXACT' ELSE UTL.MATCH_RESULT_CD END,1,3) = 'EXA') THEN 'EXCEPTION' 
	ELSE CASE SUBSTRING(UTL.MATCH_RESULT_CD,1,3) WHEN 'EXA' THEN 'EXACT' ELSE 'INEXACT' END
	END
END AS MATCH,
...
*/
;With UPC(OWNER_POLICY_ID, DEDUCTIBLE_NO, TYPE_CD, SUB_TYPE_CD, START_DT)
 As
(
	Select Top 100 Percent pc.OWNER_POLICY_ID, pc.DEDUCTIBLE_NO, pc.TYPE_CD, pc.SUB_TYPE_CD, pc.START_DT
