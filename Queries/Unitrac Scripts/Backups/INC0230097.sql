...
WHERE   WORKFLOW_DEFINITION_ID = 11
        ...
        AND ID IN ( 30564194, 30589414, 30601813, 30639863, 30689156, 30731524, 30740084, 30752614, 30756498, 30764448, 30777983, 30790010 , 30830142  )
...
SELECT T.ID [WI_ID],  WIPLI.ID [WIPLI_ID], WIPLI.PURGE_DT [WIPLI.PURGE_DT], PLI.ID [PLI_ID], PLI.PURGE_DT [PLI.PURGE_DT], WIA.ID [WIA.ID],  WIA.PURGE_DT [WIA.PURGE_DT]
--INTO #TMP
 FROM #TMPWI T
...
SELECT * INTO #TMP_ESCROW_01 
FROM #TMP_ESCROW WHERE WORK_ITEM_ID NOT IN 
(
SELECT DISTINCT WORK_ITEM_ID FROM #TMP_ESCROW
WHERE USER_ACTION = ''
)
----
SELECT ESC_STATUS_CD, ESC_SUB_STATUS_CD, USER_ACTION,* FROM #TMP_ESCROW_01
