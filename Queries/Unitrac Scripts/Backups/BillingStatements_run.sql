...
/****** Object:  StoredProcedure [dbo].[Report_BillingStatements]    Script Date: 7/14/2017 11:18:09 AM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[Report_BillingStatements] 
(
...
BEGIN
   ...
   BEGIN
      ...
      SET      CPI_BILLED_NET_TOTAL_NO = 
                  CASE WHEN PRD_TOTAL_AMT_NO - PRD_PAYMENT_TOT >= 0 
                     ...
                     ELSE
                        -- Special Case: If you had not had a payment yet for the 
                        -- term, and your a R-C is less than zero, which can happen 
                        -- with the refund method and proration, then in this case, 
                        -- it would become a charge on the report (rather than a refund)
                        CASE WHEN PRD_PAYMENT_TOT = 0 THEN ABS(PRD_TOTAL_AMT_NO - PRD_PAYMENT_TOT) ELSE 0 END
                  END,
   ...
   BEGIN
      ...
      UPDATE   T
      SET      PRD_START_DT = C.PERIOD_START_DT,
               PRD_END_DT = C.PERIOD_END_DT,
...
BEGIN
      ...
      --       to remove appropriate items created during the SMP splitting
		...
		FROM #tmptable t
				outer apply
				(
				   SELECT * FROM dbo.fn_GetOutstandingFinancialTxns(t.FPC_ID, @BillingGroupID)
				) FTX_IDs

	   --Create a temporary table to hold monthly period information such as term and policy start/end dates per bill cycle
	   DECLARE @smpPeriods AS TABLE  (FPC_ID BIGINT, SMP_TERM_NO INT, SMP_TOTAL_AMT_NO DECIMAL(8,2), TXN_TYPE_CD NVARCHAR(10),SMP_START_DT DATETIME, SMP_END_DT DATETIME)
	 ...
	 --Update the fields that are used in the report with the new values
      ...
      ALTER TABLE #tSMP DROP COLUMN SMP_TOTAL_AMT_NO, SMP_START_DT, SMP_END_DT, SMP_TERM_NO
