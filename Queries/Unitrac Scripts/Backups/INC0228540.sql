...
SELECT DISTINCT  L.NUMBER_TX ,
        ...
        RC.ID [RC_ID] ,
        L.ID [LoanID]
--INTO UniTracHDStorage..INC0228540_RC
FROM    LOAN L
...
SET ESCROW_IN = 'N', UPDATE_DT = GETDATE(), UPDATE_USER_TX = 'INC0228540', LOCK_ID = LOCK_ID+1
--SELECT ESCROW_IN, LOCK_ID [RC], * FROM dbo.REQUIRED_COVERAGE
WHERE ID IN (SELECT [RC_ID] FROM UniTracHDStorage..INC0228540_RC)
 ...
 SELECT DISTINCT 'Allied.UniTrac.RequiredCoverage' , RC.ID , 'INC0228540' , 'N' , 
 GETDATE() ,  1 , 
 'Removed Escrow Flag', 
 'Y' , 'N' , 1 ,  'Allied.UniTrac.RequiredCoverage' , RC.ID , 'PEND' , 'N'
FROM REQUIRED_COVERAGE RC 
WHERE RC.ID IN (SELECT RC_ID FROM UniTracHDStorage..INC0228540_RC)


--INC0228540_RE is actually originial backup for ticket INC0231872


SELECT * 
INTO UniTracHDStorage..INC0228540_RE_2
FROM dbo.REQUIRED_ESCROW
WHERE REQUIRED_COVERAGE_ID IN (SELECT RC_ID FROM UniTracHDStorage..INC0228540_RC)
...
WHERE ID IN (SELECT ID FROM  UniTracHDStorage..INC0228540_RE_2) AND SUB_STATUS_CD = 'POD'
