DECLARE @xlstName nvarchar(50) = 'FICS-ESCROW'
...
'<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xslHelper="urn:xslHelper" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:FormatInsurance="urn:custom-scripts-csharp" version="1.0">
   ...
   <xsl:template match="EscrowBaseXML">
      ...
      <!-- 01: Lender # -->
      <xsl:value-of select="FormatInsurance:PadSpaceRight(RDLenderCode, 10)" />
      <!-- 11: Loan Number -->
      <xsl:value-of select="FormatInsurance:PadSpaceRight(LoanNumber, 20)"/>
      <!-- 31: Filler -->
      <xsl:value-of select="FormatInsurance:PadSpaceRight('''' , 1)" />
      <!-- 32: Delinquent Date -->
      <xsl:value-of select="FormatInsurance:PadSpaceRight(DelinquentDate, 8)" />
      <!-- 40: Unique Iden Code -->
      <xsl:value-of select="FormatInsurance:PadSpaceRight('''', 11)" />
      <!-- 51: Insurance Type Cd -->
      <xsl:value-of select="FormatInsurance:PadSpaceRight(xslHelper:GetInsuranceTypeCd(TypeCode), 3)" />
      <!-- 54: Program Type Cd -->
      ...
      <xsl:value-of select="FormatInsurance:PadSpaceRight('''', 5)" />
      <!-- 98: Premium Amount -->
      <xsl:value-of select="FormatInsurance:FormatAmount(TotalFormatted, 11)"/>
      <!-- 109: Premium Due Date -->
      <xsl:value-of select="FormatInsurance:PadSpaceRight(DueDate, 8)" />
      <!-- 117: Payment Identifier -->
      <xsl:value-of select="FormatInsurance:PadSpaceRight('''', 15)" />
      <!-- 132: Premium Paid Date -->
      <xsl:value-of select="FormatInsurance:PadSpaceRight(PaidThruDate, 8)"/>
      ...
      <xsl:value-of select="FormatInsurance:PadSpaceRight(xslHelper:PaymentFrequency(FormatInsurance:DateFromStringYYYYMMDD(PolicyEffectiveDate), FormatInsurance:DateFromStringYYYYMMDD(PaidThruDate)), 3)" />
      <!-- 170: Coverage Amount -->
      <xsl:value-of select="FormatInsurance:FormatAmountNoDecimal(CoverageAmount, 9)"/>
      <!-- 179: Policy Number -->
      ...
      <!--222: Name-->
      <xsl:value-of select="FormatInsurance:LimitLength(FormatInsurance:PadSpaceRight(Company, 40), 40)"/>
      <!-- 262: Co-Maker Name -->
      ...
      <xsl:value-of select="FormatInsurance:LimitLength(FormatInsurance:PadSpaceRight(PrevPolicyNumber, 25), 25)" />
      <!--709: Filler -->
