DECLARE @xlstName nvarchar(50) = 'DNH-BILLING'
...
'<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xslHelper="urn:xslHelper" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:FormatInsurance="urn:custom-scripts-csharp" version="1.0">
  ...
  <xsl:template match="BillingXML">
    ...
    <!-- Loan Number -->
    <xsl:value-of select="FormatInsurance:PadSpaceRight(LoanNumber, 35)" />
    <!-- Coverage Type -->
    <xsl:value-of select="'' ''" />
    <!-- Coverage Amount -->
    <xsl:value-of select="FormatInsurance:FormatAmount(Basis, 11)" />
    <!-- Agent Payee -->
    ...
    <!-- Cancel Date -->
    <xsl:value-of select="FormatInsurance:FormatCYYMMDD(PolicyCancellationDate)" />
    <!-- Expire Date -->
    ...
    <!-- Premium Due Amount -->
    <xsl:value-of select="FormatInsurance:FormatAmount(Net, 8)" />
    <!-- Premium Due Date -->
    <xsl:value-of select="FormatInsurance:FormatCYYMMDD(PolicyExpirationDate)" />
    <!-- Borrower Name -->
    <xsl:value-of select="FormatInsurance:LimitLength(FormatInsurance:PadSpaceRight(concat(LastName, '', '', FirstName), 30), 30)" />
    <!-- Property Address 1 -->
    <xsl:value-of select="FormatInsurance:PadSpaceRight(OwnerAddress1, 30)" />
    <!-- Property Address 2 -->
    <xsl:value-of select="FormatInsurance:PadSpaceRight(OwnerAddress2, 30)" />
    <!-- Property City -->
    <xsl:value-of select="FormatInsurance:PadSpaceRight(OwnerCity, 25)" />
    <!-- Property State -->
    <xsl:value-of select="FormatInsurance:PadSpaceRight(OwnerState, 2)" />
    <!-- Property Zip -->
    <xsl:value-of select="FormatInsurance:LimitLength(FormatInsurance:PadSpaceRight(OwnerZip, 5), 5)" />
    <!-- Company Name -->
    ...
    <!-- Insurance Co. Name -->
    <xsl:value-of select="FormatInsurance:LimitLength(FormatInsurance:PadSpaceRight(BorrowerInsComp, 8), 8)" />
    <!-- Add Ins Flag -->
    <xsl:value-of select="'' ''" />
    <!-- CR &LF -->
