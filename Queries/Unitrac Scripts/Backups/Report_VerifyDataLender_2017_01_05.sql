...
/****** Object:  StoredProcedure [dbo].[Report_VerifyDataLender]    Script Date: 1/5/2017 12:47:37 PM ******/
SET ANSI_NULLS ON
GO
...
IF OBJECT_ID(N'tempdb..#tmp',N'U') IS NOT NULL
  DROP TABLE #tmp
IF OBJECT_ID(N'tempdb..#tmpHalfWay',N'U') IS NOT NULL
  DROP TABLE #tmpHalfWay
IF OBJECT_ID(N'tempdb..#tmpAllProp',N'U') IS NOT NULL
  DROP TABLE #tmpAllProp
IF OBJECT_ID(N'tempdb..#tmpAllCol',N'U') IS NOT NULL
  DROP TABLE #tmpAllCol
...
SELECT 
		...
		wi.CREATE_DT As workitem_create_dt, 
		l.ID as loan_id, 
		l.NUMBER_TX, 
...
#tmpHalfWay t
 OUTER APPLY
   ...
   [PROPERTY_DESCRIPTION] [nvarchar] (800) NULL,
	...
	[RC_ID] [bigint] NULL
	,WI_CREATE_DT DateTime NULL	--WORK_ITEM CREATE_DT
) ON [PRIMARY]
...
CASE WHEN ISNULL(t.ADDRESS_ID,'') <> ''
  ...
  PROPERTY_DESCRIPTION = dbo.fn_GetPropertyDescriptionForReports(t.FinalCollID),
  t.TempPropertyType,
...
t.CurrentValue,
	'' as VerifyDataComment,
		...
		,WI_CREATE_DT = t.workitem_create_dt
