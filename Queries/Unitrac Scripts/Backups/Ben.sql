...
SELECT RELATE_ID INTO #tmpWI FROM dbo.WORK_ITEM
WHERE  WORKFLOW_DEFINITION_ID = 9 AND ID IN () 
...
WHERE PROCESS_LOG_ID IN (SELECT * FROM #tmpWI)
...
SELECT rh.REPORT_DATA_XML.value( '(/ReportData/Report/WorkItemId/@value)[1]', 'varchar(500)' ) as [WI.ID],
	...
	WHERE ID IN (SELECT * FROM #tmpRH) AND rh.REPORT_ID = 32
