/****** Object:  StoredProcedure [dbo].[Report_PendingMessageServer]     ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Report_PendingMessageServer]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Report_PendingMessageServer]
GO

/****** Object:  StoredProcedure [dbo].[Report_PendingMessageServer]    Script Date: 09/26/2016 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[Report_PendingMessageServer]
AS
BEGIN
		  ;With
		   tmpLenders As
          ...
          SELECT
		   [Direction] = case when IsNull(MESSAGE_DIRECTION_CD,'I')='I' then 'INBOUND' else MESSAGE_DIRECTION_CD end
