...
SELECT * --INTO UniTracHDStorage..INC0219421
 FROM UniTrac..WORK_ITEM
WHERE CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') = '3800'
AND WORKFLOW_DEFINITION_ID IN (2,3,6,8)
--AND STATUS_CD = 'Initial'
AND WORK_ITEM.STATUS_CD NOT LIKE 'Complete' AND WORK_ITEM.STATUS_CD NOT LIKE 'Withdrawn'
ORDER BY WORKFLOW_DEFINITION_ID ASC

SELECT * FROM UniTracHDStorage..INC0219421
...
UPDATE  UniTrac..WORK_ITEM
SET     STATUS_CD = 'Complete' ,
        UPDATE_USER_TX = 'INC0219421'
WHERE   ID IN (SELECT ID FROM UniTracHDStorage..INC0219421 )
...
--                WHERE   UPDATE_USER_TX = 'INC0######' AND CODE_TX = '3800')
