...
SELECT L.STATUS_CD [Loan_Status], CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') Lender, WI.*  --INTO UniTracHDStorage..WI_INC0246368
FROM    UniTrac..WORK_ITEM WI 
INNER JOIN dbo.LOAN L ON L.ID = WI.RELATE_ID AND WI.RELATE_TYPE_CD = 'Allied.UniTrac.Loan'
WHERE   WI.ID IN ( 30235462, 30238174 , 30238251,32605421  )
...
WHERE   LENDER.CODE_TX = '3087'
        ...
        AND LOAN.RECORD_TYPE_CD IN ('G','A', 'P')
		AND WI.ID IN (SELECT ID FROM UniTracHDStorage..WI_INC0246368)
		
SELECT * FROM #TMPWI

SELECT * INTO UniTracHDStorage..LOAN_INC0246368
FROM dbo.LOAN WHERE ID IN (SELECT RELATE_ID FROM UniTracHDStorage..WI_INC0246368)


UPDATE  LN
SET     SPECIAL_HANDLING_XML = NULL ,
        UPDATE_DT = GETDATE() ,
        UPDATE_USER_TX = 'INC0246368' ,
...
--SELECT COUNT(*)
FROM    LOAN LN
        JOIN #TMPWII ON #TMPWII.LOAN_ID = LN.ID
...
--SELECT COUNT(*)
FROM    WORK_ITEM WI
        JOIN #TMPWII ON #TMPWII.WI_ID = WI.ID




SELECT STATUS_CD, UPDATE_DT, UPDATE_USER_TX, LOCK_ID FROM dbo.WORK_ITEM
WHERE ID IN (SELECT WI_ID FROM #TMPWII)



SELECT SPECIAL_HANDLING_XML, UPDATE_DT, UPDATE_USER_TX, LOCK_ID
FROM dbo.LOAN WHERE ID IN (SELECT LOAN_ID FROM #TMPWII)
