...
SELECT L.STATUS_CD [Loan_Status], CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') Lender, WI.*  --INTO UniTracHDStorage..WI_INC0248469
FROM    UniTrac..WORK_ITEM WI  JOIN dbo.LOAN L ON L.ID = WI.RELATE_ID AND WI.RELATE_TYPE_CD = 'Allied.UniTrac.Loan'
WHERE   WI.ID IN (  32305046, 33215496, 3356197 , 33607190 )
...
WHERE   LENDER.CODE_TX = '1940'
		...
		AND WI.ID IN (SELECT ID FROM UniTracHDStorage..WI_INC0248469)
		
DROP TABLE #TMPWI

SELECT * INTO UniTracHDStorage..LOAN_INC0248469
