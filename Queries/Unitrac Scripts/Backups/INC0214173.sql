...
SELECT CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') Lender, * --INTO UniTracHDStorage..INC0214173
FROM    UniTrac..WORK_ITEM
WHERE   ID IN ( 27652399,27653373, 27819529  )
...
WHERE   LENDER.CODE_TX = '6079'
		...
		AND WI.ID IN (SELECT ID FROM UniTracHDStorage..INC0214173)
