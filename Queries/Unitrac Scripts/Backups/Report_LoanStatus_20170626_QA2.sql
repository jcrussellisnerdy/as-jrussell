...
/****** Object:  StoredProcedure [dbo].[Report_LoanStatus]    Script Date: 6/26/2017 11:54:46 PM ******/
SET ANSI_NULLS ON
GO
...
--COVERAGE:
	...
	[LENDER_REVIEW_EVENT] [nvarchar] (1000),
--INSURANCE:
...
'
	  ...
	  L.PREDICTIVE_SCORE_NO as LOAN_PREDICTIVE_SCORE_NO,
	  L.PREDICTIVE_DECILE_NO as LOAN_PREDICTIVE_DECILE_NO,
'
select @columns = @columns +
--LOAN RELATED DATA
'
...
+ case when @ReportType = 'LNDRREVWEVT'
		...
		LEFT JOIN REF_CODE LndrRevRef on LndrRevRef.DOMAIN_CD = ''LenderReviewAdditionalComment'' and LndrRevRef.CODE_CD = SPECIAL_HANDLING_XML.value(''(/SH/AdditionalComment)[1]'', ''nvarchar(max)'')
			WHERE IH.PROPERTY_ID = P.ID AND IH.TYPE_CD = ''LRVWEVNT'' AND IH.PURGE_DT IS NULL
...
Begin
	...
	delete #tmptable where BORRINSCOMPANY_EXP_DAYS < 0 or BORRINSCOMPANY_EXP_DAYS > 30
End
...
Begin
	delete #tmptable where COLLATERAL_STATUSCODE <> 'A' and REQUIREDCOVERAGE_STATUSCODE  <> 'A'
	delete #tmptable where BORRINSCOMPANY_EXP_DAYS < 0 or BORRINSCOMPANY_EXP_DAYS > 60
End
