...
SELECT L.STATUS_CD [Loan_Status], CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') Lender, WI.*  INTO UniTracHDStorage..WI_INC0247176
FROM    UniTrac..WORK_ITEM WI 
INNER JOIN dbo.LOAN L ON L.ID = WI.RELATE_ID AND WI.RELATE_TYPE_CD = 'Allied.UniTrac.Loan'
WHERE   WI.ID IN ( SELECT WI_ID FROM #TMPWI)
...
SELECT * INTO UniTracHDStorage..LOAN_INC0247176
FROM dbo.LOAN WHERE ID IN (SELECT LOAN_ID FROM #TMPWI)


SELECT T.WI_ID [Work Item], L.NUMBER_TX FROM dbo.LOAN L
 JOIN #TMPWI T ON L.ID = T.LOAN_ID

UPDATE  LN
SET     SPECIAL_HANDLING_XML = NULL ,
        UPDATE_DT = GETDATE() ,
        UPDATE_USER_TX = 'INC0247176' ,
