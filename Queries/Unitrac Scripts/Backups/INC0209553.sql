...
SELECT * --INTO UniTracHDStorage..INC0209553
FROM dbo.WORK_ITEM
WHERE ID IN (26367663 , 26388032, 26719091)
...
WHERE   LENDER.CODE_TX = '6350'
        ...
        AND LOAN.PURGE_DT IS NULL
        AND LOAN.RECORD_TYPE_CD IN ('G','A')
		AND WI.ID IN (26367663 , 26388032)
...
--SELECT COUNT(*)
FROM    LOAN LN
        JOIN #TMPWI2 ON #TMPWI2.LOAN_ID = LN.ID
...
--SELECT COUNT(*)
FROM    WORK_ITEM WI
        JOIN #TMPWI2 ON #TMPWI2.WI_ID = WI.ID
