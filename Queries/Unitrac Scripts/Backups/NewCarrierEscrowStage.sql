...
/****** Object:  StoredProcedure [dbo].[Report_EscrowNewCarrier]    Script Date: 10/8/2015 1:39:54 PM ******/
...
Declare @ProcessLogID as bigint=0
 ...
 Begin
  Select @LenderCode=CASE
	WHEN @Debug=1 THEN '2771' -- PenFed
	WHEN @Debug=2 THEN '2771' -- PenFed
	WHEN @Debug=3 THEN '2252' -- Space Coast
	WHEN @Debug=4 THEN '2028' -- Truity
	WHEN @Debug=5 THEN '4286' -- Royal CU
	WHEN @Debug=6 THEN '7150' -- Country Bank for Savings
	WHEN @Debug=7 THEN '7500' -- Greater Nevada Mortgage
	WHEN @Debug=8 THEN '8500' -- Texas Dow Employees CU

	WHEN @Debug>999 THEN Cast(@Debug As NVarChar(10))
 ...
 */
 ...
 AND 1 = (CASE WHEN NOT(Coalesce(NullIf(BIC.NAME, ''), NullIf(BICP.NAME, '')) IS NULL OR RTrim(UPPER(STRIPPED_BORRINSCOMPANY.NAME_TX)) = RTrim(UPPER(STRIPPEDPREV_BORRINSCOMPANY.NAME_TX))) THEN 1
...
From LENDER_CTE LND WITH(NOLOCK)
 ...
 AND 1 = (CASE WHEN NOT(Coalesce(NullIf(OP.BIC_NAME_TX, ''), NullIf(POP.BIC_NAME_TX, '')) IS NULL OR RTrim(UPPER(STRIPPED_BORRINSCOMPANY.NAME_TX)) = RTrim(UPPER(STRIPPEDPREV_BORRINSCOMPANY.NAME_TX))) THEN 1
 /* exclude same borrowers whose policies 1st 6 digits are the same */
  WHEN RTrim(UPPER(STRIPPED_BORRINSCOMPANY.POLICY_NO)) = RTrim(UPPER(STRIPPEDPREV_BORRINSCOMPANY.POLICY_NO)) THEN 0
 /* exclude same borrowers whose policies go from Binder to policy number */
  WHEN (STRIPPEDPREV_BORRINSCOMPANY.POLICY_NO LIKE '%BIND%' And STRIPPED_BORRINSCOMPANY.POLICY_NO NOT LIKE '%BIND%') THEN 0
