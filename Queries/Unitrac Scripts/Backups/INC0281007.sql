...
SELECT  *
--INTO UniTracHDStorage..INC0281007
FROM    UniTrac..WORK_ITEM
WHERE   ID IN (36804397 , 36804394 )

SELECT * FROM dbo.PROCESS_LOG_ITEM
WHERE PROCESS_LOG_ID IN (41360965,
41360968) --AND STATUS_CD <> 'err'
AND RELATE_TYPE_CD ='Allied.UniTrac.Notice'


SELECT PDF_GENERATE_CD, * FROM dbo.NOTICE
WHERE id IN (21017245,21017244,
21017246)
...
--        LOCK_ID = LOCK_ID + 1 ,
--        UPDATE_DT = GETDATE() ,
--        UPDATE_USER_TX = 'INC0281007'
--WHERE   ID IN ( SELECT ID FROM UniTracHDStorage..INC0281007 )
