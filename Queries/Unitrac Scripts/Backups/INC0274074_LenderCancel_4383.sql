...
----REPLACE 4383 WITH THE THE Lender ID
----REPLACE INC0274074 WITH THE TICKET ID
---Checking UTLs WIs
USE UniTrac

--SELECT * FROM dbo.LENDER WHERE CODE_TX IN ('4383')
...
--INTO UniTracHDStorage..INC0274074
FROM    WORK_ITEM
WHERE   RELATE_TYPE_CD = 'Allied.UniTrac.UTLMatchResult'
        ...
        WHERE   LENDER.CODE_TX = '4383' AND WORK_ITEM.STATUS_CD NOT IN ('Complete', 'Withdrawn')
			   )
...
WHERE WORK_ITEM.STATUS_CD NOT IN ('Complete', 'Withdrawn')) AND WORK_ITEM.LENDER_ID = '903'
...
SET     STATUS_CD = 'Complete' ,
        UPDATE_USER_TX = 'INC0274074',
		 LOCK_ID = CASE WHEN LOCK_ID >= 255 THEN 1
                  ELSE LOCK_ID + 1 END
WHERE   ID IN  (  SELECT ID FROM UniTracHDStorage..INC0274074)
...
SELECT * INTO UniTracHDStorage..LOAN_INC0274074
