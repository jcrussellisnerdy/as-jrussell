...
----REPLACE INC0218934 WITH THE TICKET ID
---Checking UTLs WIs
USE UniTrac
...
SELECT * --INTO UniTracHDStorage..INC0218934
FROM UniTrac..WORK_ITEM
WHERE CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') = '7596'
...
SET     STATUS_CD = 'Complete' ,
        UPDATE_USER_TX = 'INC0218934',
		 LOCK_ID = CASE WHEN LOCK_ID >= 255 THEN 1
                  ELSE LOCK_ID + 1 END
WHERE   ID IN  (SELECT ID FROM UniTracHDStorage..INC0218934)
...
SELECT * INTO UniTracHDStorage..LOAN_INC0218934
...
--                WHERE   UPDATE_USER_TX = 'INC0218934' AND CODE_TX = '7596')



--SELECT CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)'),  * FROM dbo.WORK_ITEM
--		WHERE WORKFLOW_DEFINITION_ID = '8' AND STATUS_CD = 'WITHDRAWN' AND  UPDATE_USER_TX = 'INC0218934'
--		ORDER BY RELATE_ID ASC

--		SELECT * FROM dbo.LOAN
--		WHERE UPDATE_USER_TX = 'INC0218934' 
--		ORDER BY ID ASC
