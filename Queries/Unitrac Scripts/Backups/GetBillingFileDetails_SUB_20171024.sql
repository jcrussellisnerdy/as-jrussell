...
/****** Object:  StoredProcedure [dbo].[GetBillingFileDetails]    Script Date: 10/24/2017 11:05:38 AM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
   ...
   BEGIN
      ...
      FROM     FINANCIAL_TXN_APPLY fta
               ...
               -- If there is a Payment Term 1, then all payments have occurred after payment terms went live.
               -- * DO NOT INCLUDE FPCS HAVING PAYMENTS (Posted or Unposted) WITH A TERM ZERO IN NON-TERMIFIED 
               --   PAYMENT DISTRIBUTION IF A PAYMENT TERM 1 HAS OCCURRED
               OUTER APPLY
               (
                  select   CASE WHEN FPC_ID IS NULL THEN 0 ELSE 1 END HAS_TERM_ONE
                  from     FINANCIAL_TXN HPTFTX
                           LEFT JOIN FINANCIAL_TXN_PAYMENT_TERM_DISTRIBUTION HPTFTPTD ON HPTFTPTD.FTX_ID = HPTFTX.ID
                  where    FPC_ID = ftx.FPC_ID
                           and TXN_TYPE_CD = 'P'
                           and ISNULL(HPTFTPTD.TERM_NO, 0) = 1
               ) PAYMENT_TERM_CHECK
