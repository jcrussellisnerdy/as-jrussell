...
WHERE   WORKFLOW_DEFINITION_ID = 11
        ...
        AND ID IN ( 32886605, 32892080, 32910399, 32913412, 32914608, 32917107, 32937188, 32937258, 32943776, 32944594, 32958773, 32958844, 32988054, 3295453, 33000357, 33005639, 33015771, 33023267, 33023873, 33030025, 33046678, 33054952, 33068628, 33068881 , 33079203  )
...
WHERE WI_ID IN (32914608,32943776,33000357)

SELECT * FROM #TMP
/*
UPDATE WIP
SET PURGE_DT = T.[WIPLI.PURGE_DT] 
FROM  dbo.WORK_ITEM_PROCESS_LOG_ITEM_RELATE WIP
join #TMP T ON T.WIPLI_ID = WIP.ID
--SELECT * FROM dbo.WORK_ITEM_PROCESS_LOG_ITEM_RELATE
WHERE ID IN (SELECT WIPLI_ID FROM #TMP where WI_ID IN (32914608,32943776,33000357))


UPDATE PLI
SET PURGE_DT = [PLI.PURGE_DT]
FROM  dbo.PROCESS_LOG_ITEM PLI 
join #TMP T ON T.PLI_ID  = PLI.ID
--SELECT * FROM dbo.PROCESS_LOG_ITEM
WHERE ID IN (SELECT PLI_ID FROM #TMP where WI_ID IN (32914608,32943776,33000357))
