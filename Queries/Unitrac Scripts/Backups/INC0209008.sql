...
SELECT  * ---INTO UniTracHDStorage..INC0209008
FROM    UniTrac..WORK_ITEM
WHERE   ID IN ( 27413838  )
...
WHERE   ID IN ( SELECT ID FROM UniTracHDStorage..INC0209008 )
        AND WORKFLOW_DEFINITION_ID = 3
        AND ACTIVE_IN = 'Y'

SELECT * FROM UniTracHDStorage..INC0209008
