SELECT * --INTO UniTracHDStorage..INC0209071
FROM dbo.WORK_ITEM
WHERE ID IN (27234578,
27276748,
27364539,
27364540,
27390313,
27491894 ,
27514324 ) AND STATUS_CD = 'Initial'
...
WHERE   WORKFLOW_DEFINITION_ID = 11
        ...
        AND ID IN ( SELECT ID FROM UniTracHDStorage..INC0209071)--WHERE STATUS_CD = 'Initial' )
...
FROM
 ...
 PROCESS_LOG_ITEM PLI
JOIN  WORK_ITEM_PROCESS_LOG_ITEM_RELATE WIPLIR  
 ...
 CROSS APPLY (
  ...
  JOIN #TMPWI ON #TMPWI.id = PLI.RELATE_ID
...
SELECT * FROM dbo.WORK_ITEM_ACTION WHERE WORK_ITEM_ID IN (
SELECT ID FROM #TMPWI)

SELECT DISTINCT WORK_ITEM_ID FROM #TMPWI
WHERE USER_ACTION = ''
