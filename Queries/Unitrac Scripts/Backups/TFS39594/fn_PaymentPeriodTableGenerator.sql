IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[fn_PaymentPeriodTableGenerator]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
    DROP FUNCTION [dbo].[fn_PaymentPeriodTableGenerator] 
GO
...
begin
   ...
   BEGIN
      ...
      -- If the running payment total is greater than the "charges" (and is greater than zero),
      -- then update the payments for this term to be completely satisfied (if the "charges" 
      -- are less than zero, then there is nothing to charge, so neither the term payments
      -- will be updated, nor will the running payment total be decremented
      IF ABS(@runningpaymenttotal) > @charges AND ABS(@runningpaymenttotal) > 0
      BEGIN
            if (@charges > 0) 
            begin
               UPDATE @Temp1
