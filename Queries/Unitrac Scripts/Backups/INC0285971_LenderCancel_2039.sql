...
----REPLACE 2039 WITH THE THE Lender ID
----REPLACE INC0285971 WITH THE TICKET ID
---Checking UTLs WIs
USE UniTrac

--SELECT * FROM dbo.LENDER WHERE CODE_TX IN ('2039')
...
SET     STATUS_CD = 'Complete' ,
        UPDATE_USER_TX = 'INC0285971',
		 LOCK_ID = CASE WHEN LOCK_ID >= 255 THEN 1
                  ELSE LOCK_ID + 1 END
WHERE   ID IN  (  SELECT ID FROM UniTracHDStorage..INC0285971)
