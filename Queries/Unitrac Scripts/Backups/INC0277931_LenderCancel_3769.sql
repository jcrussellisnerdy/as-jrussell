...
--SELECT * FROM dbo.LENDER WHERE CODE_TX IN ('3769')

SELECT 
COUNT(*)
FROM UniTrac..WORK_ITEM WI
JOIN dbo.LENDER L ON L.ID = WI.LENDER_ID
WHERE L.CODE_TX IN ('3769')
...
INTO UniTracHDStorage..INC0277931
FROM UniTrac..WORK_ITEM WI
JOIN dbo.LENDER L ON L.ID = WI.LENDER_ID
WHERE L.CODE_TX IN ('3769')
AND WI.WORKFLOW_DEFINITION_ID IN (2,3,6,8)
...
SET     STATUS_CD = 'Complete' ,
        UPDATE_USER_TX = 'INC0277931',
		 LOCK_ID = CASE WHEN LOCK_ID >= 255 THEN 1
                  ELSE LOCK_ID + 1 END
WHERE   ID IN  (  SELECT ID FROM UniTracHDStorage..INC0277931)
...
SELECT * INTO UniTracHDStorage..LOAN_INC0277931
