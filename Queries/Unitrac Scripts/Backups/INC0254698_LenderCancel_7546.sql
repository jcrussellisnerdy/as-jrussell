...
----REPLACE 7546 WITH THE THE Lender ID
----REPLACE INC0254698 WITH THE TICKET ID
---Checking UTLs WIs
USE UniTrac
...
--INTO UniTracHDStorage..INC0254698
FROM    WORK_ITEM
WHERE   RELATE_TYPE_CD = 'Allied.UniTrac.UTLMatchResult'
        ...
        WHERE   LENDER.CODE_TX = '7546' AND WORK_ITEM.STATUS_CD NOT IN ('Complete', 'Withdrawn')
			   )
...
SET     STATUS_CD = 'Complete' ,
        UPDATE_USER_TX = 'INC0254698',
		 LOCK_ID = CASE WHEN LOCK_ID >= 255 THEN 1
                  ELSE LOCK_ID + 1 END
WHERE   ID IN  (  SELECT ID FROM UniTracHDStorage..INC0254698)
...
SELECT * INTO UniTracHDStorage..LOAN_INC0254698
