...
WHERE   WORKFLOW_DEFINITION_ID = 11
       AND STATUS_CD = 'INITIAL'
        AND ID IN ( 33242873, 33254851, 33256731, 33259248, 33278014, 33278428, 33280453, 33290558, 33311289, 33313214, 33326371, 33342689, 33353962, 33355794, 33365576, 33365737, 33372437, 33377678, 33394288, 33402727, 33433517, 33439232, 33451531, 33463660, 33464248, 33464325, 33477287  )
...
SELECT  WI_ID ,
        ...
        [PLI.PURGE_DT] ,
        [WIA ID] ,
        [WIA.PURGE_DT] FROM #TMP2


UPDATE W
SET W.PURGE_DT = T.[WIPLI.PURGE_DT]
--Select *
FROM dbo.WORK_ITEM_PROCESS_LOG_ITEM_RELATE W
JOIN #tmp T ON T.WIPLI_ID = W.ID
