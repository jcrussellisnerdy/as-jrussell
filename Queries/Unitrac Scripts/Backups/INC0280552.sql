...
where LL.CODE_TX = '3124' and L.NUMBER_TX IN 
('10192622563', '2550007828', '1050001828') and 
CAST(N.UPDATE_DT AS DATE) >= CAST(GETDATE()-7 AS DATE) and PDF_GENERATE_CD = 'ERR'
order by CPI_QUOTE_ID ASC

SELECT  WI.*
--INTO UniTracHDStorage..INC0280552_WI
FROM dbo.WORK_ITEM WI
JOIN dbo.LENDER L ON L.ID = WI.LENDER_ID
WHERE WI.ID IN ( 36820182)


select * from process_log_item
where process_log_ID iN (41402427) and relate_type_cd = 'Allied.UniTrac.Notice'

select *
INTO UniTracHDStorage..INC0280552_Notice
 from notice
where ID IN (21017254,
21017255,
21017256)


update N
set purge_dt= getdate(), lock_id = lock_id+1, update_dt = getdate(), update_user_tx = 'INC0280552'
--select *
 from notice N
where ID IN (21017254,
21017255,
...
SET     STATUS_CD = 'Complete' ,
        LOCK_ID = LOCK_ID + 1 ,
        UPDATE_DT = GETDATE() ,
        UPDATE_USER_TX = 'INC0280552'
WHERE   ID IN ( SELECT ID FROM UniTracHDStorage..INC0280552_wi )
...
--WHERE   ID IN ( SELECT ID FROM UniTracHDStorage..XXXXXXX )
--        AND WORKFLOW_DEFINITION_ID = 9
--        AND ACTIVE_IN = 'Y'
