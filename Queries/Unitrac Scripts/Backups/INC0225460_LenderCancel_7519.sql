...
----REPLACE INC0225460 WITH THE TICKET ID
---Checking UTLs WIs
USE UniTrac
...
SELECT * --INTO UniTracHDStorage..INC0225460
 FROM UniTrac..WORK_ITEM
WHERE CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') = '7519'
AND WORKFLOW_DEFINITION_ID IN (2,3,6,8)
--AND STATUS_CD = 'Initial'
AND WORK_ITEM.STATUS_CD NOT LIKE 'Complete' AND WORK_ITEM.STATUS_CD NOT LIKE 'Withdrawn'
ORDER BY WORKFLOW_DEFINITION_ID ASC

SELECT * FROM UniTracHDStorage..INC0225460

-------- Clear UTLs (- rows)
UPDATE  UniTrac..WORK_ITEM
SET     STATUS_CD = 'Complete' ,
        UPDATE_USER_TX = 'INC0225460',
		 LOCK_ID = CASE WHEN LOCK_ID >= 255 THEN 1
                  ELSE LOCK_ID + 1 END
--SELECT * FROM dbo.WORK_ITEM
WHERE   ID IN  (SELECT ID FROM  UniTracHDStorage..INC0225460 )
...
SELECT * INTO UniTracHDStorage..LOAN_INC0225460
