...
----REPLACE 2909 WITH THE THE Lender ID
----REPLACE INC0255252 WITH THE TICKET ID
---Checking UTLs WIs
USE UniTrac

--SELECT * FROM dbo.LENDER WHERE CODE_TX IN ('2909')
...
--INTO UniTracHDStorage..INC0255252
FROM    WORK_ITEM
WHERE   RELATE_TYPE_CD = 'Allied.UniTrac.UTLMatchResult'
        ...
        WHERE   LENDER.CODE_TX = '2909' AND WORK_ITEM.STATUS_CD NOT IN ('Complete', 'Withdrawn')
			   )
...
WHERE WORK_ITEM.STATUS_CD NOT IN ('Complete', 'Withdrawn')) AND WORK_ITEM.LENDER_ID = '1119'
...
SELECT * FROM UniTracHDStorage..INC0255252


-------- Clear UTLs (- rows)
UPDATE  UniTrac..WORK_ITEM
SET     STATUS_CD = 'Complete' ,
        UPDATE_USER_TX = 'INC0255252',
		 LOCK_ID = CASE WHEN LOCK_ID >= 255 THEN 1
                  ELSE LOCK_ID + 1 END
WHERE   ID IN  (  SELECT ID FROM UniTracHDStorage..INC0255252)
...
SELECT * INTO UniTracHDStorage..LOAN_INC0255252
...
WHERE ID IN (SELECT ID FROM UniTracHDStorage..INC0255252) AND STATUS_CD 
...
UPDATE LL
SET LL.SPECIAL_HANDLING_XML = INC.SPECIAL_HANDLING_XML
--SELECT LL.SPECIAL_HANDLING_XML, INC.SPECIAL_HANDLING_XML, *
FROM dbo.LOAN LL 
JOIN UniTracHDStorage..LOAN_INC0255252 INC ON LL.ID = INC.ID 
JOIN UniTracHDStorage..INC0255252 o  ON  INC.ID = o.RELATE_ID
WHERE o.[Collateral] <> 'RE'

UPDATE WI
SET WI.STATUS_CD = o.STATUS_CD
--SELECT  WI.STATUS_CD, o.STATUS_CD, *
FROM work_item WI
JOIN UniTracHDStorage..INC0255252 o  ON  WI.ID = o.ID
WHERE o.[Collateral] <> 'RE'
