...
----REPLACE 6546 WITH THE THE Lender ID
----REPLACE INC0248225 WITH THE TICKET ID
---Checking UTLs WIs
USE UniTrac
...
--INTO UniTracHDStorage..INC0248225
FROM    WORK_ITEM
WHERE   RELATE_TYPE_CD = 'Allied.UniTrac.UTLMatchResult'
        ...
        WHERE   LENDER.CODE_TX = '6546' AND WORK_ITEM.STATUS_CD NOT IN ('Complete', 'Withdrawn')
			   )
...
WHERE   CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') = '6546'
        AND WORKFLOW_DEFINITION_ID IN ( 2, 3, 6, 8 )
        AND WORK_ITEM.STATUS_CD NOT IN ( 'Complete', 'Withdrawn' )
        --AND ( CONTENT_XML.value('(/Content/Collateral/Type)[1]',
        --                        'varchar (50)') = 'VEH'
        --      OR CONTENT_XML.value('(/Content/Property/Description)[1]',
        --                           'varchar (50)') = 'VEH'
        --    )
...
SET     STATUS_CD = 'Complete' ,
        UPDATE_USER_TX = 'INC0248225',
		 LOCK_ID = CASE WHEN LOCK_ID >= 255 THEN 1
                  ELSE LOCK_ID + 1 END
WHERE   ID IN  (  SELECT ID FROM UniTracHDStorage..INC0248225)
...
WHERE   LENDER.CODE_TX = '6546'
		...
		AND WI.ID IN ( SELECT ID FROM UniTracHDStorage..INC0248225)

SELECT * FROM #TMPWI

SELECT * INTO UniTracHDStorage..LOAN_INC0248225
