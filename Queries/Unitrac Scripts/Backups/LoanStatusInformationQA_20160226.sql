...
/****** Object:  StoredProcedure [dbo].[Report_LoanStatus]    Script Date: 2/26/2016 1:28:26 PM ******/
...
'	   CASE
	   ...
	   ELSE CASE
		...
		WHEN L.STATUS_CD = ''A'' and RC.STATUS_CD in (''A'',''D'') and (RC.SUMMARY_STATUS_CD in (''A'',''N'') AND ISNULL(Notice.LastSent,'''') = '''')
				THEN RCISRef.MEANING_TX
		WHEN L.STATUS_CD = ''A'' and RC.STATUS_CD in (''A'',''D'') and (RC.SUMMARY_STATUS_CD not in (''A'',''N'') OR ISNULL(Notice.LastSent,'''') <> '''')
