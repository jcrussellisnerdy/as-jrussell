...
SELECT --OP.EXPIRATION_DT ,
       RC.* --INTO UniTracHDStorage..INC0312123_RC
FROM   LOAN L
...
WHERE  LL.CODE_TX = '1040' AND RC.INSURANCE_STATUS_CD <> 'e'
       AND L.NUMBER_TX IN ( '1017', '1019', '1039', '1042', '1043', '1049' ,
...
UPDATE OP
SET OP.EXPIRATION_DT = NULL, OP.UPDATE_DT = GETDATE(), OP.LOCK_ID =OP.LOCK_ID+1, OP.SUB_STATUS_CD = 'R',
OP.UPDATE_USER_TX = 'INC0312123'
--
SELECT EXPIRATION_DT, * 
FROM dbo.OWNER_POLICY OP
WHERE op.ID IN (SELECT ID FROM  UniTracHDStorage..INC0312123) 
...
---- SELECT INSURANCE_STATUS_CD, INSURANCE_SUB_STATUS_CD,*
FROM REQUIRED_COVERAGE RC
WHERE ID IN  (SELECT ID FROM  UniTracHDStorage..INC0312123_RC) AND RC.INSURANCE_STATUS_CD <> 'E'
...
FROM   LOAN L
       ...
       INNER JOIN dbo.OWNER_POLICY OP ON OP.ID = POP.OWNER_POLICY_ID
       INNER JOIN dbo.POLICY_COVERAGE PC ON PC.OWNER_POLICY_ID = OP.ID
WHERE  LL.CODE_TX = '1040' AND L.NUMBER_TX = '3759'


SELECT * FROM dbo.PROPERTY_CHANGE pc 
LEFT JOIN dbo.PROPERTY_CHANGE_UPDATE pcu ON pc.id = pcu.CHANGE_ID
WHERE pc.ENTITY_ID = '178977899' AND pc.ENTITY_NAME_TX = 'Allied.UniTrac.RequiredCoverage'
...
FROM
	...
	AND EE.REQUIRED_COVERAGE_ID IN (SELECT ID FROM  UniTracHDStorage..INC0312123_RC)
	----78


--	SELECT IH.ID AS IH_ID
--FROM  INTERACTION_HISTORY IH 
--WHERE  IH.PROPERTY_ID IN (SELECT PROPERTY_ID FROM  UniTracHDStorage..INC0312123_RC)
--AND 
--		IH.TYPE_CD = 'CPI'
