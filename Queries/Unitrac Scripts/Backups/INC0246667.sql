...
---- 
UPDATE WI
SET     STATUS_CD = 'Complete', LOCK_ID = CASE WHEN LOCK_ID >= 255 THEN 1 ELSE LOCK_ID + 1 END,
...
WHERE ID IN (SELECT MAX(ID) FROM dbo.WORK_ITEM_ACTION
WHERE WORK_ITEM_ID IN (SELECT ID FROM #TMPWI))

SELECT * FROM dbo.WORK_ITEM_ACTION
WHERE WORK_ITEM_ID = '33561382'
