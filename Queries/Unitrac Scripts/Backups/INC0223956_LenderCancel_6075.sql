...
SELECT * --INTO UniTracHDStorage..INC0223956
FROM UniTrac..WORK_ITEM
WHERE CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') = '6075'
AND WORKFLOW_DEFINITION_ID IN (2,3,6,8)
--AND STATUS_CD = 'Initial'
AND WORK_ITEM.STATUS_CD NOT LIKE 'Complete' AND WORK_ITEM.STATUS_CD NOT LIKE 'Withdrawn'
ORDER BY WORKFLOW_DEFINITION_ID ASC

SELECT * FROM UniTracHDStorage..INC0223956
ORDER BY WORKFLOW_DEFINITION_ID ASC
...
SET     STATUS_CD = 'Complete' ,
        UPDATE_USER_TX = 'INC0223956',
		 LOCK_ID = CASE WHEN LOCK_ID >= 255 THEN 1
                  ELSE LOCK_ID + 1 END
WHERE   ID IN  ( SELECT ID FROM UniTracHDStorage..INC0223956 )
...
SELECT * INTO UniTracHDStorage..LOAN_INC0223956
...
ORDER BY [ServiceName]
	...
	SELECT P.CODE_TX, P.NAME_TX, P.ProcessDefID [Process ID], --PD.NAME_TX [Process Name], 
	P.ServiceName [Process ServiceName],
	R.ProcessDefID [ReProcess ID],-- PDR.NAME_TX [Reprocess Name],
	 R.ServiceName [ReProcess ServiceName]
	FROM #process P
	INNER JOIN #reprocess R oN R.CODE_TX = P.CODE_TX
	--LEFT JOIN dbo.PROCESS_DEFINITION PD ON P.ProcessDefID = PD.ID
	--LEFT JOIN dbo.PROCESS_DEFINITION PDR ON R.ProcessDefID = PDR.ID
	WHERE P.CODE_TX = '6075'
