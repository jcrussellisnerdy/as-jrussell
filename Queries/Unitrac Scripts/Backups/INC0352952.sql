...
SELECT L.NUMBER_TX, L.EFFECTIVE_DT, RC.* 
INTO UniTracHDStorage..INC0352952_RequiredCoverageNewStatus
FROM LOAN L
INNER JOIN COLLATERAL C ON L.ID = C.LOAN_ID
INNER JOIN PROPERTY P ON C.PROPERTY_ID = P.ID
INNER JOIN REQUIRED_COVERAGE RC ON P.ID = RC.PROPERTY_ID
WHERE RC.ID IN (SELECT ID FROM UniTracHDStorage..INC0352952_RequiredCoverageStatus) 
AND RC.INSURANCE_STATUS_CD = 'N' AND CAST(L.EFFECTIVE_DT AS DATE) <= CAST(GETDATE()-90 AS DATE)
...
WHERE RC.ID IN (SELECT ID FROM UniTracHDStorage..INC0352952_RequiredCoverageNewStatus)
----- 10701
...
UPDATE_DT = GETDATE() , UPDATE_USER_TX = 'INC0352952' , 
...
-- Add Property change log
 ...
 SELECT DISTINCT 'Allied.UniTrac.RequiredCoverage' , TMP.RC_ID , 'INC0352952' , 'N' ,
 GETDATE(),  1 ,
