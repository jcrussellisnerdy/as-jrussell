...
SELECT  *
INTO UniTracHDStorage..INC0245324
FROM    UniTrac..WORK_ITEM
WHERE   ID IN ( 33351550,33353882, 33351558)


SELECT * FROM dbo.WORK_ITEM_ACTION
WHERE WORK_ITEM_ID IN (33351550,33351558,33353882)

SELECT * FROM dbo.BILLING_GROUP
WHERE ID IN (1525797,1525798,1525809)
...
UPDATE  UniTrac..WORK_ITEM
SET     STATUS_CD = 'Complete'
WHERE   ID IN ( select ID from UniTracHDStorage..INC0245324 )
        AND WORKFLOW_DEFINITION_ID = 10
        AND ACTIVE_IN = 'Y'
