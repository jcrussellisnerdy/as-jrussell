...
/****** Object:  StoredProcedure [dbo].[Report_PreEscrow]    Script Date: 11/19/2015 3:26:52 PM ******/
...
BORRINSCOMPANY_POLICY_NO, BORRINSCOMPANY_EFF_DT, BORRINSCOMPANY_EXP_DT, BORRINSCOMPANY_EXPCXL_DT, INSAGENCY_NAME_TX,INSAGENCY_PHONE_TX, OP_CANCEL_REASON_CD,
--ESCROW
ESCROW_DUE_DT,ESCROW_END_DT,ESCROW_PREMIUM_NO,ESCROW_OTHER_NO,ESCROW_FEE_NO,ESCROW_TOTAL_NO,ESCROW_PRIOR_TOTAL_NO,ESCROW_TOTAL_AMT_CHG_DT,MORT_OPTION_PRM_VARIANCE,	 
...
--ESCROW
		...
		DATEDIFF(d,GETDATE(), E.END_DT) AS [ESCROW_PAID_THROUGH_DT_NO],
		E.TYPE_CD,
		E.SUB_TYPE_CD,
...
Join ESCROW_REQUIRED_COVERAGE_RELATE ERCR ON E.ID = ERCR.ESCROW_ID AND ERCR.PURGE_DT IS NULL
Join REQUIRED_COVERAGE RC_PRE on ERCR.REQUIRED_COVERAGE_ID = RC_PRE.ID AND RC_PRE.PURGE_DT IS NULL
LEFT JOIN REQUIRED_ESCROW RE on RC_PRE.ID = RE.REQUIRED_COVERAGE_ID 
	AND RC_PRE.ESCROW_IN = 'Y' AND ISNULL(RE.ACTIVE_IN,'') = 'Y'
Join REQUIRED_COVERAGE RC on RE.REQUIRED_COVERAGE_ID = RC.ID AND RC.PURGE_DT IS NULL  
     AND ISNULL(RE.STATUS_CD,'') = 'BM'
...
L.STATUS_CD = 'A' and -- L.STATUS_CD != 'U' 
C.STATUS_CD != 'U' and
...
Select LOAN_BRANCHCODE_TX,LOAN_DIVISIONCODE_TX,LOAN_TYPE_TX,REQUIREDCOVERAGE_CODE_TX,REQUIREDCOVERAGE_TYPE_TX,LOAN_NUMBER_TX,LOAN_NUMBERSORT_TX,
...
ESCROW_PAYEE_CODE_TX,ESCROW_TOTAL_AMT_CHG_DT,MORT_OPTION_PRM_VARIANCE,LEND_PAYEE_CODE_MATCH_NO,
LAST_ESCROW_EVENT_TX,LAST_ESCROW_EVENT_DT,ESCROW_EXCEPTION,ESCROW_PAID_THROUGH_DT_NO,ESCROW_TYPE_CD,ESCROW_SUB_TYPE_CD,ESCROW_EXCESS_IN,
ESCROW_ID,LOAN_ID,COLLATERAL_ID,PROPERTY_ID,REQUIREDCOVERAGE_ID,
LOAN_STATUSCODE,LOAN_STATUSMEANING_TX,COLLATERAL_STATUSCODE,COLLATERAL_STATUSMEANING_TX,
REQUIREDCOVERAGE_STATUSCODE,REQUIREDCOVERAGE_STATUSMEANING_TX,REQUIREDCOVERAGE_SUBSTATUSCODE,
