...
SELECT CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') Lender, * --INTO UniTracHDStorage..WI_INC0214728
FROM    UniTrac..WORK_ITEM
WHERE   ID IN ( 26789140,
27917118  )


SELECT * FROM UniTracHDStorage..WI_INC0214728
...
WHERE   LENDER.CODE_TX = '6034'
		...
		AND WI.ID IN (SELECT ID FROM UniTracHDStorage..INC0214728)
		
SELECT * FROM #TMPWI

SELECT * INTO UniTracHDStorage..LOAN_INC0214728
FROM dbo.LOAN WHERE ID IN (SELECT LOAN_ID FROM #TMPWI)

SELECT * FROM UniTracHDStorage..LOAN_INC0214728


UPDATE  LN
SET     SPECIAL_HANDLING_XML = NULL ,
        UPDATE_DT = GETDATE() ,
        UPDATE_USER_TX = 'INC0214728' ,
