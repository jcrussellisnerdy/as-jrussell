...
/****** Object:  StoredProcedure [dbo].[Report_EscrowInsurance]    Script Date: 12/1/2017 3:44:45 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[Report_EscrowInsurance] 
	...
	@ReportDomainName as varchar(30) = 'Report_EscrowInsurance',
...
DECLARE @ReportConfigFilter as varchar(50)
if @ReportConfig is NULL or @ReportConfig = '' or @ReportConfig = '0000'
	Set @ReportConfigFilter = @ReportType
else
	Set @ReportConfigFilter = @ReportConfig
...
left Join 
  ...
  Join REPORT_CONFIG_ATTRIBUTE RCA on RCA.REPORT_CONFIG_ID = RC.ID
   ...
   on Custom.CODE_TX = @ReportConfigFilter and Custom.REPORT_DOMAIN_CD = RAD.DOMAIN_CD and Custom.REPORT_REF_ATTRIBUTE_CD = RAD.ATTRIBUTE_CD --and Custom.REPORT_CD = @ReportConfig
...
BORRINSCOMPANY_POLICY_NO, BORRINSCOMPANY_EFF_DT, BORRINSCOMPANY_EXP_DT, BORRINSCOMPANY_EXPCXL_DT, INSAGENCY_NAME_TX,INSAGENCY_PHONE_TX,	
--ESCROW
ESCROW_DUE_DT,ESCROW_PREMIUM_NO,ESCROW_OTHER_NO,ESCROW_FEE_NO,ESCROW_TOTAL_NO,ESCROW_PRIOR_TOTAL_NO,ESCROW_PAYEE_CODE_TX,ESCROW_TOTAL_AMT_CHG_DT,MORT_OPTION_PRM_VARIANCE,LEND_PAYEE_CODE_MATCH_NO,	 
--IDs, STATUS
...
--ESCROW
		...
		PC.ID AS [LEND_PAYEE_CODE_MATCH_NO],
--IDs, STATUS
       L.ID as [LOAN_ID], 
...
--OTHER
		...
		'' AS [OPTION_TRACK_ESCROW_TX],
		0 AS [OPTION_PREM_DUE_DAYS_TX]
...
CROSS APPLY 
(
	SELECT ERCR.REQUIRED_COVERAGE_ID, max(E.ID) as ID,MAX(E.POLICY_NUMBER_TX) as POLICY_NUMBER_TX, MAX(E.DUE_DT) as DUE_DT, max(E.END_DT) as END_DT,
	max(E.STATUS_CD) as STATUS_CD, MAX(E.PREMIUM_NO) as PREMIUM_NO, MAX(E.OTHER_NO) as OTHER_NO, MAX(E.FEE_NO) as FEE_NO,
	...
	FROM ESCROW E 
	join ESCROW_REQUIRED_COVERAGE_RELATE ERCR on E.ID = ERCR.ESCROW_ID AND ERCR.PURGE_DT IS NULL
	where E.LOAN_ID = L.ID 
	AND E.PROPERTY_ID = P.ID 
	AND E.PURGE_DT IS NULL 
	group by ERCR.REQUIRED_COVERAGE_ID
) AS E
...
Join REQUIRED_COVERAGE RC on E.REQUIRED_COVERAGE_ID = RC.ID AND RC.PURGE_DT IS NULL AND RC.ESCROW_IN = 'Y'
JOIN REQUIRED_ESCROW RE ON RE.REQUIRED_COVERAGE_ID = RC.ID AND RE.PURGE_DT IS NULL AND RE.ACTIVE_IN = 'Y'
...
OUTER APPLY
(
	SELECT DISTINCT LM.ID, LF.PAYEE_CODE_TX 
...
and 
(L.STATUS_CD IN ('A','B','N','O') OR C.STATUS_CD IN ('A','S','X','Z'))
and
(RC.STATUS_CD <> 'T')
...
[OPTION_TRACK_ESCROW_TX] [nvarchar] (2),
[OPTION_PREM_DUE_DAYS_TX] [int]
)
INSERT INTO @RC2(ID,OPTION_TRACK_ESCROW_TX,OPTION_PREM_DUE_DAYS_TX)
SELECT RC.ID,
...
WHERE ID IN(SELECT DISTINCT REQUIREDCOVERAGE_ID FROM #tmptable)
UPDATE #tmptable 
SET #tmptable.[OPTION_TRACK_ESCROW_TX] = RC2.OPTION_TRACK_ESCROW_TX, #tmptable.[OPTION_PREM_DUE_DAYS_TX] = RC2.OPTION_PREM_DUE_DAYS_TX
FROM @RC2 RC2
JOIN #tmptable TT ON RC2.ID = TT.REQUIREDCOVERAGE_ID
...
INSERT INTO #tmpTable SELECT * FROM dbo.#T2 WHERE 
	(@ESCINSCXL = 'N' OR (@ESCINSCXL = 'Y' AND [REQUIREDCOVERAGE_INSSTATUSCODE] in ('P', 'F') AND [REQUIREDCOVERAGE_INSSUBSTATUSCODE] NOT IN  ('C','P') 
			AND [OPTION_TRACK_ESCROW_TX] = '1' AND [OWNER_POLICY_CANCEL_DT] < DATEADD(d, 60, GETDATE()) AND [OWNER_POLICY_CANCEL_DT] >= GETDATE()))
	AND (@ESCINSEXP = 'N' OR (@ESCINSEXP = 'Y' AND [REQUIREDCOVERAGE_INSSTATUSCODE] in ('P', 'F') AND [REQUIREDCOVERAGE_INSSUBSTATUSCODE] NOT IN  ('C','P') 
			AND [OPTION_TRACK_ESCROW_TX] = '1' AND [OWNER_POLICY_EXP_DT] < DATEADD(d, [OPTION_PREM_DUE_DAYS_TX], GETDATE()) AND [OWNER_POLICY_EXP_DT] >= GETDATE()))
	AND (@ESCINSEXP3060 = 'N' OR (@ESCINSEXP3060 = 'Y' AND [REQUIREDCOVERAGE_INSSTATUSCODE] in ('P', 'F') AND [REQUIREDCOVERAGE_INSSUBSTATUSCODE] NOT IN  ('C','P') 
			AND [OPTION_TRACK_ESCROW_TX] = '1' AND [OWNER_POLICY_EXP_DT] < DATEADD(d, 60, GETDATE()) AND [OWNER_POLICY_EXP_DT] >= GETDATE()))
	AND (@ESCUNINS = 'N' OR (@ESCUNINS = 'Y' AND [REQUIREDCOVERAGE_INSSTATUSCODE] in ('N','A','E','EH','C','CH') OR [REQUIREDCOVERAGE_INSSUBSTATUSCODE] IN  ('B','I') 
			AND [OPTION_TRACK_ESCROW_TX] = '1'))
