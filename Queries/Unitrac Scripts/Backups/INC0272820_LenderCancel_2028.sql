...
----REPLACE 2028 WITH THE THE Lender ID
----REPLACE INC0272820 WITH THE TICKET ID
---Checking UTLs WIs
USE UniTrac

--SELECT * FROM dbo.LENDER WHERE CODE_TX IN ('2028')
...
--INTO UniTracHDStorage..INC0272820
FROM    WORK_ITEM
WHERE   RELATE_TYPE_CD = 'Allied.UniTrac.UTLMatchResult'
        ...
        WHERE   LENDER.CODE_TX = '2028' AND WORK_ITEM.STATUS_CD NOT IN ('Complete', 'Withdrawn')
			   )
...
WHERE LENDER_ID = '168'
AND WORKFLOW_DEFINITION_ID IN (2,3,6,8) AND CONTENT_XML.value('(/Content/Collateral/Type)[1]', 'varchar (50)') = 'RE'
...
SET     STATUS_CD = 'Complete' ,
        UPDATE_USER_TX = 'INC0272820',
		 LOCK_ID = CASE WHEN LOCK_ID >= 255 THEN 1
                  ELSE LOCK_ID + 1 END
WHERE   ID IN  (  SELECT ID FROM UniTracHDStorage..INC0272820)
...
WHERE   LENDER.CODE_TX = '2028'
        AND WI.ID IN ( SELECT ID FROM UniTracHDStorage..INC0272820)

DROP TABLE #TMPWI

SELECT * INTO UniTracHDStorage..LOAN_INC0272820
