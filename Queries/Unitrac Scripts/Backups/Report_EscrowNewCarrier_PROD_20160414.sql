...
/****** Object:  StoredProcedure [dbo].[Report_EscrowNewCarrier]    Script Date: 4/14/2016 8:08:03 PM ******/
...
(
 ...
 --BORROWER INSURANCE
     ...
     [STRIPPED_BORRINSCOMPANY_NAME_TX] = STRIPPED_BORRINSCOMPANY.NAME_TX,
 ...
 --Previous BORROWER INSURANCE
     ...
     [STRIPPEDPREV_BORRINSCOMPANY_NAME_TX] = STRIPPEDPREV_BORRINSCOMPANY.NAME_TX,
 ...
 */
 ...
 ,PRIOR_RC.SUMMARY_SUB_STATUS_CD
  From REQ_COV_CTE As PRIOR_RC WITH(NOLOCK)
  Join dbo.ESCROW_REQUIRED_COVERAGE_RELATE As ERCR WITH(NOLOCK)
    On PRIOR_RC.ID = ERCR.REQUIRED_COVERAGE_ID
  Join dbo.ESCROW As ESP WITH(NOLOCK)
...
,IsNull(ES.END_DT, DATEADD(month, 12 , ES.DUE_DT)) Desc) As CLOSEDESC
 ...
 CROSS APPLY (SELECT
	 POLICY_NO = Substring(IsNull(
		 --CASE When @Debug>0 THEN
		  ...
		  dbo.NormalizeAlphaNum(ES.POLICY_NUMBER_TX,'Y',0)
		 --END
		 , ''), 1, 6)
	,NAME_TX = UPPER(Substring(RTrim(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Lower(BIC.NAME),'-',''),'.',''),' ins ',' '),'insurance',''),' comp ',' '),' co ',' '),'company','')),1,7))
	 ) AS [STRIPPED_BORRINSCOMPANY]
 CROSS APPLY (SELECT
	 POLICY_NO = Substring(IsNull(
		 --CASE When @Debug>0 THEN
		  --PES.POLICY_NUMBER_TX Else
		  dbo.NormalizeAlphaNum(PES.POLICY_NUMBER_TX,'Y',0)
		 --END
		 , ''), 1, 6)
	,NAME_TX = UPPER(Substring(RTrim(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Lower(BIC.NAME),'-',''),'.',''),' ins ',' '),'insurance',''),' comp ',' '),' co ',' '),'company','')),1,7))
 ...
 AND 1 = (CASE 
  ...
  WHEN NOT(Coalesce(NullIf(BIC.NAME, ''), NullIf(BICP.NAME, '')) IS NULL OR RTrim(UPPER(STRIPPED_BORRINSCOMPANY.NAME_TX)) = RTrim(UPPER(STRIPPEDPREV_BORRINSCOMPANY.NAME_TX))) THEN 1
  ELSE 1 END)
...
From LENDER_CTE LND WITH(NOLOCK)
 ...
 CROSS APPLY (SELECT
	 POLICY_NO = Substring(IsNull(
		 --CASE When @Debug>0 THEN
		  ...
		  dbo.NormalizeAlphaNum(OP.POLICY_NUMBER_TX,'Y',0)
		 --END
		 , ''), 1, 6)
	,NAME_TX = UPPER(Substring(RTrim(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Lower(OP.BIC_NAME_TX),'-',' '),'.',''),' ins ',' '),'insurance',''),' comp ',' '),' co ',' '),'company','')),1,7))
 ) AS [STRIPPED_BORRINSCOMPANY]
 CROSS APPLY (SELECT
	 POLICY_NO = Substring(IsNull(
		 --CASE When @Debug>0 THEN
		  --POP.POLICY_NUMBER_TX Else
		  dbo.NormalizeAlphaNum(POP.POLICY_NUMBER_TX,'Y',0)
		 --END
		 , ''), 1, 6)
	,NAME_TX = UPPER(Substring(RTrim(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Lower(POP.BIC_NAME_TX),'-',' '),'.',''),' ins ',' '),'insurance',''),' comp ',' '),' co ',' '),'company','')),1,7))
 ) AS [STRIPPEDPREV_BORRINSCOMPANY]
 ...
 (
	 ...
	 AND 
	 ( 	   
	   STRIPPED_BORRINSCOMPANY.POLICY_NO =  SUBSTRING(ISNULL(dbo.NormalizeAlphaNum(ES.POLICY_NUMBER_TX,'Y',0), ''), 1, 6)
	 )
 ) ESCR
 ...
 AND 1 = (CASE 
  ...
  WHEN NOT(Coalesce(NullIf(OP.BIC_NAME_TX, ''), NullIf(POP.BIC_NAME_TX, '')) IS NULL OR RTrim(UPPER(STRIPPED_BORRINSCOMPANY.NAME_TX)) = RTrim(UPPER(STRIPPEDPREV_BORRINSCOMPANY.NAME_TX))) THEN 1
  ELSE 1 END)
...
*/
 ...
 COVERAGE_STATUS_TX, 
 --BORROWER INSURANCE
 BORRINSCOMPANY_NAME_TX, STRIPPED_BORRINSCOMPANY_NAME_TX, BORRINSCOMPANY_POLICY_NO, STRIPPED_BORRINSCOMPANY_POLICY_NO, BORRINSCOMPANY_EFF_DT, BORRINSCOMPANY_EXP_DT, 
 --ESCROW
