...
SELECT  WI.*
INTO UniTracHDStorage..INC0280585
FROM dbo.WORK_ITEM WI
JOIN dbo.LENDER L ON L.ID = WI.LENDER_ID
WHERE WI.ID IN ( 36804529 , 36804530)


select * from process_log_item
where process_log_ID iN (41361425,
41361429) and relate_type_cd = 'Allied.UniTrac.Notice'

select *
INTO UniTracHDStorage..INC0280585_Notice
 from notice
where ID IN (21017268,
21017269)


update N
set purge_dt= getdate(), lock_id = lock_id+1, update_dt = getdate(), update_user_tx = 'INC0280585'
--select *
 from notice N
where ID IN (21017268,
21017269)
...
SET     STATUS_CD = 'Complete' ,
        LOCK_ID = LOCK_ID + 1 ,
        UPDATE_DT = GETDATE() ,
        UPDATE_USER_TX = 'INC0280585'
WHERE   ID IN ( SELECT ID FROM UniTracHDStorage..INC0280585 )
