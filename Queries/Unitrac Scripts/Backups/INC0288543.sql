...
FROM    UniTrac..WORK_ITEM
WHERE   ID IN (37589578   )


--Create a temp table for notices and certs 
select RELATE_ID INTO #tmpN from process_log_item PLI
where PLI.process_log_ID iN (SELECT * FROM #tmp) and relate_type_cd IN ('Allied.UniTrac.Notice')

select RELATE_ID INTO #tmpFPC from process_log_item PLI
where PLI.process_log_ID iN (SELECT * FROM #tmp) and relate_type_cd IN ('Allied.UniTrac.ForcePlacedCertificate')
...
/*
select *
INTO UniTracHDStorage..INC0288543_Notice
 from notice
WHERE ID IN (SELECT * FROM #tmpN) AND PDF_GENERATE_CD = 'NT'
...
update N
set PDF_GENERATE_CD = 'PEND', lock_id = lock_id+1, update_dt = getdate(), update_user_tx = 'INC0288543'
--select *
 from notice N
WHERE ID IN (SELECT * FROM #tmpN) AND PDF_GENERATE_CD = 'NT'
