...
SELECT CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') Lender, * --INTO UniTracHDStorage..INC0214578
FROM    UniTrac..WORK_ITEM
WHERE   ID IN ( 28279711   )
...
WHERE   LENDER.CODE_TX = '4220'
		...
		AND WI.ID IN (SELECT ID FROM UniTracHDStorage..INC0214578)
