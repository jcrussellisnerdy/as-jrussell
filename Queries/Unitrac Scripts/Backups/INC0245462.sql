...
SELECT L.STATUS_CD [Loan_Status], CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') Lender, WI.*  INTO UniTracHDStorage..WI_INC0245462
FROM    UniTrac..WORK_ITEM WI 
INNER JOIN dbo.LOAN L ON L.ID = WI.RELATE_ID AND WI.RELATE_TYPE_CD = 'Allied.UniTrac.Loan'
WHERE   WI.ID IN ( 33045046 )
...
WHERE   LENDER.CODE_TX = '6276'
		...
		AND WI.ID IN (XXXXXXX)
		
SELECT * FROM #TMPWI

SELECT * INTO UniTracHDStorage..LOAN_INC0245462
