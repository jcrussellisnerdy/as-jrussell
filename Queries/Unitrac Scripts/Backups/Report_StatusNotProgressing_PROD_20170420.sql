...
/****** Object:  StoredProcedure [dbo].[Report_StatusNotProgressing]    Script Date: 4/20/2017 8:22:50 AM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[Report_StatusNotProgressing] 
--declare
...
--
	...
	Where P.PURGE_DT Is Null
	  ...
	  And L.STATUS_CD NOT IN ('U','I')  --Unmatched, Inactive
	  And IsNull(C.STATUS_CD,'') IN ('S','A') -- Only  Active & CPI Suspend Status - Suspend isnt often used but sometimes selectd by accident
	  AND CC.CODE_TX != 'VEH-A' -- Do not included A Paper loans
