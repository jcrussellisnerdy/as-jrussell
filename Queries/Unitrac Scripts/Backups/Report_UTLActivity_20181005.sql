...
/****** Object:  StoredProcedure [dbo].[Report_UTLActivity]    Script Date: 10/5/2018 10:17:36 AM ******/
SET ANSI_NULLS ON
GO
...
--debug/test:
  ...
  select @lendercode=2771
  select @debug=@LenderCode
  --select @workitemtype=coalesce(@workitemtype, 0)
  select @workitemtype='2' --I.B.C.
  --select @workitemtype='6' --Verif.Event
  select @fromdate='5/1/2018'
  select @todate='5/31/2018'
    SELECT @Division = '3'
*/
...
begin
		...
		select isnull(@joins,'@joins IS NULL')
		select isnull(@conditions, '@conditions IS NULL') 
		SELECT @coverage coverage, @finalquery
end
...
*/
	...
	BEGIN
		SELECT @finalquery = N';select
		  Interaction_History_ID = IH_ID
		 ,Insurance_Company_Name = IH.SPECIAL_HANDLING_XML.value(''(/SH[1]/InsuranceCompanyName)[1]'', ''nvarchar(100)'')
		 ,Caller_ID = IH.SPECIAL_HANDLING_XML.value(''(/SH[1]/CallID)[1]'', ''nvarchar(30)'')
		 ,FQ.*
