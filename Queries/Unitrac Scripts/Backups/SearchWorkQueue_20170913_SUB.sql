...
/****** Object:  StoredProcedure [dbo].[SearchWorkQueue]    Script Date: 9/20/2017 9:48:22 AM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	begin
		...
		SELECT  
			...
			CONVERT(VARCHAR(20),isnull(wia.CREATE_DT, '1/1/1900'), 113) AS 'DateOfLastAction',
...
--	UNION
		...
		FROM 
			...
			left outer join USERS u ON wi.CURRENT_OWNER_ID = u.ID
			left outer join REF_CODE rc ON (rc.DOMAIN_CD = 'ExtractWorkItemStatus' and rc.CODE_CD = wi.STATUS_CD)
			left outer JOIN LENDER LDR ON LDR.ID = @lenderId or LDR.ID = wi.LENDER_ID
		WHERE 
			wi.PURGE_DT is NULL
			and (@lenderId is null or wi.LENDER_ID = @lenderId)
			and (@queueId is null or wq.ID = @queueId)
			and (@workflowId is null or wd.ID = @workflowId)
			and (@userId is null or u.ID = @userId)
			and (@statusCd is null or rc.CODE_CD = @statusCd)
