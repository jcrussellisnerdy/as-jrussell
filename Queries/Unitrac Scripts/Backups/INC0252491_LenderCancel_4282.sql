...
----REPLACE 4282 WITH THE THE Lender ID
----REPLACE INC0252491 WITH THE TICKET ID
---Checking UTLs WIs
USE UniTrac
...
--INTO UniTracHDStorage..INC0252491
FROM    WORK_ITEM
WHERE   RELATE_TYPE_CD = 'Allied.UniTrac.UTLMatchResult'
        ...
        WHERE   LENDER.CODE_TX = '4282' AND WORK_ITEM.STATUS_CD NOT IN ('Complete', 'Withdrawn')
			   )
...
WHERE CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') = '4282'
...
SET     STATUS_CD = 'Complete' ,
        UPDATE_USER_TX = 'INC0252491',
		 LOCK_ID = CASE WHEN LOCK_ID >= 255 THEN 1
                  ELSE LOCK_ID + 1 END
WHERE   ID IN  (  SELECT ID FROM UniTracHDStorage..INC0252491)
...
SELECT * INTO UniTracHDStorage..LOAN_INC0252491
