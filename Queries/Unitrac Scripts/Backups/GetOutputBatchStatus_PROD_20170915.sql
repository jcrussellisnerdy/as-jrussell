...
/****** Object:  StoredProcedure [dbo].[GetOutputBatchStatus]    Script Date: 9/15/2017 5:27:19 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN
		...
		OC.TYPE_CD, 
		OB.SUB_TYPE_CD, 
		OB.OUTSOURCER_STATUS_CD		,
		PLI.RELATE_ID as PLI_RELATE_ID,
		pli.INFO_XML AS PLI_INFO_XML
		INTO #tmp
			FROM
			OUTPUT_BATCH OB
			...
			INNER loop JOIN PROCESS_DEFINITION PD ON PL.PROCESS_DEFINITION_ID=PD.ID  AND PD.PROCESS_TYPE_CD = ''CYCLEPRC''
			INNER  JOIN PROCESS_LOG_ITEM PLI ON PL.ID=PLI.PROCESS_LOG_ID AND PLI.RELATE_TYPE_CD=''Allied.UniTrac.Notice''
	...
	BEGIN
	   ...
	   ' SELECT
			...
			PD.NAME_TX AS [PROCESS_DEFINITION_NAME],
			OC.TYPE_CD,
			OB.SUB_TYPE_CD,
			OB.OUTSOURCER_STATUS_CD,
			FPC.ID AS FPC_ID	
   ...
   set @QUERY = @QUERY +
			...
			' SELECT 
						...
						MIN(t.CREATE_DT) AS [BATCH_DATE],
			MIN(t.STATUS_CD) AS [BATCH_STATUS],
			t.OB_ID AS [BATCH_ID],
			MIN(t.OUTPUT_TYPE_CD) AS [OUTPUT_TYPE_CD],
			MIN(t.EXTERNAL_ID) AS [EXTERNAL_BATCH_ID],
			MIN(t.lender_name_tx) AS [LENDER_NAME],
			MIN(t.PD_name_tx) AS [PROCESS_DEFINITION_NAME],
			MIN(t.TYPE_CD) AS [BATCH_TYPE],
			MIN(t.SUB_TYPE_CD) AS [BATCH_SUB_TYPE],
			MIN(t.OUTSOURCER_STATUS_CD) AS [BATCH_FULFILLMENT_STATUS],
			DATEDIFF(DAY,MIN(t.CREATE_DT), GETDATE()) AS [DAYS_SINCE_CYCLE],
			MAX( DC.PRINT_STATUS_ASSIGNED_DT ) AS [LAST_SUBMITTED_DATE],
			MAX( CASE t.OUTPUT_TYPE_CD WHEN ''IH'' THEN DC.PRINTED_DT ELSE OBL.CREATE_DT END) AS [COMPLETED_DATE],
			...
			CASE WHEN DATEDIFF(DAY,MIN(t.CREATE_DT), GETDATE()) > 2 AND MIN(t.OUTSOURCER_STATUS_CD) IS NULL THEN 1 ELSE 0 END [IS_RED]
			FROM #tmp t			
			INNER loop JOIN NOTICE N ON T.PLI_RELATE_ID=N.ID 
			LEFT OUTER JOIN DOCUMENT_CONTAINER DC ON N.ID=DC.RELATE_ID AND DC.RELATE_CLASS_NAME_TX=''Allied.UniTrac.Notice''
			LEFT OUTER JOIN OUTPUT_BATCH_LOG OBL ON OBL.OUTPUT_BATCH_ID = t.OB_ID AND OBL.LOG_TXN_TYPE_CD=''Consolidate''
			WHERE 
				DC.PURGE_DT IS NULL AND
			N.PURGE_DT IS NULL AND			
			(t.pli_INFO_XML.exist (''(/INFO_LOG/OUTPUT_TYPE)[1]'') = 0 OR t.pli_INFO_XML.value (''(/INFO_LOG/OUTPUT_TYPE)[1]'',''nvarchar(10)'') IN ('''',''IH'',''OS''))
			GROUP BY OB_ID 
		...
		OUTPUT_BATCH OB
			...
			+
			/* Added this + because oddly some of the query was being truncated here */
			 '
			INNER loop JOIN PROCESS_DEFINITION PD ON PL.PROCESS_DEFINITION_ID=PD.ID
	...
	BEGIN
		...
		SELECT
			...
			LEFT OUTER JOIN DOCUMENT_CONTAINER DC ON t.FPC_ID=DC.RELATE_ID AND DC.RELATE_CLASS_NAME_TX=''Allied.UniTrac.ForcePlacedCertificate''
			LEFT OUTER JOIN OUTPUT_BATCH_LOG OBL ON OBL.OUTPUT_BATCH_ID = t.OB_ID AND OBL.LOG_TXN_TYPE_CD=''Consolidate''
