...
SELECT WI.CONTENT_XML.value('(/Content/Lender/Name)[1]', 'varchar (50)') Lender, WI.*
--INTO UniTracHDStorage..INC0224229
FROM    UniTrac..WORK_ITEM WI 
INNER JOIN LOAN L ON L.ID = WI.RELATE_ID AND RELATE_TYPE_CD = 'Allied.UniTrac.Loan'
WHERE   WI.CONTENT_XML.value('(/Content/Lender/Code)[1]', 'varchar (50)') = '1854' 
AND WI.STATUS_CD NOT IN ('Complete' ,'Withdrawn' ,'ImportCompleted')
AND WI.WORKFLOW_DEFINITION_ID = '8'


SELECT * FROM dbo.LOAN
WHERE SPECIAL_HANDLING_XML IS NOT NULL 
--AND LENDER_ID = '138' AND
 UPDATE_USER_TX = 'INC0224229'



SELECT CONTENT_XML.value('(/Content/Lender/Name)[1]', 'varchar (50)') Lender,
CONTENT_XML.value('(/Content/Collateral/Type)[1]', 'varchar (50)') Lender,
* FROM UniTracHDStorage..INC0224229
...
WHERE   LENDER.CODE_TX = '1854'
		...
		AND WI.ID IN (SELECT ID FROM UniTracHDStorage..INC0224229)
		
SELECT * FROM #TMPWI

SELECT * INTO UniTracHDStorage..LOAN_INC0224229
