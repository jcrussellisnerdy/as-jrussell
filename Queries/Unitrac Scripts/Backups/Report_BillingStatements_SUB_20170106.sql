...
/****** Object:  StoredProcedure [dbo].[Report_BillingStatements]    Script Date: 1/6/2017 11:32:31 AM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
   ...
   DECLARE @monthlyPeriods AS TABLE  (FPC_ID BIGINT, MTHN_R_TOT_PRM DECIMAL(8,2), MTHN_C_TOT_PRM DECIMAL(8,2), 
         ...
         MTHN_P_TOT_PRM DECIMAL(8,2) DEFAULT 0.00, USE_OVERAGES int)
   ...
   BEGIN
      ...
      -- Calculate the Period Total (Receivables and Cancels)
      SELECT   t.*, 
               MTHN_R_TOT_PRM - MTHN_C_TOT_PRM as PRD_TOTAL_AMT_NO, 
               MONTH_FINANCIAL.PRD_START_DT, 
               MONTH_FINANCIAL.PRD_END_DT,
               MONTH_FINANCIAL.TERM_NO as TERM_NO,
               MONTH_FINANCIAL.MTHN_P_TOT_PRM as PRD_PAYMENT_TOT -- NOTE: RW 9/9/2016 SAVE THE NEW PAYMENT FOR THE TERM
               INTO #t6
      FROM     #t5 t
      ...
      -- question it for the month period in question is 0, then there are no charges at this time
      ...
      UPDATE   #t6
      SET      CPI_BILLED_NET_TOTAL_NO = CASE WHEN PRD_TOTAL_AMT_NO - PRD_PAYMENT_TOT >= 0 THEN PRD_TOTAL_AMT_NO - PRD_PAYMENT_TOT ELSE 0 END,
   ...
   BEGIN
      ...
      INSERT INTO @monthlyPeriods EXEC RetrieveMonthlyBillingPeriods @fpcIds, 1, 1, 1

      -- Build a second temporary table to hold the full original table, but also the 
      -- calculation of what the month amounts will be (the split amounts) ... this 
      -- handles the possibility that there may be more than one transaction PER month
      SELECT   t.*, mf.MTHN_R_TOT_PRM, mf.MTHN_C_TOT_PRM, mf.MTHN_PC_TOT_PRM, mf.PRD_START_DT, mf.PRD_END_DT, mf.TERM_NO, mf.USE_OVERAGES
               INTO #t7
      FROM     #t5 t
      ...
      --         - The CPI_ISSUED_PREMIUM_AMOUNT_NO will be equal to the MTHN_C_TOT_PRM
      --         - The CPI_CANCELLEDPREMIUM_AMOUNT_NO will not be changed
      --         - The CPI_CANCEL_TYPE_CD will not be changed
      
      --       * For an overage scenario:
      --         - The CPI_ISSUED_PREMIUM_AMOUNT_NO should be the CHARGE for the month 
      --           (because the OVERAGE is in the CANCEL column) it will be MTHN_R_TOT_PRM
      --         - The CPI_CANCELLEDPREMIUM_AMOUNT_NO should be the OVERAGE which is in the MTHN_C_TOT_PRM column (made negative)
      --         - The CPI_CANCEL_TYPE_CD should be set to "O" for overage
      --         - The CPI_BILLED_NET_TOTAL_NO should also be set to the OVERAGE in the MTHN_C_TOT_PRM column so it displays on the report output (made negative)
      
      UPDATE   #t7
      SET      I_TOT_PRM = MTHN_R_TOT_PRM,
               C_TOT_PRM = MTHN_C_TOT_PRM,
               PC_TOT_PRM = MTHN_PC_TOT_PRM,
		         CPI_CANCEL_TYPE_CD = case when USE_OVERAGES = 1 and MTHN_C_TOT_PRM > 0 and MTHN_PC_TOT_PRM = 0 THEN 'O' ELSE CPI_CANCEL_TYPE_CD END,	
               CPI_ISSUEDPREMIUM_AMOUNT_NO = case when USE_OVERAGES = 1 and MTHN_C_TOT_PRM > 0 and MTHN_PC_TOT_PRM = 0 THEN 1 * MTHN_R_TOT_PRM ELSE MTHN_C_TOT_PRM END,
               CPI_CANCELLEDPREMIUM_AMOUNT_NO = case when USE_OVERAGES = 1 and MTHN_C_TOT_PRM > 0 and MTHN_PC_TOT_PRM = 0 THEN -MTHN_C_TOT_PRM ELSE CPI_CANCELLEDPREMIUM_AMOUNT_NO END,
               CPI_BILLED_NET_TOTAL_NO = case when USE_OVERAGES = 1 and MTHN_C_TOT_PRM > 0 and MTHN_PC_TOT_PRM = 0 THEN -MTHN_C_TOT_PRM else -MTHN_PC_TOT_PRM END,
               INSCOMPANY_EFF_DT = PRD_START_DT,
               INSCOMPANY_CAN_DT = PRD_END_DT,
               INSCOMPANY_EXP_DT = PRD_END_DT,
               INSCOMPANY_EXPCXL_DT = PRD_END_DT,
      ...
      ALTER TABLE #t7 DROP COLUMN MTHN_R_TOT_PRM, MTHN_C_TOT_PRM, MTHN_PC_TOT_PRM, PRD_START_DT, PRD_END_DT, USES_CUSTOM_MONTHLY_BILLING_CD, TERM_NO, USE_OVERAGES
