...
IF NOT EXISTS (SELECT *
               FROM   sys.objects
               WHERE  object_id = Object_id(N'[dbo].[TempProc]')
                      AND type IN ( N'P', N'PC' ))
  BEGIN
      /* Create Empty Stored Procedure */
      EXEC dbo.Sp_executesql
        @statement = N'CREATE PROCEDURE [dbo].[TempProc] AS RETURN 0;';
  END;
...
ALTER PROCEDURE [dbo].[Tempproc] (@DryRun     BIT= 1,
                                  @Debug      BIT= 0,
                                  @Verbose    BIT= 0,
...
AS
  BEGIN
      ...
      EXEC [dbo].[TempProc] @debug= 1, @verbose=1
      EXEC [dbo].[TempProc] @debug= 1, @verbose=1, @DryRun = 0
              ...
              BEGIN
                ...
                ADD FILE (
                               ...
                               NAME = tempdev'
                                    ...
                                    + ',
                               SIZE = '
                                    + @AmountPerDataFileGB ---need to populate the amount from query above
                                    + 'MB,
                               FILEGROWTH = '
