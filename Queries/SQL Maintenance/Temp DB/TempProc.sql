...
IF NOT EXISTS (SELECT *
               FROM   sys.objects
               WHERE  object_id = Object_id(N'[dbo].[AStoredProcName]')
                      AND type IN ( N'P', N'PC' ))
  BEGIN
      /* Create Empty Stored Procedure */
      EXEC dbo.Sp_executesql
        @statement = N'CREATE PROCEDURE [dbo].[AStoredProcName] AS RETURN 0;';
  END;

GO

/* Alter Stored Procedure */
ALTER PROCEDURE [dbo].[Astoredprocname](@Percentage NVARCHAR(5) =NULL,
                                        @FileGrowth INT =NULL,
                                        @Dryrun     BIT = 1,
                                        @Verbose    BIT = 1,
                                        @Force      BIT = 1)
AS
  BEGIN
      ...
      exec  [DBA].[dbo].[AStoredProcName] --Shows data on the system and code that would be done 
      exec  [DBA].[dbo].[AStoredProcName] @Verbose= 0 --Pulls table of the all the data about the temp DB
      exec  [DBA].[dbo].[AStoredProcName] @Percentage= '0.95' ---What ever the percentage entered in it will cut that part of out the drive and use the remaining then subtract the log file from that and divide the rest up for the datafiles
      exec  [DBA].[dbo].[AStoredProcName] @FileGrowth= '262144' ---What ever the percentage entered in it will place as the file and log grow file. 
      exec  [DBA].[dbo].[AStoredProcName] @Dryrun = 0 ---Does the work
      */
      SET ANSI_NULLS ON;
      SET QUOTED_IDENTIFIER ON;
