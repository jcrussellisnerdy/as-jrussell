...
DECLARE @sqlcmd        VARCHAR(max),
        ...
        @DatabaseName  SYSNAME = '?', ---Specific Database or if you want all type ? 
        @type          NVARCHAR(50) ='full', --LOG or FULL Backup
        @WhatIf        INT = 1 --1 preview / 0 executes it 
...
IF EXISTS (SELECT ServerLocation
           FROM   DBA.info.Instance
           WHERE  ServerLocation = 'ON-PREM')
  BEGIN
      PRINT 'DO SOMETHING: THIS IS A ON-PREM INSTANCE AND USE DDBOOST'
  END
ELSE IF EXISTS (SELECT ServerLocation
           FROM   DBA.info.Instance
           WHERE  ServerLocation = 'AWS-RDS')
  BEGIN
      PRINT 'DO NOTHING: THIS IS A AWS-RDS INSTANCE'
  END
ELSE IF EXISTS (SELECT ServerLocation
           FROM   DBA.info.Instance
           WHERE  ServerLocation = 'MS-AZURE')
  BEGIN
      PRINT 'DO NOTHING: THIS IS A MS-AZURE INSTANCE'
  END
ELSE IF EXISTS (SELECT ServerLocation
           FROM   DBA.info.Instance
           WHERE  ServerLocation = 'AWS-EC2')
  BEGIN
      IF @DatabaseName = '?'
        BEGIN
