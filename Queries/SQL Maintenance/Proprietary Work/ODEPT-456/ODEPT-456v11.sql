            ...
            CASE --1
               ...
               ELSE
                 ...
                 THEN
                     ...
                     END --4
					 ELSE 
					 CASE WHEN  (select count(*) from dba.info.instance where sqlservername LIKE ('DBA%')) = 1 --5
					 THEN 
					    CASE
                       WHEN ( 0.205 * TotalGB - FreeGB ) <= '10' --4
                     THEN Cast(Cast((0.410 * TotalGB - FreeGB) AS DECIMAL(5, 0)) AS VARCHAR(100))
                 ...
                 END --2
				 END --5
             END AS [SpaceIncreasedToGetBackto20Percent], --1
			 CASE WHEN PATH IN (SELECT DISTINCT concat(vs.volume_mount_point, logical_volume_name) FROM sys.master_files AS f WITH (NOLOCK) CROSS APPLY sys.dm_os_volume_stats(f.database_id, f.[file_id]) AS vs) THEN 'I got your datafiles!'
			 ELSE 'No datafiles on drive'
			 END  AS [EndOfTheRoad]
             FROM   #DriveUsage) AS new ( [Path], [TotalGB], [FreeGB], [FreePct], [SpaceIncreasedToGetBackto20Percent], [EndOfTheRoad] )
...
ELSE
  BEGIN
      SELECT @HostName [ServerName], [Path],
             [TotalGB],
             [FreeGB],
             [FreePct],
             CASE --1
               ...
               ELSE
                 ...
                 THEN
                     CASE
                       WHEN ( 0.205 * TotalGB - FreeGB ) <= '10' AND @units ='GB' --4
