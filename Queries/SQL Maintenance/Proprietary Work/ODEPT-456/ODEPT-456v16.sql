...
in the stored procedure are @dryrun, @units and @DriveLetter
...
/*
Pretty standard if the @dryrun = 1 it displays the information. If it is 0 stored to a table named dbo.DriveUsage
*/
...
IF( @dryRun = 0 )
  BEGIN
      ...
      USING (SELECT @HostName [Hostname],
                    ...
                    [FreePct],
                    CASE
                      WHEN [FreePct] > 20 THEN 'Resource already over 20%' /* Checks to see if the drive is over 20% */
                      ELSE
                        CASE
