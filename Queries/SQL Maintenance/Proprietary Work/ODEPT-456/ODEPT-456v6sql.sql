...
IF( @units = 'MB' )
  BEGIN
      ...
      SET @sql = 'select rtrim(ltrim(SUBSTRING(line,1,CHARINDEX(''|'',line) -1))) as drivename
			...
			(.2 *(CAST((CONVERT(DECIMAL(18,2),cast(rtrim(ltrim(SUBSTRING(line,CHARINDEX(''|'',line)+1,
...
--	,CASE WHEN (CAST((1.0 - vs.available_bytes * 1.0 / vs.total_bytes) * 100.0 AS DECIMAL(10,2))) > 20 THEN ''Resource already has 20%'' ELSE  CASE WHEN  (CAST((1.0 - vs.available_bytes * 1.0 / vs.total_bytes) * 100.0 AS DECIMAL(10,2))) < 19 THEN CAST (.2 *(CAST((CAST(vs.total_bytes / 1073741824.0 AS DECIMAL(10,2))) as DECIMAL(18,0)))  - (CAST( (CAST(vs.available_bytes / 1073741824.0 AS DECIMAL(10,2))) as DECIMAL(18,0)) ) AS nvarchar(100)) + '' GB'' ELSE  CAST(.2 *(CAST((CAST(vs.total_bytes / 1073741824.0 AS DECIMAL(10,2)))*1024 as DECIMAL(18,0)))  - (CAST( (CAST(vs.available_bytes / 1073741824.0 AS DECIMAL(10,2)))*1024 as DECIMAL(18,0)) ) AS nvarchar(100)) + '' MB'' END END AS  [SpaceIncreased]
...
ELSE
  ...
  ----fix math 
	        SELECT [Path],
             ...
             [FreeGB],
             [FreePct],
				CASE WHEN [FreePct] > 20 THEN 'Resource already has 20%'
			ELSE  CASE WHEN [FreePct] < 19 THEN 
			CAST(ABS(.2 *(CAST(TotalGB as DECIMAL(18,0))) -( (CAST(TotalGB as DECIMAL(18,0)))  - (CAST( FreeGB as DECIMAL(18,0))) )) AS nvarchar(100)) + ' GB'
			ELSE 
			CASE WHEN  (CAST(ABS(.2 *(CAST(TotalGB as DECIMAL(18,0))) -( (CAST(TotalGB as DECIMAL(18,0)))  - (CAST( FreeGB as DECIMAL(18,0))) )) AS nvarchar(100))) > '1024'
			THEN CAST(ABS(.2 *(CAST(TotalGB as DECIMAL(18,0))) -( (CAST(TotalGB as DECIMAL(18,0)))  - (CAST( FreeGB as DECIMAL(18,0))) )) AS nvarchar(100))  + ' MB'
			ELSE 
			CAST(ABS(.2 *(CAST(TotalGB*1024 as DECIMAL(18,0))) -( (CAST(TotalGB*1024 as DECIMAL(18,0)))  - (CAST( FreeGB*1024 as DECIMAL(18,0))) )) AS nvarchar(100)) + ' MB'
			END 
			END
