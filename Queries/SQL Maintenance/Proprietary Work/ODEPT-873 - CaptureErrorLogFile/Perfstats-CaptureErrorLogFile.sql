...
DECLARE @JobName        VARCHAR(200) = 'Perfstats-CaptureErrorLogFile',
...
IF EXISTS (SELECT rs.role_desc
 ...
 BEGIN
  ...
  END
ELSE IF NOT EXISTS (SELECT rs.role_desc
           FROM   sys.dm_hadr_availability_replica_cluster_nodes n
                  JOIN sys.dm_hadr_availability_replica_cluster_states cs
                    ON n.replica_server_name = cs.replica_server_name
                  JOIN sys.dm_hadr_availability_replica_states rs
...
SELECT @description = 'Records the Table Sizes '
...
SET @StepName = 'Run PerfStats..CaptureErrorLogFile'
...
EXEC msdb.dbo.Sp_update_jobstep
  ...
  @os_run_priority=0,
  @subsystem=N'TSQL',
  @command=N'exec PerfStats..CaptureErrorLogFile @WhatIf = 0',
