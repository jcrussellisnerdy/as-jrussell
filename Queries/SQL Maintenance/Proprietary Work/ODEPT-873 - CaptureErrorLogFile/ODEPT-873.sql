...
AS
  BEGIN
      DECLARE @Path           NVARCHAR(250),
              @ErrorLogSize   NVARCHAR(100),
              @ErrorLogSizeKB NVARCHAR(100),
              @ErrorLogSizeGB NVARCHAR(100),
              @MaxCount       NVARCHAR(100),
              @ExecCommand    VARCHAR(150),
      ...
      --exec  [PerfStats].[dbo].[CaptureErrorLogFile] @WhatIF= 0 Adds to table if it's over 50 GB if not produces how much the combined error logs are 
      ...
      IF @WhatIf = 0
        ...
        IF @ErrorLogSize >= 20000 -- this is in MB which translates to 20 GB
              BEGIN
                  ...
                  IF @ErrorLogSize >= 75000 -- this is in MB which translates to 75 GB; System will auto force the log to be recreated so that it can be removed if desired (or protection of continous problem).
                    BEGIN
            ...
            FROM   #ErrorLogs
              ...
              END
        END
      ELSE IF @Force = 0 -- No matter what the size is the system will auto force the log to be recreated so that it can be removed if desired (or protection of continous problem).
        BEGIN
            EXEC Sp_cycle_errorlog;
