...
/*
 ...
 SELECT open_tran,* FROM master.sys.sysprocesses WHERE SPID=285
go
*/
...
--SELECT COUNT(*)
   FROM    sys.dm_exec_requests a
            LEFT JOIN sys.dm_exec_sessions ses ON ses.session_id = a.session_idhm
            CROSS APPLY sys.dm_exec_sql_text(a.sql_handle) b
    WHERE   a.status <> 'background'
            AND DATEDIFF(MINUTE, a.start_time, CURRENT_TIMESTAMP) > 121--(or 241 in Non Production)
            AND ses.original_login_name NOT IN ('SYSTEM', 'UniTrac-Daemon', 'UTdbInternalDashWinServiceProd', 'ELDREDGE_A\UniTrac-Daemon', 'ELDREDGE_A\RRankins', 'UTPushUser')
...
--~ 
	...
	--Retrieve data about the calling session?yu
	@show_own_spid BIT = 0,
...
BEGIN;
 ...
 declare @filter  NVARCHAR(max)	
	Declare @filter_type  NVARCHAR(max)
	Declare @not_filter  NVARCHAR(max)
	Declare @not_filter_type  NVARCHAR(max)
	Declare @show_own_spid  NVARCHAR(max)
	Declare @show_system_spids  NVARCHAR(max)
	Declare @show_sleeping_spids  NVARCHAR(max)
	Declare @get_full_inner_text  NVARCHAR(max)
	Declare @get_plans  NVARCHAR(max)
	Declare @get_outer_command  NVARCHAR(max)
	Declare @get_transaction_info  NVARCHAR(max)
	Declare @get_task_info  NVARCHAR(max)
	Declare @get_locks  NVARCHAR(max)
	Declare @get_avg_time  NVARCHAR(max)
	Declare @get_additional_info  NVARCHAR(max)
	Declare @find_block_leaders  NVARCHAR(max)
	Declare @delta_interval  NVARCHAR(max)
	Declare @format_output  NVARCHAR(max)
	Declare @output_column_list  NVARCHAR(max)
	Declare @sort_order  NVARCHAR(max)
	Declare @return_schema  NVARCHAR(max)
