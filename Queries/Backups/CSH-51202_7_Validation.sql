DECLARE @DatabaseName nvarchar(100) = 'IVOS_tools' 
DECLARE @Acct nvarchar(100) = 'APP_ACCESS'
...
select DB_NAME(), rp.name [Application Role Name] ,rp.type_desc, mp.name [Login Name], mp.type, mp.default_schema_name
	from sys.database_role_members dm
	join  sys.database_principals rp on rp.principal_id = dm.role_principal_id
	join  sys.database_principals mp on mp.principal_id = dm.member_principal_id
	where rp.name  like ''%'+ @Acct  +'%''
...
select DB_NAME()[Database Name], USER_NAME(grantee_principal_id),OBJECT_NAME(major_id), permission_name,
