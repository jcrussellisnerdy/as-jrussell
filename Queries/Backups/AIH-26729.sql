...
USE ROLE OWNER_EDW_STGSUPPORT; --OWNER OF THE DB
USE WAREHOUSE EDW_STAGE_MED_WH;  --YOUR DEFAULT WAREHOUSE WOULD BE SUPPLIED 
USE DATABASE EDW_STAGE; ---DATABASE WHERE SCHEMA IS GOING 

BEGIN
	CASE WHEN (select count(*) from information_schema.schemata  where CATALOG_NAME = 'EDW_STAGE'AND SCHEMA_NAME = 'FLYWAY' ) = 0
	THEN
...
BEGIN
	CASE WHEN (select count(*) from information_schema.schemata  where CATALOG_NAME = 'EDW_STAGE'AND SCHEMA_NAME = 'FLYWAY' ) = 1
	THEN
...
USE ROLE SYSADMIN; 
CALL DBA.DEPLOY.USER_ACCESS('OCTOPUSDEPLOY2', '"OWNER_EDW_STGSUPPORT"');
...
END;
GRANT USAGE ON SCHEMA DBA TO ROLE OWNER_EDW_STGSUPPORT;


use role ACCOUNTADMIN;
use database EDW_STAGE;
USE SCHEMA DBA;
...
GRANT USAGE ON PROCEDURE TABLESMODIFIED(VARCHAR) TO ROLE "OWNER_EDW_STGSUPPORT" ;
