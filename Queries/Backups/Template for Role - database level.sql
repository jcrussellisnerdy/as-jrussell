...
DECLARE @AppRole varchar(100) = 'SomeMoreAccess_APP_ACCESS'
DECLARE @TargetDB varchar(100) = 'TestDB' 
DECLARE @AccountRoot varchar(100) = 'SomeMoreAccess_APP_ACCESS1'
...
USE '+ @TargetDB +'
IF NOT EXISTS (SELECT 1 FROM sys.database_permissions where TYPE = ''CO'' AND USER_NAME(grantee_principal_id) ='''+ @AppRole +''')
BEGIN
...
USE '+ @TargetDB +'
IF NOT EXISTS (SELECT 1 FROM sys.database_permissions where TYPE = ''CRTB'' AND USER_NAME(grantee_principal_id) ='''+ @AppRole +''')
BEGIN
...
USE '+ @TargetDB +'
IF NOT EXISTS (SELECT 1 FROM sys.database_permissions where TYPE = ''CRVW'' AND USER_NAME(grantee_principal_id) ='''+ @AppRole +''')
BEGIN
		GRANT CREATE VIEW TO  ['+ @AppRole +'] 
		PRINT ''SUCCESS: GRANT CREATE VIEW  was issued to ' + @AppRole + ' on ' + @TargetDB  +'''
END
	ELSE 
BEGIN
		PRINT ''WARNING: GRANTS ALREADY EXISTS''
