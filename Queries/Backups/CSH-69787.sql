...
END;'
END
IF  @AppRole <> '' AND @Object <> '' AND @Type  <> '' 
BEGIN
SELECT @ObjectPermission ='
USE ['+ @TargetDB +']
IF NOT EXISTS (SELECT 1 FROM sys.database_permissions where OBJECT_NAME(major_id) ='''+ @Object + '''AND TYPE = '''+ @TYPE  +''' AND USER_NAME(grantee_principal_id) ='''+ @AppRole +''')
BEGIN
		GRANT '+ @Permission + ' ON [dbo]. [' + @Object +'] TO  ['+ @AppRole +'] 
	PRINT ''SUCCESS: GRANT ' + @Permission +' was issued to ' + @AppRole + ' on ' + @TargetDB  +'on object name: '+ @Object + ' ''
END
	ELSE 
BEGIN
