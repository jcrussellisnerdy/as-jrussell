...
DECLARE @sqlcmdObject3 VARCHAR(MAX)
DECLARE @sqlcmdObject4 VARCHAR(MAX)
...
DECLARE @AccountRoot VARCHAR(100) = 'svc_ORCL_PRD01' --User Account 
DECLARE @Password VARCHAR(100) = ''
DECLARE @ObjectName VARCHAR(100) = 'GetDailyPostingActivity' --Object either Stored Proc/Table/etc.
...
DECLARE @ObjectName2 VARCHAR(100) = 'GetServiceFeeWorkItemDetails' --Object either Stored Proc/Table/etc.
DECLARE @TypeObject2 VARCHAR(10) ='EX'---lowest and most common access for stored proc is: EX and Tables is: SL
DECLARE @PermissionObject2 VARCHAR(100)
DECLARE @ObjectName3 VARCHAR(100) = 'GetLenderPayGPFileData' --Object either Stored Proc/Table/etc.
DECLARE @TypeObject3 VARCHAR(10) = 'EX'---lowest and most common access for stored proc is: EX and Tables is: SL
DECLARE @PermissionObject3 VARCHAR(100)
DECLARE @ObjectName4 VARCHAR(100) = 'GetFinanceFileIds' --Object either Stored Proc/Table/etc.
DECLARE @TypeObject4 VARCHAR(10) ='EX' ---lowest and most common access for stored proc is: EX and Tables is: SL
DECLARE @PermissionObject4 VARCHAR(100)
...
SELECT @PermissionObject3 = permission_name
FROM   Fn_builtin_permissions(DEFAULT)
WHERE  type = @TypeObject3

SELECT @PermissionObject4 = permission_name
FROM   Fn_builtin_permissions(DEFAULT)
WHERE  type = @TypeObject4
...
BEGIN
		CREATE LOGIN ['
                              ...
                              + @Password + ''', DEFAULT_DATABASE=[tempdb], CHECK_EXPIRATION=OFF, CHECK_POLICY=ON
		PRINT ''SUCCESS: LOGIN CREATED''
END
	ELSE 
BEGIN
...
END'
      ...
      IF @ObjectName3 <> ''
         AND @PermissionObject3 <> ''
        BEGIN
            SELECT @sqlcmdObject3 = '
USE [' + @TargetDB
                               + ']
IF NOT EXISTS (SELECT 1 FROM sys.database_permissions where OBJECT_NAME(major_id) ='''
                               + @ObjectName3 + '''AND TYPE = ''' + @TypeObject3
                               + ''' AND USER_NAME(grantee_principal_id) ='''
                               + @AppRole + ''')
BEGIN
		GRANT ' + @PermissionObject3
                               + ' ON [dbo]. [' + @ObjectName3 + '] TO  ['
                               + @AppRole + '] 
	PRINT ''SUCCESS: GRANT '
                               + @PermissionObject3 + ' was issued to ' + @AppRole
...
END'
      ...
      IF @ObjectName4 <> ''
         AND @PermissionObject4 <> ''
        BEGIN
            SELECT @sqlcmdObject4 = '
USE [' + @TargetDB
                               + ']
IF NOT EXISTS (SELECT 1 FROM sys.database_permissions where OBJECT_NAME(major_id) ='''
                               + @ObjectName4 + '''AND TYPE = ''' + @TypeObject4
                               + ''' AND USER_NAME(grantee_principal_id) ='''
                               + @AppRole + ''')
BEGIN
		GRANT ' + @PermissionObject4
                               + ' ON [dbo]. [' + @ObjectName4 + '] TO  ['
                               + @AppRole + '] 
	PRINT ''SUCCESS: GRANT '
                               + @PermissionObject4 + ' was issued to ' + @AppRole
                               + ' on ' + @TargetDB + ' for ' + @ObjectName4 + '''
END
	ELSE 
BEGIN
		PRINT ''WARNING: GRANTS ALREADY EXISTS''
