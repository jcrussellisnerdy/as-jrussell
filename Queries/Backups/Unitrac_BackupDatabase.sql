...
/****** Object:  StoredProcedure [backup].[BackupDatabase]    Script Date: 10/24/2022 8:10:54 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [backup].[BackupDatabase]
...
BEGIN
        ...
        BEGIN
            IF( EXISTS( SELECT * FROM MASTER..SYSOBJECTS where name = 'dm_hadr_cluster') AND (@SQLDatabaseName = 'USER_DATABASES') )
                BEGIN
                    ...
                    ELSE
                        BEGIN
                            SET @ClientHost = ( SELECT [info].[GetSystemConfig] ( 'Instance.Name', UPPER( convert(varchar(100), SERVERPROPERTY('MachineName')) )) +'.'+ [info].[GetSystemConfig] ( 'Instance.Domain', '' ) )
                        END
                END
            ELSE
                BEGIN  
    ...
    ELSE IF( @BackupSoftware = 'AWS-RDS')
        BEGIN
            IF( @SQLBackupPath is null ) SELECT @SQLBackupPath = [info].GetDatabaseConfig('AWS-RDS','StorageGatewayPath', 'RDS-WalkAway'); 
            IF( @RetentionDays = 0 ) SELECT @RetentionDays = info.GetDatabaseConfig('AWS-RDS','Retention', '');
        END
