...
IF NOT EXISTS (select * from sys.syslogins where loginname ='ELDREDGE_A\SvcClmIsoImportProd' )
CREATE LOGIN [ELDREDGE_A\SvcClmIsoImportProd] FROM WINDOWS WITH DEFAULT_DATABASE=[RepoPlusAnalytics]
ELSE 
...
IF NOT EXISTS (select * from sys.sysusers where name = 'ELDREDGE_A\SvcClmIsoImportProd' )
CREATE USER [ELDREDGE_A\SvcClmIsoImportProd] FOR LOGIN [ELDREDGE_A\SvcClmIsoImportProd];
ELSE 
...
IF NOT EXISTS (	SELECT * FROM sys.database_permissions where USER_NAME(grantee_principal_id) ='REPO_APP_ACCESS' and OBJECT_NAME(major_id) = 'IsoInsuranceData' )
GRANT UPDATE ON [RepoPlusAnalytics].[dbo].[IsoInsuranceData] TO [REPO_APP_ACCESS];
ELSE 
PRINT 'Table access for IsoInsuranceData already exists' 


IF NOT EXISTS (	SELECT * FROM sys.database_permissions where USER_NAME(grantee_principal_id) ='REPO_APP_ACCESS' and OBJECT_NAME(major_id) = 'IsoClaimData' )
...
ALTER ROLE [REPO_APP_ACCESS] ADD MEMBER [ELDREDGE_A\SvcClmIsoImportProd];
ELSE 
