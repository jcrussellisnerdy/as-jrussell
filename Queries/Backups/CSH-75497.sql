...
IF NOT EXISTS (SELECT *
               FROM   sys.objects
               WHERE  object_id = Object_id(N'[deploy].[SetDatabaseRename]')
                      AND type IN ( N'P', N'PC' ))
  BEGIN
      /* Create Empty Stored Procedure */
      EXEC dbo.Sp_executesql
        @statement = N'CREATE PROCEDURE [deploy].[SetDatabaseRename] AS RETURN 0;';
  END;

GO

/* Alter Stored Procedure */
ALTER PROCEDURE [deploy].[SetDatabaseRename] (@AppName VARCHAR(50) = 'RFPL', @old VARCHAR(128) ='_PROD', @new VARCHAR(128)= '_UAT',   @WhatIF BIT = 1)
...
WHILE EXISTS(SELECT *
  ...
  BEGIN
      ...
      SELECT @SQL = 'EXEC Sp_renamedb ' + @TargetName + ',' + @newname+ ';				  
                PRINT ''The ' + @TargetName+ ' database has been renamed to ' + @newname+ '.'';'
