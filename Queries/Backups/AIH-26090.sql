...
DECLARE @AppRole VARCHAR(30) = 'INFA_APP_ACCESS'
...
FROM   sys.databases
WHERE  name IN ('PRL_FIRSTHELPFINANCIAL_8948_PROD','PRL_YAMAHAFINANCE_8942_PROD','PRL_GREATERNEVADACU_6748_PROD')
...
USE [' + @DatabaseName 
                                       + ']
IF NOT EXISTS (SELECT 1 FROM sys.database_permissions where CLASS_DESC = ''DATABASE'' AND TYPE = ''' + @TYPE
...
BEGIN
	...
	PRINT ''SUCCESS: GRANT ' + @Permission
                                       + ' was issued to ' + @AppRole + ' on ' + @DatabaseName 
                                       + '''
END
	ELSE 
BEGIN
