...
DECLARE @AppRole varchar(30) = 'db_datareader'--db_datareader, db_datawriter, db_owner (Only for SAs) 
DECLARE @AppRole2 varchar(30) = 'db_datawriter'--db_datareader, db_datawriter, db_owner (Only for SAs) 
DECLARE @TargetDB varchar(100) = 'ACSYSTEM' 
DECLARE @AccountRoot varchar(100) = 'ELDREDGE_A\SQL_OCR_ADMIN_TOOL'
...
IF NOT EXISTS (select mp.name, rp.name [Application Role Name] ,rp.type_desc, mp.name [Login Name], mp.type, mp.default_schema_name 
				...
				join  sys.database_principals rp on rp.principal_id = dm.role_principal_id
				join  sys.database_principals mp on mp.principal_id = dm.member_principal_id
				where mp.name = '''+ @AccountRoot +''' AND RP.NAME ='''+@AppRole2+''')
