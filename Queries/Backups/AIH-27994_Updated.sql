...
DECLARE @JobName        VARCHAR(200) = 'UTRC-HijackServiceCenterForCondoUnitStatus',
        ...
        @JobCategory    VARCHAR(100) = 'Unitrac',-- Was Unitrac 
        @description    VARCHAR(256),
        @notifyOperator VARCHAR(100) = 'UniTracAdmins';
/* JOB required parameter */
DECLARE @Version NUMERIC(18, 10),
        @cmd     NVARCHAR(MAX),
        @output  NVARCHAR(MAX)

IF @EnableNewJobs IS NULL 
BEGIN
IF EXISTS (SELECT rs.role_desc
  ...
  END
ELSE IF EXISTS (SELECT rs.role_desc
           FROM   sys.dm_hadr_availability_replica_cluster_nodes n
                  JOIN sys.dm_hadr_availability_replica_cluster_states cs
                    ON n.replica_server_name = cs.replica_server_name
                  JOIN sys.dm_hadr_availability_replica_states rs
...
SELECT @description = 'Proc name: HijackServiceCenterForCondoUnitStatus

Pass in lendercode as @LenderCode

Please run once a night. The time of day can be chosen based on typical DB load. I assume 8 - 10 pm would be a good time.
AIH-27993
AIH-27994
'
...
SET @StepName = 'Run UniTrac..HijackServiceCenterForCondoUnitStatus_6155'
...
EXEC msdb.dbo.Sp_update_jobstep
  ...
  @os_run_priority=0,
  @subsystem=N'TSQL',
  @command=N'IF EXISTS (SELECT 1 FROM SYS.OBJECTS WHERE NAME = ''HijackServiceCenterForCondoUnitStatus'') BEGIN EXEC dbo.HijackServiceCenterForCondoUnitStatus @lenderCode = ''6155'' END',
...
SET @StepName = 'Run UniTrac..HijackServiceCenterForCondoUnitStatus_4392'
...
EXEC msdb.dbo.Sp_update_jobstep
  ...
  @os_run_priority=0,
  @subsystem=N'TSQL',
  @command=N'IF EXISTS (SELECT 1 FROM SYS.OBJECTS WHERE NAME = ''HijackServiceCenterForCondoUnitStatus'') BEGIN EXEC dbo.HijackServiceCenterForCondoUnitStatus @lenderCode = ''4392'' END', 
  @database_name=N'Unitrac',
  @flags=4
