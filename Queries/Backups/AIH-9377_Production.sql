...
END
	...
	USE [master]
IF NOT EXISTS (SELECT * FROM sys.database_principals where name = 'ELDREDGE_A\svc_ctpt_prd01')
BEGIN
		CREATE LOGIN [ELDREDGE_A\svc_ctpt_prd01] FROM WINDOWS WITH DEFAULT_DATABASE=[tempdb]

	USE [IND_AlliedSolutions_157GIC109]
		CREATE USER [ELDREDGE_A\svc_ctpt_prd01] FOR LOGIN [ELDREDGE_A\svc_ctpt_prd01];
...
BEGIN
		...
		ALTER ROLE [CP_APP_ACCESS] ADD MEMBER [ELDREDGE_A\svc_ctpt_prd01];
