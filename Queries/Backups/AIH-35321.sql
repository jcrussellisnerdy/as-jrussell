...
DECLARE @AppServerRole2 VARCHAR(100) ='db_executor'
DECLARE @AccountRoot VARCHAR(100)   
DECLARE @Password VARCHAR(100) = ''
DECLARE @DryRun INT = 0
DECLARE @DatabaseName SYSNAME

--EXEC DBA.deploy.SetDatabaseRole @DryRun = 0
select top 1 @AccountRoot =
 CASE WHEN ServerEnvironment = 'DEV'  THEN   'ELDREDGE_A\svc_AZDO_DEV01'
 WHEN ServerEnvironment = 'TST'  THEN   'ELDREDGE_A\svc_AZDO_TST01'
 WHEN ServerEnvironment = 'STG'  THEN   'ELDREDGE_A\svc_AZDO_STG01'
 WHEN ServerEnvironment = 'PRD'  THEN   'ELDREDGE_A\svc_AZDO_PRD01'
 END
--select *
...
FROM   sys.databases
WHERE  name IN ('HDTStorage','LetterGen','QCModule','Unitrac','UNITRAC_MORTGAGE','VUT')
...
BEGIN
		...
		ALTER ROLE ['+@AppServerRole2+'] ADD MEMBER  ['+@AppRole+']
