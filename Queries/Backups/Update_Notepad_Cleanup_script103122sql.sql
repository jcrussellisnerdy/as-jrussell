...
IF EXISTS(SELECT [name] FROM tempdb.sys.tables WHERE [name] like '#Notepad_Cleanup%') 
BEGIN
   DROP TABLE #Notepad_Cleanup ;
END; 

/* Step 1 - Calculate rows to be changed - same query as populate Storage table */
with g1 as (
Select Row_Number() over (partition by cast(Body as nvarchar(2000)) order by notepad_id) as grank,* from notepad
where add_Date > '10/7/2022'
and overview = 'Police Report'
and add_user = 'svcPAIR_Inbound'
),
g2 as (
	Select distinct claim_id from g1
	where claim_id in (1548554
		,1548575
		,1548580
		,1548581
		,1548594
...
_data as (
Select Row_Number() over (partition by cast(Body as nvarchar(2000)) order by notepad_id) as grank, n.*
from notepad n
inner join g2 on g2.claim_id = n.claim_id
where add_Date > '10/7/2022'
...
IF NOT EXISTS (SELECT *
	...
	IF( @@RowCount =  @RowsToChange )
		BEGIN
			...
			FROM notepad n
  			INNER JOIN #Notepad_Cleanup nc on n.notepad_id = nc.notepad_id
			WHERE n.claim_id <> '1560362'
           ...
           ( [TableName], [Counter], [total_rows], [RowsToChange], [DateTimeStamp] )
			VALUES
           ( 'notepad', @counter, @total_rows, @RowsToChange, getdate() )
            END
