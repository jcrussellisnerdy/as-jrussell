...
DECLARE @AppRole VARCHAR(100) = 'CTPT_APP_ACCESS' ---Name of Role Application Acroynm_APP_ACCESS 
DECLARE @TargetDB VARCHAR(100) = 'Unitrac' ---Database Name 
DECLARE @AccountRoot VARCHAR(100) = '' --User Account 
DECLARE @ObjectName VARCHAR(100) = 'document_management' --Object either Stored Proc/Table/etc.
DECLARE @TypeObject VARCHAR(10) = 'SL' ---lowest and most common access for stored proc is: EX and Tables is: SL
DECLARE @PermissionObject VARCHAR(100)
DECLARE @ObjectName2 VARCHAR(100) = 'LENDER' --Object either Stored Proc/Table/etc.
DECLARE @TypeObject2 VARCHAR(10) = 'SL' ---lowest and most common access for stored proc is: EX and Tables is: SL
DECLARE @PermissionObject2 VARCHAR(100)
DECLARE @DryRun INT = 0

--select * from UniTrac.dbo.document_management
