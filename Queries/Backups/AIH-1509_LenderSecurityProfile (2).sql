...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[CP_LenderSecurityProfile]') AND type in (N'U'))
BEGIN
...
IF EXISTS(SELECT *
              FROM sys.columns 
              WHERE Name      = N'Id'
                AND Object_ID = Object_ID(N'[CP_LenderSecurityProfile]'))
BEGIN
...
DROP TABLE [CenterPointSecurity].[dbo].[CP_LenderSecurityProfile]
