...
IF NOT EXISTS (SELECT *
               FROM   sys.objects
               WHERE  object_id = Object_id(N'[dbo].[GetLenderInfo2]')
                      AND type IN ( N'P', N'PC' ))
  BEGIN
      /* Create Empty Stored Procedure */
      EXEC dbo.Sp_executesql
        @statement = N'CREATE PROCEDURE [dbo].[GetLenderInfo2] AS RETURN 0;';
  END;

GO

/* Alter Stored Procedure */
---EXEC [dbo].[GetLenderInfo2] @Lender_List ='3Rivers'
ALTER PROCEDURE [dbo].[Getlenderinfo2] (@LENDER_LIST NVARCHAR(100)='')
AS
  BEGIN
...
FROM   #USERS1
	 ...
	 SELECT DISTINCT LI.NAME_TX,
						 ...
						 GAPLatestProductDate , MBPLatestProductDate , DPWLatestProductDate ,
						 VPPLatestProductDate , CILatestProductDate , DPLatestProductDate 
						 INTO #LDR_INFO
         --SELECT *
         FROM   #LENDER_INFO LI
                JOIN #TRAINO
	 ...
	 Row_number()
                  OVER (
                    ...
                    ORDER BY GAPLatestProductDate DESC, MBPLatestProductDate DESC, DPWLatestProductDate DESC, VPPLatestProductDate DESC, CILatestProductDate DESC, DPLatestProductDate DESC) 
				AS PRODUCT_RECENCY
	,
                * INTO #ORDERED_INFO
         FROM  	  #LDR_INFO
